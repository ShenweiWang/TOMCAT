...
public class TestCometProcessor extends TomcatBaseTest {
	...
	public void testAsyncClose() throws Exception {
		...
		count = 0;
		while (count < 10
				&& !readThread.getResponse().contains(
	...
	private static class SimpleCometServlet extends HttpServlet implements
		...
		public void init() throws ServletException {
			failOnBegin = Boolean.valueOf(
					getServletConfig().getInitParameter(FAIL_ON_BEGIN))
					.booleanValue();
			failOnRead = Boolean.valueOf(
					getServletConfig().getInitParameter(FAIL_ON_READ))
					.booleanValue();
			failOnEnd = Boolean.valueOf(
					getServletConfig().getInitParameter(FAIL_ON_END))
					.booleanValue();
		}
