...
/**
 * Simple client for unit testing. It isn't robust, it isn't secure and should
 * not be used as the basis for production code. Its only purpose is to do the
 * bare minimum for the unit tests.
 */
public abstract class SimpleHttpClient {
	...
	/*
	 * Send the component parts of the request (be tolerant and simply skip null
	 * entries)
	 */
	...
	/*
	 * Accumulate the response headers into a map, and extract interesting
	...
	/*
	 * Read the body of the server response. Save its contents and search it for
	 * uri-elements only if requested
	 */
	...
	/*
	 * Scan an html body for a given html uri element, starting from the given
	 * index into the source string. If any are found, simply accumulate them as
	 * literal strings, including angle brackets. note: nested elements will not
	 * be collected.
	 * 
