...
public class TestHostConfigAutomaticDeployment extends TomcatBaseTest {
	 ...
	 * unpackWARs XML WAR DIR Y/N Y/N Y/N Y N N
	 * 
	 ...
	 * external WAR. deployXML copyXML unpackWARs XML WAR DIR Y/N Y/N Y Y N Y
	 * Y/N Y/N N Y N N
	 * 
	 * Notes: No WAR file is present in the appBase because it is an external
	 * WAR. Any context.xml file embedded in the external WAR file is ignored.
	 */
	@Test
	...
	/*
	 * Expected behaviour for deployment of an XML file that points to an
	 * external DIR. deployXML copyXML unpackWARs XML WAR DIR Y/N Y/N Y/N Y N N
	 * 
	 ...
	 * deployXML copyXML unpackWARs XML WAR DIR N Y/N N N Y N N Y/N Y N Y Y Y N
	 * N N Y N Y N Y N Y Y Y Y N Y Y N Y Y Y Y Y Y
	 */
	@Test
	...
	public void testDeploymentWarXmlFFT() throws Exception {
		createWar(WAR_XML_SOURCE, true);
		doTestDeployment(false, false, true, LifecycleState.FAILED, null,
				false, true, true);
	}
	...
	public void testDeploymentWarXmlFTF() throws Exception {
		createWar(WAR_XML_SOURCE, true);
		doTestDeployment(false, true, false, LifecycleState.FAILED, null,
				false, true, false);
	}
	 ...
	 * deployXML copyXML unpackWARs XML WAR DIR Y/N Y/N N N Y N Y/N Y/N Y N Y Y
	 */
	@Test
	...
	public void testDeploymentWarFTT() throws Exception {
		createWar(WAR_SOURCE, true);
		doTestDeployment(false, true, true, LifecycleState.STARTED, null,
				false, true, true);
	}
	...
	public void testDeploymentWarTFT() throws Exception {
		createWar(WAR_SOURCE, true);
		doTestDeployment(true, false, true, LifecycleState.STARTED, null,
				false, true, true);
	}
	...
	public void testDeploymentWarTTF() throws Exception {
		createWar(WAR_SOURCE, true);
		doTestDeployment(true, true, false, LifecycleState.STARTED, null,
				false, true, false);
	}
	 ...
	 * deployXML copyXML unpackWARs XML WAR DIR N Y/N Y/N N N Y Y N Y/N N N Y Y
	 * Y Y/N Y N Y
	 */
	@Test
	 ...
	 * deployXML copyXML unpackWARs XML WAR DIR Y/N Y/N Y/N N N Y
	 */
	@Test
	...
	private void doTestDeployment(boolean deployXML, boolean copyXML,
		...
		File xml = new File(getConfigBaseFile(host), APP_NAME.getBaseName()
				+ ".xml");
		Assert.assertEquals(Boolean.valueOf(resultXml),
				Boolean.valueOf(xml.isFile()));

		File war = new File(getAppBaseFile(host), APP_NAME.getBaseName()
				+ ".war");
		Assert.assertEquals(Boolean.valueOf(resultWar),
				Boolean.valueOf(war.isFile()));

		File dir = new File(host.getAppBase(), APP_NAME.getBaseName());
		Assert.assertEquals(Boolean.valueOf(resultDir),
	 ...
	 * 
	 * Artifacts present Artifact Artifacts remaining XML WAR EXT DIR Removed
	 * XML WAR EXT DIR Notes N N N Y DIR - - - N N Y N N WAR - N - - N Y N Y DIR
	 * - Y - R 1 N Y N Y WAR - N - N Y N N N XML N - - - Y N N Y DIR N - - N Y N
	 * N Y XML R - - Y 2 Y N Y N EXT Y - N - Y N Y N XML N - Y - Y N Y Y DIR R -
	 * Y R 1,2 Y N Y Y EXT Y - N N Y N Y Y XML N - Y N Y Y N N WAR N N - - Y Y N
	 * N XML N N - - Y Y N Y DIR R Y - R 1,2 Y Y N Y WAR N N - - Y Y N Y XML R Y
	 * - Y
	 * 
	 * Notes: 1. The DIR will be re-created since unpackWARs is true. 2. The XML
	 * will be extracted from the WAR/DIR if deployXML and copyXML are true.
	 */
	@Test
	...
	public void testDeleteXmlRemoveXml() throws Exception {
		doTestDelete(true, false, false, false, false, XML, false, false,
				false, null);
	}
	 ...
	 * 
	 * Artifacts present Artifact Artifacts remaining XML WAR EXT DIR Modified
	 * XML WAR EXT DIR Action N N N Y DIR - - - M None N Y N N WAR - M - -
	 * Redeploy N Y N Y DIR - Y - M None N Y N Y WAR - M - R Redeploy Y N N N
	 * XML M - - - Redeploy Y N N Y DIR Y - - M None Y N N Y XML M - - Y
	 * Redeploy Y N Y N EXT Y - M - Reload if WAR Y N Y N XML M - Y - Redeploy Y
	 * N Y Y DIR Y - Y M None Y N Y Y EXT Y - M R Reload Y N Y Y XML M - Y Y
	 * Redeploy Y Y N N WAR Y M - - Reload Y Y N N XML M Y - - Redeploy Y Y N Y
	 * DIR Y Y - M None Y Y N Y WAR Y M - - Reload Y Y N Y XML M Y - Y Redeploy
	 */
	@Test
	 ...
	 * 
	 * Artifacts present copyXML deployXML Artifact Artifacts remaining XML WAR
	 * EXT DIR Added XML WAR EXT DIR Action N Y N N N Y DIR - Y - A None N N N Y
	 * N Y WAR - A - R Redeploy Y N N N N Y DIR Y - - A None N N N Y N Y XML A -
	 * - Y Redeploy Y N N N N Y WAR Y A - - Reload N Y N N N Y XML A Y - -
	 * Redeploy Y Y N N N Y DIR Y Y - A None Y N N Y N Y WAR Y A - N Reload N Y
	 * N Y N Y XML A Y - Y Redeploy Y N Y N N Y DIR Y - Y A None Y N Y N N Y WAR
	 * Y A Y - None N N N Y N Y EXT A - A R Redeploy N Y N N N Y EXT A Y A -
	 * Redeploy
	 * 
	 * N N N Y Y/N N DIR+XML - - - Y Redeploy (failed) N N N Y Y Y DIR+XML A - -
	 * Y Redeploy N N N Y N Y DIR+XML - - - Y Redeploy
	 * 
	 * Addition of a file is treated as if the added file has been modified with
	 * the following additional actions: - If a WAR is added, any DIR is removed
	 * and may be recreated depending on unpackWARs. - If an XML file is added
	 * that refers to an external docBase any WAR or DIR in the appBase will be
	 * removed. The DIR may be recreated if the external resource is a WAR and
	 * unpackWARs is true. - If a DIR is added when a WAR already exists and
	 * unpackWARs is false, the DIR will be ignored but a warning will be logged
	 * when the DIR is first detected. If the WAR is removed, the DIR will be
	 * left and may be deployed via automatic deployment. - If a WAR is added
	 * when an external WAR already exists for the same context, the WAR will be
	 * treated the same way as a DIR is treated in the previous bullet point.
	 */
	@Test
	public void testAdditionWarAddDir() throws Exception {
		doTestAddition(false, false, false, true, false, DIR, false, true,
				true, WAR_COOKIE_NAME, NONE);
	}

	@Test
	public void testAdditionDirAddWar() throws Exception {
		doTestAddition(false, false, false, false, true, WAR, false, true,
				true, WAR_COOKIE_NAME, REDEPLOY);
	}

	@Test
	public void testAdditionXmlAddDir() throws Exception {
		doTestAddition(true, false, false, false, false, DIR, true, false,
				true, XML_COOKIE_NAME, NONE);
	}

	@Test
	public void testAdditionDirAddXml() throws Exception {
		doTestAddition(false, false, false, false, true, XML, true, false,
				true, XML_COOKIE_NAME, REDEPLOY);
	}

	@Test
	public void testAdditionXmlAddWar() throws Exception {
		doTestAddition(true, false, false, false, false, WAR, true, true,
				false, XML_COOKIE_NAME, RELOAD);
	}

	@Test
	public void testAdditionWarAddXml() throws Exception {
		doTestAddition(false, false, false, true, false, XML, true, true,
				false, XML_COOKIE_NAME, REDEPLOY);
	}
	...
	public void testAdditionDirAddXmlExtwar() throws Exception {
		doTestAddition(false, false, false, false, true, EXT, true, false,
				true, XML_COOKIE_NAME, REDEPLOY);
	}

	@Test
	public void testAdditionWarAddXmlExtwar() throws Exception {
		doTestAddition(false, false, false, true, false, EXT, true, true,
				false, XML_COOKIE_NAME, REDEPLOY);
	}
	...
	/*
	 * Test context unpackWAR setting. If context.getUnpackWAR !=
	 * Host.getUnpackWARs the Host wins.
	 */
	@Test
	...
	/*
	 * Test context copyXML setting. If context.copyXML != Host.copyXML the Host
	 * wins. For external WARs, a context.xml must always already exist
	 */
	@Test
