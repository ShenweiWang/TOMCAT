...
public class TestWebSocket extends TomcatBaseTest {
	...
	public void testSimple() throws Exception {
		...
		client.writer.write("GET " + TesterEchoServer.Config.PATH_BASIC
				+ " HTTP/1.1" + CRLF);
	...
	public void testKey() throws Exception {
		...
		while (!responseHeaderLine.equals("")) {
			if (responseHeaderLine.startsWith("Sec-WebSocket-Accept: ")) {
				accept = responseHeaderLine.substring(responseHeaderLine
						.indexOf(":") + 2);
				break;
			}
			responseHeaderLine = client.reader.readLine();
	...
	private static class Bug53339WsInbound extends MessageInbound {
		...
		protected void onOpen(WsOutbound outbound) {
			...
			try {
				...
				msg = (String) initCtx.lookup("java:comp/env/" + JNDI_NAME);
	...
	private static class WebSocketClient {
		...
		/*
		 * This is not a real reader but just a thin wrapper around an input
		 * stream that allows to switch encoding during reading. An example is
		 * reading headers using ISO-8859-1 and a body using UTF-8. The class is
		 * not thread-safe and not well-performing.
		 */
		private class CustomReader {
			...
			public CustomReader(InputStream is, String encoding)
					throws IOException {
