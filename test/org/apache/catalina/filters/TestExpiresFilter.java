...
public class TestExpiresFilter extends TomcatBaseTest {
	...
	public void testConfiguration() throws Exception {
		...
		try {
			...
			// VERIFY DEFAULT CONFIGURATION
			{
				...
				Assert.assertEquals(1, expiresConfiguration.getDurations()
						.size());
				Assert.assertEquals(DurationUnit.MONTH, expiresConfiguration
						.getDurations().get(0).getUnit());
				Assert.assertEquals(1,
						expiresConfiguration.getDurations().get(0).getAmount());
			}
			...
			// VERIFY TEXT/HTML
			{
				...
				Assert.assertEquals(3, expiresConfiguration.getDurations()
						.size());
			...
			// VERIFY IMAGE/GIF
			{
				...
				Assert.assertEquals(2, expiresConfiguration.getDurations()
						.size());
				...
				Duration threeMinutes = expiresConfiguration.getDurations()
						.get(1);
			...
			// VERIFY IMAGE/JPG
			{
				...
				Duration tenThousandSeconds = expiresConfiguration
						.getDurations().get(0);
			...
			{
				ExpiresConfiguration expiresConfiguration = expiresFilter
						.getExpiresConfigurationByContentType().get(
								"video/mpeg");
				Assert.assertEquals(StartingPoint.LAST_MODIFICATION_TIME,
						expiresConfiguration.getStartingPoint());
				...
				Duration twentyThousandSeconds = expiresConfiguration
						.getDurations().get(0);
				Assert.assertEquals(DurationUnit.SECOND,
	...
	protected void validate(HttpServlet servlet,
		...
		try {
			...
			String cacheControlHeader = httpURLConnection
