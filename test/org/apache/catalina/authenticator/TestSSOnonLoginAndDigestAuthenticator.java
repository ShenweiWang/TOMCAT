...
/**
 * Test DigestAuthenticator and NonLoginAuthenticator when a SingleSignOn Valve
 * is active.
 *
 * <p>
 ...
 * successfully authenticating a different webapp under the DigestAuthenticator
 * triggers the additional SSO logic for both webapps.
 *
 * <p>
 * Note: these tests are intended to exercise the SSO logic of the
 * Authenticator, but not to comprehensively test all of its logic paths. That
 * is the responsibility of the non-SSO test suite.
 */
public class TestSSOnonLoginAndDigestAuthenticator extends TomcatBaseTest {
	...
	private static final long SHORT_TIMEOUT_DELAY_MSECS = ((SHORT_TIMEOUT_SECS + 3) * 1000);
	private static final int LONG_TIMEOUT_SECS = 10;
	private static final long LONG_TIMEOUT_DELAY_MSECS = ((LONG_TIMEOUT_SECS + 2) * 1000);
	...
	/**
	 * Try to access an unprotected resource without an established SSO session.
	...
	/*
	 * Try to access a protected resource without an established SSO session.
	 ...
	 * 401 status, which will then be followed by successful re-authentication.
	 */
	@Test
	 ...
	 * Logon to access a protected resource using DIGEST authentication, which
	 * will establish an SSO session. Immediately try to access a protected
	 * resource in the NonLogin webapp while sending the SSO session cookie
	 * provided by the first webapp. This should be successful with SC_OK 200
	 * status.
	 */
	@Test
	 ...
	 * status.
	 * 
	 * Then, wait long enough for the DIGEST session to expire. (The SSO session
	...
	public void doTestDigest(String user, String pwd, String uri,
		...
		Map<String, List<String>> reqHeaders1 = new HashMap<String, List<String>>();
		Map<String, List<String>> respHeaders1 = new HashMap<String, List<String>>();
		...
		Map<String, List<String>> respHeaders2 = new HashMap<String, List<String>>();
		...
		if (useServerNonce) {
			if (useServerOpaque) {
				...
				auth.add(buildDigestResponse(user, pwd, digestUri,
						getAuthToken(respHeaders1, REALM),
						getAuthToken(respHeaders1, NONCE), "null", nc1, cnonce,
						qop));
			}
		} else {
			auth.add(buildDigestResponse(user, pwd, digestUri,
					getAuthToken(respHeaders2, REALM), "null",
					getAuthToken(respHeaders1, OPAQUE), nc1, cnonce, QOP));
		}
	...
	protected static String getAuthToken(Map<String, List<String>> respHeaders,
		...
		List<String> authHeaders = respHeaders
				.get(AuthenticatorBase.AUTH_HEADER_NAME);
	...
	/*
	 * Notes from RFC2617 H(data) = MD5(data) KD(secret, data) =
	 * H(concat(secret, ":", data)) A1 = unq(username-value) ":"
	 * unq(realm-value) ":" passwd A2 = Method ":" digest-uri-value
