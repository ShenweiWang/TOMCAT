...
public class LoadTest implements MembershipListener, ChannelListener, Runnable {
	...
	public LoadTest(ManagedChannel channel, boolean send, int msgCount,
			boolean debug, long pause, int stats, boolean breakOnEx) {
			...
			while (total < msgCount) {
				if (channel.getMembers().length == 0 || (!send)) {
					...
					try {
						...
						if (pause > 0) {
							if (debug)
								System.out.println("Pausing sender for "
										+ pause + " ms.");
							Thread.sleep(pause);
						}
						...
						if (debug)
							log.error(
									"Unable to send message:" + x.getMessage(),
									x);
						log.error("Unable to send message:" + x.getMessage());
						ChannelException.FaultyMember[] faulty = x
								.getFaultyMembers();
	 ...
	 *            org.apache.catalina.tribes.MembershipListener method
	 */
	@Override
	...
	public static void usage() {
		...
		System.out
				.println("The load tester can be used in sender or received mode or both");
		System.out
				.println("Usage:\n\t"
	...
	public static void main(String[] args) throws Exception {
		...
		LoadTest test = new LoadTest(channel, send, count, debug, pause, stats,
				breakOnEx);
		...
		while (threads > 1) {
			...
			t.start();
			threads--;
			test = new LoadTest(channel, send, count, debug, pause, stats,
					breakOnEx);
			test.channelOptions = channelOptions;
		}
		test.run();
