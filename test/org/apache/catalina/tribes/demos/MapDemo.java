 ...
 * BackupManager works in a Tomcat cluster
 * 
 * @author fhanik
 * @version 1.1
 */
public class MapDemo implements ChannelListener, MembershipListener {
	...
	public MapDemo(Channel channel, String mapName) {
		...
		table = SimpleTableDemo.createAndShowGUI(map,
				channel.getLocalMember(false).getName());
	...
	/**
	 * Decides if the messageReceived should be invoked will always return false
	 * since we rely on the lazy map to do all the messaging for us
	 */
	@Override
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println("System test complete, time to start="
	...
	public static class Shutdown extends Thread {
		...
		public void run() {
			...
			// create an exit thread that forces a shutdown if the JVM wont exit
			// cleanly
		...
		public void actionPerformed(ActionEvent e) {
			...
			if ("add".equals(e.getActionCommand())) {
				System.out.println("Add key:" + txtAddKey.getText() + " value:"
						+ txtAddValue.getText());
				map.put(txtAddKey.getText(),
						new StringBuilder(txtAddValue.getText()));
			}
