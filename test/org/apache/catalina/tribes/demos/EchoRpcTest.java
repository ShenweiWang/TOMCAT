...
public class EchoRpcTest implements RpcCallback, Runnable {
	...
	public EchoRpcTest(Channel channel, String name, int count, String message,
			long pause, int options, long timeout) {
	 ...
	 *            org.apache.catalina.tribes.tipis.RpcCallback method
	 */
	@Override
	 ...
	 * @param sender
	 *            Member
	 * @return Serializable - null if no reply should be sent TODO Implement
	 *         this org.apache.catalina.tribes.tipis.RpcCallback method
	 */
	@Override
	...
	public void run() {
		...
		while (counter < count) {
			...
			try {
				...
				Response[] resp = rpc.send(channel.getMembers(), msg, options,
						Channel.SEND_OPTIONS_DEFAULT, timeout);
				System.out.println("Send of [" + msg
						+ "] completed. Nr of responses=" + resp.length
	...
	public static void main(String[] args) throws Exception {
		...
		ManagedChannel channel = (ManagedChannel) ChannelCreator
				.createChannel(args);
		EchoRpcTest test = new EchoRpcTest(channel, name, count, message,
				pause, options, timeout);
		channel.start(Channel.DEFAULT);
		Runtime.getRuntime().addShutdownHook(new Shutdown(channel));
		test.run();

		System.out
				.println("System test complete, sleeping to let threads finish.");
