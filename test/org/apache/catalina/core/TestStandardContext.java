...
public class TestStandardContext extends TomcatBaseTest {
	...
	public void testBug49922() throws Exception {
		...
		int rc = getUrl("http://localhost:" + getPort() + "/bug49922/foo",
				result, null);
	...
	/**
	 * Test case for bug 49711: HttpServletRequest.getParts does not work in a
	 * filter.
	 */
	@Test
	public void testBug49711() {
		...
		assertEquals("Incorrect response for GET request", "parts=0",
		...
		assertEquals("Regular multipart doesn't work", "parts=1",
				client.getResponseBody());
		...
		assertEquals(
				"Incorrect response for non-configured casual multipart request",
	...
	private static class Bug49711Servlet extends HttpServlet {
		...
		protected void service(HttpServletRequest req, HttpServletResponse resp)
			...
			out.println("parts="
					+ (null == req.getParts() ? "null" : Integer.valueOf(req
							.getParts().size())));
		}
	}
			...
			try {
				...
				if (makeMultipartRequest) {
					...
					request = new String[] { "POST http://localhost:"
							...
							+ boundary + CRLF + "Content-Length: "
							+ content.length() + CRLF + CRLF + content + CRLF };
				} else {
					request = new String[] { "GET http://localhost:"
							+ getPort() + uri + " HTTP/1.1" + CRLF
							+ "Host: localhost" + CRLF + "Connection: close"
							+ CRLF + CRLF };
				}
