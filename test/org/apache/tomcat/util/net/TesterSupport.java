...
public final class TesterSupport {
	...
	protected static void initSsl(Tomcat tomcat, String keystore,
		...
		if (protocol.indexOf("Apr") == -1) {
			...
			java.net.URL keyStoreUrl = cl
					.getResource("org/apache/tomcat/util/net/" + keystore);
			File keystoreFile = toFile(keyStoreUrl);
			connector.setAttribute("keystoreFile",
					keystoreFile.getAbsolutePath());

			java.net.URL truststoreUrl = cl
			...
			java.net.URL keyStoreUrl = cl
					.getResource("org/apache/tomcat/util/net/localhost-cert.pem");
			File keystoreFile = toFile(keyStoreUrl);
			tomcat.getConnector().setAttribute("SSLCertificateFile",
					keystoreFile.getAbsolutePath());

			java.net.URL sslCertificateKeyUrl = cl
					.getResource("org/apache/tomcat/util/net/localhost-key.pem");
	...
	protected static void configureClientSsl() {
		try {
			...
			sc.init(TesterSupport.getUser1KeyManagers(),
					TesterSupport.getTrustManagers(), null);
			javax.net.ssl.HttpsURLConnection
					.setDefaultSSLSocketFactory(new TesterSSLSocketFactory(sc
							.getSocketFactory()));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private static class TesterSSLSocketFactory extends SSLSocketFactory {
		...
		public Socket createSocket(Socket socket, String s, int i, boolean flag)
				throws IOException {
			SSLSocket result = (SSLSocket) factory.createSocket(socket, s, i,
					flag);
