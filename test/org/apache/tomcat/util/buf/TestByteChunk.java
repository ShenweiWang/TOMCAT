...
public class TestByteChunk {
	 ...
	 *
	 * <p>
	 * As discussed in the "Re: r944918" thread on dev@,
	 * {@code ByteChunk.indexOf()} works for 0-127 ASCII chars only, and cannot
	 * find any chars outside of the range. {@code ByteChunk.findByte()} works
	 * for any ISO-8859-1 chars.
	 */
	@Test
	...
	public void testFindBytes() throws UnsupportedEncodingException {
		...
		assertEquals(
				5,
				ByteChunk.findBytes(bytes, 0, len, new byte[] {
						(byte) '\u00a0', 'x' }));
		assertEquals(
				5,
				ByteChunk.findBytes(bytes, 0, len - 4, new byte[] { 'x',
						(byte) '\u00a0' }));
		assertEquals(len - 1,
				ByteChunk.findBytes(bytes, 2, len, new byte[] { 'x', 'd' }));
		assertEquals(1,
				ByteChunk.findBytes(bytes, 0, len, new byte[] { 'o', 'e' }));
	...
	public void testFindNotBytes() throws UnsupportedEncodingException {
		...
		assertEquals(
				4,
				ByteChunk.findNotBytes(bytes, 0, len, new byte[] { 'l', 'e',
						'H' }));
