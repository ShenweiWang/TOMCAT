...
public class TestLimitLatch {
	...
	public void testOneThreadNoWait() throws Exception {
		...
		testThread.start();
		Thread.sleep(50);
		assertEquals("0 threads should be waiting", 0, latch.getQueuedThreads()
				.size());
	...
	public void testOneThreadWaitCountUp() throws Exception {
		...
		testThread.start();
		Thread.sleep(50);
		assertEquals("1 threads should be waiting", 1, latch.getQueuedThreads()
				.size());
		latch.countDown();
		Thread.sleep(50);
		assertFalse("No threads should be waiting", latch.hasQueuedThreads());
	...
	public void testTenWait() throws Exception {
		...
		Thread.sleep(50);
		assertEquals("20 threads should be waiting", 20, latch
				.getQueuedThreads().size());
		Thread.sleep(1000);
		assertEquals("10 threads should be waiting", 10, latch
				.getQueuedThreads().size());
		Thread.sleep(1000);
		assertFalse("No threads should be waiting", latch.hasQueuedThreads());
	}
