...
public class TestAbstractAjpProcessor extends TomcatBaseTest {
	...
	public void testKeepAlive() throws Exception {
		...
		// Two requests
		for (int i = 0; i < 2; i++) {
			TesterAjpMessage responseHeaders = ajpClient
	...
	public void doTestPost(boolean multipleCL, int expectedStatus)
		...
		TesterAjpMessage bodyMessage = ajpClient.createBodyMessage("test=data"
				.getBytes());

		TesterAjpMessage responseHeaders = ajpClient.sendMessage(
				forwardMessage, bodyMessage);
	...
	private void validateResponseEnd(TesterAjpMessage message,
		...
		Assert.assertEquals(Boolean.valueOf(expectedReuse),
				Boolean.valueOf(reuse));
	}
