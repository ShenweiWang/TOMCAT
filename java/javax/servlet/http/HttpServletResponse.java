...
public interface HttpServletResponse extends ServletResponse {
	...
	/**
	 * Sends an error response to the client using the specified status code and
	 * clears the buffer. This is equivalent to calling
	 * {@link #sendError(int, String)} with the same status code and
	 * <code>null</code> for the message.
	 * 
	 * @param sc
	 *            the error status code
	 * @exception IOException
	 ...
	 * only the first is returned; use {@link #getHeaders(String)} to retrieve
	 * all of them.
	 *
	 * @param name
