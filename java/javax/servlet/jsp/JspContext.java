...
/**
 * <p>
 * <code>JspContext</code> serves as the base class for the PageContext class
 * and abstracts all information that is not specific to servlets. This allows
 * for Simple Tag Extensions to be used outside of the context of a
 * request/response Servlet.
 * <p>
 * The JspContext provides a number of facilities to the page/component author
 ...
 * representing scopes. The implementation must use the underlying machinery
 * corresponding to that scope, so information can be passed back and forth
 * between the underlying environment (e.g. Servlets) and JSP pages. The methods
 * are: <code>setAttribute()</code>, <code>getAttribute()</code>,
 ...
 *
 * <p>
 * The following methods provide <B>programmatic access</b> to the Expression
 * Language evaluator: <code>getExpressionEvaluator()</code>,
 * <code>getVariableResolver()</code>
 *
 * @since 2.0
 */
...
public abstract class JspContext {
	...
	/**
	 * Register the name and value specified with page scope semantics. If the
	 * value passed in is <code>null</code>, this has the same effect as calling
	 * <code>removeAttribute( name, PageContext.PAGE_SCOPE )</code>.
	 *
	 * @param name
	 *            the name of the attribute to set
	 * @param value
	 *            the value to associate with the name, or null if the attribute
	 *            is to be removed from the page scope.
	 ...
	 * calling <code>removeAttribute( name, scope )</code>.
	 * 
	 * @param name
	 *            the name of the attribute to set
	 * @param value
	 *            the object to associate with the name, or null if the
	 *            attribute is to be removed from the specified scope.
	 * @param scope
	 ...
	 *             if the scope is PageContext.SESSION_SCOPE but the page that
	 *             was requested does not participate in a session or the
	...
	/**
	 * Returns the object associated with the name in the page scope or null if
	 * not found.
	 *
	 * @param name
	 *            the name of the attribute to get
	 * @return the object associated with the name in the page scope or null if
	 *         not found.
	 * 
	 * @throws NullPointerException
	 *             if the name is null
	...
	/**
	 * Return the object associated with the name in the specified scope or null
	 * if not found.
	 *
	 * @param name
	 *            the name of the attribute to set
	 ...
	 * @return an enumeration of names (java.lang.String) of all the attributes
	 *         the specified scope
	 * @throws IllegalArgumentException
	 *             if the scope is invalid
	 * @throws IllegalStateException
	 ...
	 * @since 2.0
	 * @deprecated As of JSP 2.1, replaced by javax.el.ELContext.getELResolver()
	 ...
	 * Saves the current "out" JspWriter, and updates the value of the "out"
	 * attribute in the page scope attribute namespace of the JspContext.
	 * <p>
	 ...
	 *
	 * @param writer
	 *            The Writer for the returned JspWriter to send output to.
	 ...
	 * update the value of the "out" attribute in the page scope attribute
	 * namespace of the JspContext.
	 *
