 ...
 * <li>The <code>setJspContext()</code> and <code>setParent()</code> methods are
 * called by the container. The <code>setParent()</code> method is only called
 * if the element is nested within another tag invocation.</li>
 * <li>The setters for each attribute defined for this tag are called by the
 * container.</li>
 * <li>If a body exists, the <code>setJspBody()</code> method is called by the
 * container to set the body of this tag, as a <code>JspFragment</code>. If the
 * action element is empty in the page, this method is not called at all.</li>
