...
/**
 * The interface of a classic tag handler that does not want to manipulate its
 * body. The Tag interface defines the basic protocol between a Tag handler and
 * JSP page implementation class. It defines the life cycle and the methods to
 ...
 * assumed to hold a state that must be preserved. After the doEndTag
 * invocation, the tag handler is available for further invocations (and it is
 * expected to have retained its properties).
 * </p>
 *
 * <p>
 * <B>Lifecycle</B>
 ...
 * following comments:
 * <ul>
 * <li>[1] This transition is intended to be for releasing long-term data. no
 * guarantees are assumed on whether any properties have been retained or not.
 * <li>[2] This transition happens if and only if the tag ends normally without
 * raising an exception
 * <li>[3] Some setters may be called again before a tag handler is reused. For
 * instance, <code>setParent()</code> is called if it's reused within the same
 * page but at a different level, <code>setPageContext()</code> is called if
 * it's used in another page, and attribute setters are called if the values
 * differ or are expressed as request-time attribute values.
 * <li>Check the TryCatchFinally interface for additional details related to
 ...
 * invoked on it. Once a release method is invoked <em>all</em> properties,
 * including parent and pageContext, are assumed to have been reset to an
 * unspecified value. The page compiler guarantees that release() will be
 * invoked on the Tag handler before the handler is released to the GC.
 * </p>
 *
 * <p>
 ...
 * an empty action, by an &lt;body-content&gt; entry of "empty", then the
 ...
 * 
 * <p>
 * If EVAL_BODY_INCLUDE is returned, the body is evaluated and "passed through"
 * to the current out.
 * </p>
 */
...
public interface Tag extends JspTag {
	...
	/**
	 * Evaluate body into existing out stream. Valid return value for
	 * doStartTag.
	 */
	 ...
	 * structure at runtime for cooperation among custom actions; for example,
	 * the findAncestorWithClass() method in TagSupport provides a convenient
	 * way of doing this.
	 *
	 * <p>
	 ...
	 * implementation class, described in the tag-class sub-element of the tag
	 * element. This is extended in an informal manner by allowing the tag
	 * library author to indicate in the description sub-element an observable
	 * type. The type should be a sub-type of the tag handler implementation
	 * class or void. This additional constraint can be exploited by a
	 * specialized container that knows about that specific tag library, as in
	 * the case of the JSP standard tag library.
	 *
	 ...
	 *
	 * <p>
	 * When a Tag returns EVAL_BODY_INCLUDE the result of evaluating the body
	 * (if any) is included into the current "out" JspWriter as it happens and
	 ...
	 * the invocation of doStartTag(), except for a tag handler implementing
	 * BodyTag whose doStartTag() method returns BodyTag.EVAL_BODY_BUFFERED.
	 *
	 ...
	 *
	 * <p>
	 * This method will be called after returning from doStartTag. The body of
	 * the action may or may not have been evaluated, depending on the return
	 * value of doStartTag.
	 *
	 * <p>
	 ...
	 * <p>
	 * The JSP container will resynchronize the values of any AT_BEGIN and
	 * AT_END variables (defined by the associated TagExtraInfo or TLD) after
	 * the invocation of doEndTag().
	 *
