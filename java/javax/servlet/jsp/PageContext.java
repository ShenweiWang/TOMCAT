 ...
 * a JSP page, provides access to several page attributes, as well as a layer
 * above the implementation details. Implicit objects are added to the
 * pageContext automatically.
 *
 * <p>
 ...
 * <p>
 * Some methods are intended to be used by the code generated by the container,
 * not by code written by JSP page authors, or JSP tag library authors.
 * <p>
 * The methods supporting <B>lifecycle</B> are <code>initialize()</code> and
 * <code>release()</code>
 *
 * <p>
 ...
 * <code>getException()</code>, <code>getPage()</code> <code>getRequest()</code>, <code>getResponse()</code>, <code>getSession()</code>,
 * <code>getServletConfig()</code> and <code>getServletContext()</code>.
 *
 * <p>
 * The following methods provide support for <B>forwarding, inclusion and error
 * handling</B>: <code>forward()</code>, <code>include()</code>, and
 * <code>handlePageException()</code>.
 */
...
public abstract class PageContext extends JspContext {
	...
	/**
	 * Name used to store uncaught exception in ServletRequest attribute list
	 * and PageContext name table.
	 */
	 ...
	 * PageContext so that it may be used by a JSP Implementation class to
	 * service an incoming request and response within it's _jspService()
	 * method.
	 *
	 * <p>
	 * This method is typically called from JspFactory.getPageContext() in order
	 * to initialize state.
	 *
	 * <p>
	 ...
	 * @param errorPageURL
	 *            The value of the errorpage attribute from the page directive
	 *            or null
	 * @param needsSession
	...
	/**
	 * The current value of the page object (In a Servlet environment, this is
	 * an instance of javax.servlet.Servlet).
	 *
	 * @return the Page implementation class instance associated with this
	 *         PageContext
	 */
	 ...
	 * <code> ServletContext </code> for this JSP. If the path does not begin
	 * with a "/" then the URL specified is calculated relative to the URL of
	 * the request that was mapped to the calling JSP.
	 * </p>
	 * <p>
	 ...
	 *             if the page that was forwarded to throws a ServletException
	 * @throws IOException
	 ...
	 * </p>
	 * <p>
	 * If flush is true, The current JspWriter "out" for this JSP is flushed as
	 * a side-effect of this call, prior to processing the include. Otherwise,
	 * the JspWriter "out" is not flushed.
	 * </p>
	 * <p>
	 ...
	 * calculated relative to the URL of the request that was mapped to the
	 * calling JSP.
	 * </p>
	 * <p>
	 * It is only valid to call this method from a <code> Thread </code>
	 ...
	 *            included
	 * @param flush
	 *            True if the JspWriter is to be flushed before the include, or
	 *            false if not.
	 *
	 * @throws ServletException
	 *             if the page that was forwarded to throws a ServletException
	 ...
	 * forwarding the exception to the specified error page for this JSP. If
	 * forwarding is not possible (for example because the response has already
	 * been committed), an implementation dependent mechanism should be used to
	 * invoke the error page (e.g. "including" the error page instead).
	 *
	 * <p>
	 * If no error page is defined in the page, the exception should be rethrown
	 * so that the standard servlet error handling takes over.
	 *
	 * <p>
	 * A JSP implementation class shall typically clean up any local state prior
	 * to invoking this and will return immediately thereafter. It is illegal to
	 * generate any output to the client, or to modify any ServletResponse state
	 * after invoking this call.
	 *
	 * <p>
	 * This method is kept for backwards compatibility reasons. Newly generated
	 ...
	 * @throws IOException
	 *             if an I/O error occurred while invoking the error page
	 ...
	 * <p>
	 * This method is intended to process an unhandled "page" level exception by
	 * redirecting the exception to either the specified error page for this
	 ...
	 *
	 * @return an ErrorData instance containing information about the error, as
	 *         obtained from the request attributes, as per the Servlet
	 *         specification. If this is not an error page (that is, if the
	 *         isErrorPage attribute of the page directive is not set to
	 *         "true"), the information is meaningless.
	 *
	 * @since 2.0
	 */
	public ErrorData getErrorData() {
		...
		return new ErrorData((Throwable) getRequest().getAttribute(
				RequestDispatcher.ERROR_EXCEPTION), status,
