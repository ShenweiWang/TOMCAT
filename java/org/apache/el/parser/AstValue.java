...
public final class AstValue extends SimpleNode {
	...
	static {
		if (IS_SECURITY_ENABLED) {
			...
			COERCE_TO_ZERO = Boolean.valueOf(
	...
	private final Target getTarget(EvaluationContext ctx) throws ELException {
		...
		while (i < propCount) {
			if (i + 2 < propCount
				...
				base = resolver.invoke(ctx, base, this.children[i]
						.getValue(ctx), null,
	...
	public boolean isReadOnly(EvaluationContext ctx) throws ELException {
		Target t = getTarget(ctx);
		ctx.setPropertyResolved(false);
		boolean result = ctx.getELResolver()
				.isReadOnly(ctx, t.base, t.property);
		if (!ctx.isPropertyResolved()) {
			throw new PropertyNotFoundException(MessageFactory.get(
					"error.resolver.unhandled", t.base, t.property));
		}
	...
	public Object invoke(EvaluationContext ctx,
		...
		if (isParametersProvided()) {
			values = ((AstMethodParameters) this.jjtGetChild(this
					.jjtGetNumChildren() - 1)).getParameters(ctx);
	...
	private Object[] convertArgs(Object[] src, Method m) {
		...
		if (m.isVarArgs()) {
			...
			dest[paramCount - 1] = ELSupport.coerceToType(src[paramCount - 1],
					types[paramCount - 1]);
		}
