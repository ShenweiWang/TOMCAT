...
public class StaticMember extends MemberImpl {
	 ...
	 * @param domain
	 *            String, either in byte array string format, like {214,116,1,3}
	 *            or as a regular string value like 'mydomain'. The latter will
	 *            be converted using ISO-8859-1 encoding
	 */
	...
	/**
	 * @param id
	 *            String, must be in byte array string format, like
	 *            {214,116,1,3} and exactly 16 bytes long
	 */
	public void setUniqueId(String id) {
		byte[] uuid = Arrays.fromString(id);
		if (uuid == null || uuid.length != 16)
			throw new RuntimeException("UUID must be exactly 16 bytes, not:"
					+ id);
		setUniqueId(uuid);
	}
