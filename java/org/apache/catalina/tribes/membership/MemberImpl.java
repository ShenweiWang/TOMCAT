 ...
 * representation of a multicast member. Carries the host, and port of the this
 * or other cluster nodes.
 *
 * @author Filip Hanik
 */
public class MemberImpl implements Member, java.io.Externalizable {
	 ...
	 * is false
	 */
	public static final boolean DO_DNS_LOOKUPS = Boolean.parseBoolean(System
			.getProperty("org.apache.catalina.tribes.dns_lookups", "false"));
	 ...
	 * internal tribes use, such as SHUTDOWN_COMMAND
	 */
	 ...
	 *            - the tcp listen port
	 * @param aliveTime
	 *            - the number of milliseconds since this member was created
	 */
	 ...
	 *
	 * @param getalive
	 *            boolean - calculate memberAlive time
	 * @param reset
	...
	public byte[] getData(boolean getalive, boolean reset) {
		...
		if (dataPkg != null) {
			if (getalive) {
				...
				XByteBuffer
						.toBytes(alive, dataPkg, TRIBES_MBR_BEGIN.length + 4);
			}
			return dataPkg;
		}
	...
	public static MemberImpl getMember(byte[] data, int offset, int length,
		...
		if (XByteBuffer.firstIndexOf(data, offset, TRIBES_MBR_BEGIN) != pos) {
			throw new IllegalArgumentException(
					"Invalid package, should start with:"
							+ org.apache.catalina.tribes.util.Arrays
									.toString(TRIBES_MBR_BEGIN));
		}

		if (length < (TRIBES_MBR_BEGIN.length + 4)) {
			throw new ArrayIndexOutOfBoundsException(
					"Member package to small to validate.");
		}
		...
		if (length < (bodylength + 4 + TRIBES_MBR_BEGIN.length + TRIBES_MBR_END.length)) {
			throw new ArrayIndexOutOfBoundsException(
					"Not enough bytes in member package.");
		}

		int endpos = pos + bodylength;
		if (XByteBuffer.firstIndexOf(data, endpos, TRIBES_MBR_END) != endpos) {
			throw new IllegalArgumentException(
					"Invalid package, should end with:"
							+ org.apache.catalina.tribes.util.Arrays
									.toString(TRIBES_MBR_END));
		}
	...
	public String getHostname() {
		if (this.hostname != null)
			...
			try {
				if (DO_DNS_LOOKUPS)
					this.hostname = java.net.InetAddress.getByAddress(host)
							.getHostName();
				else
					this.hostname = org.apache.catalina.tribes.util.Arrays
							.toString(host, 0, host.length, true);
	...
	/**
	 * Contains information on how long this member has been online. The result
	 * is the number of milli seconds this member has been broadcasting its
	...
	public boolean equals(Object o) {
		if (o instanceof MemberImpl) {
			return Arrays.equals(this.getHost(), ((MemberImpl) o).getHost())
					...
					&& Arrays.equals(this.getUniqueId(),
							((MemberImpl) o).getUniqueId());
		} else
			return false;
	}
	...
	public void setPayload(byte[] payload) {
		...
		if (this.getData(true, true).length > McastServiceImpl.MAX_PACKET_SIZE) {
			...
			throw new IllegalArgumentException(
					"Payload is to large for tribes to handle.");
		}
