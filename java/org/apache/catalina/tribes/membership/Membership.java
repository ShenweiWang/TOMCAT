 ...
 * representation of a multicast membership. This class is responsible for
 * maintaining a list of active cluster nodes in the cluster. If a node fails to
...
public class Membership implements Cloneable {
	...
	public Object clone() {
		synchronized (membersLock) {
			...
			// map is correct type already
			final HashMap<MemberImpl, MbrEntry> tmpclone = (HashMap<MemberImpl, MbrEntry>) map
					.clone();
			clone.map = tmpclone;
	 ...
	 * 
	 * @param local
	 *            - has to be the name of the local member. Used to filter the
	 *            local member from the cluster membership
	 * @param includeLocal
	 *            - TBA
	 */
	...
	/**
	 * Reset the membership and start over fresh. Ie, delete all the members and
	 * wait for them to ping again and join this membership
	 */
	...
	/**
	 * Runs a refresh cycle and returns a list of members that has expired. This
	 * also removes the members from the membership, in such a way that
	 * getMembers() = getMembers() - expire()
	 * 
	 * @param maxtime
	 *            - the max time a member can remain unannounced before it is
	 *            considered dead.
	...
	/**
	 * Returning a list of all the members in the membership We not need a copy:
	 * add and remove generate new arrays.
	 */
