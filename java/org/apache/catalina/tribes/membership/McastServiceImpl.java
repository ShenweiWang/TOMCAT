...
public class McastServiceImpl {
	 ...
	 */
	protected ExecutorService executor = ExecutorFactory.newThreadPool(0, 2, 2,
			TimeUnit.SECONDS);
	...
	protected static class RecoveryThread extends Thread {
		...
		public boolean startService() {
			try {
				...
				log.warn("Recovery thread failed to start membership service.",
						x);
				return false;
			}
		}
						...
						if (!success) {
							if (log.isInfoEnabled())
								log.info("Recovery attempt " + (++attempt)
										...
										+ parent.recoverySleepTime + " seconds");
