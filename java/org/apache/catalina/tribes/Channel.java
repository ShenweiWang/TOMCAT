 ...
 * sort of communication with each other.<br>
 * The channel is the main API class for Tribes, this is essentially the only
 * class that an application needs to be aware of. Through the channel the
 * application can:<br>
 * 1. send messages<br>
 ...
 * 5. Channel interceptors, the ability to manipulate messages as they are sent
 * or arrive<br>
 * <br>
...
public interface Channel {
	...
	/**
	 * Start and stop sequences can be controlled by these constants This allows
	 * you to start separate components of the channel <br>
	 ...
	 * server socket in case of a TCP implementation
	 * 
	 * @see #start(int)
	 * @see #stop(int)
	 */
	 ...
	 * sockets, as sockets are opened on demand when a message is being sent
	 * 
	 * @see #start(int)
	 * @see #stop(int)
	 */
	 ...
	 * implementation this will open a datagram socket and join a group and
	 * listen for membership messages members joining
	 * 
	 * @see #start(int)
	 * @see #stop(int)
	 */
	 ...
	 * implementation this will open a datagram socket and join a group and
	 * broadcast the local member information
	 * 
	 * @see #start(int)
	 * @see #stop(int)
	 */
	 ...
	 * trigger certain behavior. Most flags are used to trigger channel
	 * interceptors as the message passes through the channel stack. <br>
	 ...
	 * marshaling or unmarshaling will be performed.<br>
	 *
	 ...
	 * message has been received by the recipient<br>
	 ...
	 * when the message has been received and processed by the recipient<br>
	 * If no ack is received, the message is not considered successful<br>
	 * 
	 * @see #send(Member[], Serializable , int)
	 * @see #send(Member[], Serializable, int, ErrorHandler)
	 ...
	 * the message has been received and processed by the recipient<br>
	 * If no ack is received, the message is not considered successful<br>
	 * 
	 * @see #send(Member[], Serializable , int)
	 * @see #send(Member[], Serializable, int, ErrorHandler)
	...
	/**
	 * Send options. When a message is sent with this flag on the system sends a
	 * UDP message on the Multicast address instead of UDP or TCP to individual
	 * addresses
	 * 
	 * @see #send(Member[], Serializable , int)
	 * @see #send(Member[], Serializable, int, ErrorHandler)
	 ...
	 * variable. <br>
	 * The default is
	 * <code>int SEND_OPTIONS_DEFAULT = SEND_OPTIONS_USE_ACK;</code><br>
	 * 
	 ...
	 * services to start The svc parameter can be the logical or value of any
	 * constants
	 * 
	 * @param svc
	 *            int value of <BR>
	 ...
	 *            transmit the correct information, it has to be started after
	 ...
	 * @throws ChannelException
	 *             if a startup error occurs or the service is already stopped
	 *             or an error occurs.
	 */
	 ...
	 * 
	 * @param destination
	 *            Member[] - the destinations, can not be null or zero length,
	 *            the reason for that is that a membership change can occur and
	 *            at that time the application is uncertain what group the
	 *            message actually got sent to.
	 * @param msg
	 *            Serializable - the message to send, has to be serializable, or
	 *            a <code>ByteMessage</code> to send a pure byte array
	 * @param options
	 *            int - sender options, see class documentation for each
	 *            interceptor that is configured in order to trigger
	 *            interceptors
	 ...
	 * @param options
	 *            int - sender options, see class documentation
	 * @param handler
	 *            ErrorHandler - handle errors through a callback, rather than
	 *            throw it
	 * @return a unique Id that identifies the message that is sent
	 ...
	 * alert interceptors and other components to clean up garbage, timed out
	 * messages etc.<br>
	 * If you application has a background thread, then you can save one thread,
	 * by configuring your channel to not use an internal heartbeat thread and
	 ...
	 * 
	 * @param enable
	 *            boolean - default value is implementation specific
	 * @see #heartbeat()
	 */
	 ...
	 * leaves or crashes <br>
	 * If the membership listener implements the Heartbeat interface the
	 * <code>heartbeat()</code> method will be invoked when the heartbeat runs
	 * on the channel
	 * 
	 * @param listener
	 *            MembershipListener
	...
	/**
	 * remove a membership listener, listeners are removed based on
	...
	/**
	 * remove a channel listener, listeners are removed based on Object.hashCode
	 * and Object.equals
	 * 
	 * @param listener
	 *            ChannelListener
	...
	/**
	 * Returns true if there are any members in the group, this call is the same
	 * as <code>getMembers().length>0</code>
	 * 
	 ...
	 * gets broadcasted through the membership broadcaster component
	 * 
	 * @param incAlive
	 ...
	 * data. Some implementations might serialize and send membership
	 * information along with a message, and instead of sending complete
	 * membership details, only send the primary identifier for the member but
	 * not the payload or other information. When such message is received the
	 * application can retrieve the cached member through this call.<br>
