...
public abstract class AbstractReplicatedMap<K, V> implements Map<K, V>,
	...
	public AbstractReplicatedMap(MapOwner owner, Channel channel, long timeout,
			...
			int channelSendOptions, ClassLoader[] cls, boolean terminate) {
	...
	protected void ping(long timeout) throws ChannelException {
		...
		MapMessage msg = new MapMessage(this.mapContextName,
				MapMessage.MSG_INIT, false, null, null, null,
		...
		synchronized (mapMembers) {
			Member[] members = mapMembers.keySet().toArray(
					new Member[mapMembers.size()]);
	...
	public Member[] getMapMembersExcl(Member[] exclude) {
		synchronized (mapMembers) {
			...
			HashMap<Member, Long> list = (HashMap<Member, Long>) mapMembers
					.clone();
	...
	public void replicate(boolean complete) {
		Iterator<Map.Entry<K, MapEntry<K, V>>> i = innerMap.entrySet()
				.iterator();
		while (i.hasNext()) {
			Map.Entry<?, ?> e = i.next();
	...
	public Serializable replyRequest(Serializable msg, final Member sender) {
		...
		if (mapmsg.getMsgType() == MapMessage.MSG_STATE
				|| mapmsg.getMsgType() == MapMessage.MSG_STATE_COPY) {
	...
	public void messageReceived(Serializable msg, Member sender) {
		...
		if (mapmsg.getMsgType() == MapMessage.MSG_PROXY) {
			MapEntry<K, V> entry = innerMap.get(mapmsg.getKey());
			if (entry == null) {
				entry = new MapEntry<K, V>((K) mapmsg.getKey(),
						(V) mapmsg.getValue());
				MapEntry<K, V> old = innerMap
	...
	public boolean accept(Serializable msg, Member sender) {
		...
		if (msg instanceof MapMessage) {
			...
			result = Arrays.equals(mapContextName,
					((MapMessage) msg).getMapId());
			if (log.isTraceEnabled())
		...
		while (i.hasNext()) {
			...
			if (entry.isProxy() && entry.getPrimary() == null
				...
				try {
					...
					entry.setBackupNodes(backup);
					if (mapOwner != null)
						mapOwner.objectMadePrimay(entry.getKey(),
								entry.getValue());
	...
	public int size() {
		...
		int counter = 0;
		Iterator<Map.Entry<K, MapEntry<K, V>>> it = innerMap.entrySet()
				.iterator();
	...
	public static class MapMessage implements Serializable {
		...
		public Serializable getKey() {
			try {
				...
				throw new RuntimeException(
						"Deserialization error of the MapMessage.key", x);
			}
		}
		...
		public Serializable getValue() {
			try {
				...
				throw new RuntimeException(
						"Deserialization error of the MapMessage.value", x);
			}
		}
