 ...
 * backup strategy. One node is always the primary and one node is always the
 * backup. This map is synchronized across a cluster, and only has one backup
 * member.<br/>
 * A perfect usage for this map would be a session map for a session manager in
 * a clustered environment.<br/>
 * The only way to modify this list is to use the
 * <code>put, putAll, remove</code> methods. entrySet, entrySetFull, keySet,
 * keySetFull, returns all non modifiable sets.<br>
 * <br>
 * If objects (values) in the map change without invoking <code>put()</code> or
 * <code>remove()</code> the data can be distributed using two different
 * methods:<br>
 ...
 * decide what objects get replicated and how much data gets replicated each
 * time.<br>
 * If you implement a smart AOP mechanism to detect changes in underlying
 * objects, you can replicate only those changes by implementing the
 * ReplicatedMapEntry interface, and return true when isDiffable() is invoked.<br>
 * <br>
 * 
 * This map implementation doesn't have a background thread running to replicate
 * changes. If you do have changes without invoking put/remove then you need to
 * invoke one of the following methods:
 * <ul>
 * <li><code>replicate(Object,boolean)</code> - replicates only the object that
 * belongs to the key</li>
 ...
 * decide whether to only replicate objects that implement the
 * <code>ReplicatedMapEntry</code> interface or to replicate all objects. If an
 * object doesn't implement the <code>ReplicatedMapEntry</code> interface each
 * time the object gets replicated the entire object gets serialized, hence a
 * call to <code>replicate(true)</code> will replicate all objects in this map
 * that are using this node as primary.
 * 
 * <br>
 * <br>
 * <b>REMBER TO CALL <code>breakdown()</code> or <code>finalize()</code> when
 * you are done with the map to avoid memory leaks.<br>
 * <br>
...
public class LazyReplicatedMap<K, V> extends AbstractReplicatedMap<K, V> {
	 ...
	 *            channel
	 * @param terminate
	 *            boolean - Flag for whether to terminate this map that failed
	 *            to start.
	 */
	public LazyReplicatedMap(MapOwner owner, Channel channel, long timeout,
	...
	protected Member[] publishEntryInfo(Object key, Object value)
		...
		do {
			...
			try {
				...
				log.error("Unable to replicate backup key:" + key
						+ " to backup:" + next + ". Reason:" + x.getMessage(),
						x);
			}
			try {
				...
				// log the error, but proceed, this should only happen if a node
				// went down,
				// and if the node went down, then it can't receive the message,
				// the others
				// should still get it.
				log.error("Unable to replicate proxy key:" + key
						+ " to backup:" + next + ". Reason:" + x.getMessage(),
						x);
			}
