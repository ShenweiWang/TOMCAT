 ...
 * membership messages in the channel stack. This allows interceptors to modify
 * the message or perform other actions when a message is sent or received.<br>
...
public interface ChannelInterceptor extends MembershipListener, Heartbeat {
	 ...
	 * ChannelMessage.getOptions() and if the bit is set, this interceptor will
	 * react to it.<br>
	 ...
	 * trigger the interceptor. The interceptor itself will decide.<br>
	 * 
	 * @return int
	 ...
	 * 
	 * @return ChannelInterceptor - returns the next interceptor in the list or
	 *         null if no more interceptors exist
	 */
	 ...
	 * 
	 * @return ChannelInterceptor - returns the previous interceptor in the list
	 ...
	 * sent to one more destinations. The interceptor can modify any of the
	 * parameters and then pass on the message down the stack by invoking
	 ...
	 * If the message is to be sent asynchronous the application can be notified
	 * of completion and errors by passing in an error handler attached to a
	 * payload object.<br>
	 * The ChannelMessage.getAddress contains Channel.getLocalMember, and can be
	 * overwritten to simulate a message sent from another node.<br>
	 * 
	 * @param destination
	 *            Member[] - the destination for this message
	 * @param msg
	 *            ChannelMessage - the message to be sent
	 * @param payload
	 *            InterceptorPayload - the payload, carrying an error handler
	 *            and future useful data, can be null
	 ...
	 * address if it has been overwritten.
	 * 
	 * @param data
	 *            ChannelMessage
	 */
	 ...
	 * interceptors to clean up resources, time out object and perform actions
	 * that are unrelated to sending/receiving data.
	 */
	@Override
