 ...
 * being sent and received with membership announcements. The channel has an
 * chain of interceptors that can modify the message or perform other logic.<br>
...
public class GroupChannel extends ChannelInterceptorBase implements
	 ...
	 * true, the channel will start a local thread for the heart beat.
	 */
	 ...
	 * chained in a linked list, so we only need a reference to the first one
	 */
	 ...
	 * turned off internal heartbeats <code>channel.setHeartbeat(false)</code>
	 */
	@Override
	...
	public UniqueId send(Member[] destination, Serializable msg, int options,
		...
		try {
			...
			if (Logs.MESSAGES.isTraceEnabled()) {
				Logs.MESSAGES.trace("GroupChannel - Sent msg:"
						+ new UniqueId(data.getUniqueId()) + " at "
	 ...
	 * This method can also be used to send a message to other components within
	 * the same application, but its an extreme case, and you're probably better
	 * off doing that logic between the applications itself.
	 * 
	 * @param msg
	 *            ChannelMessage
	 */
	...
	public void messageReceived(ChannelMessage msg) {
		...
		try {
			...
			if ((msg.getOptions() & SEND_OPTIONS_BYTE_MESSAGE) == SEND_OPTIONS_BYTE_MESSAGE) {
				...
				try {
					fwd = XByteBuffer.deserialize(msg.getMessage()
							.getBytesDirect(), 0, msg.getMessage().getLength());
				} catch (Exception sx) {
			...
			for (int i = 0; i < channelListeners.size(); i++) {
				ChannelListener channelListener = (ChannelListener) channelListeners
						.get(i);
				if (channelListener != null
					...
					// channel
					// is responsible for returning the reply, otherwise we send
					// an absence reply
	 ...
	 * This method gets invoked by the channel if a RPC message comes in and no
	 * channel listener accepts the message. This avoids timeout
	 * 
	 * @param msg
	 *            RpcMessage
	 * @param destination
	...
	protected void sendNoRpcChannelReply(RpcMessage msg, Member destination) {
		try {
			...
			if (msg instanceof RpcMessage.NoRpcChannelReply)
				return;
			RpcMessage.NoRpcChannelReply reply = new RpcMessage.NoRpcChannelReply(
					msg.rpcId, msg.uuid);
			send(new Member[] { destination }, reply,
					Channel.SEND_OPTIONS_ASYNCHRONOUS);
		} catch (Exception x) {
			log.error(
					"Unable to find rpc channel, failed to send NoRpcChannelReply.",
					x);
		}
	}
	...
	public void memberAdded(Member member) {
		...
		for (int i = 0; i < membershipListeners.size(); i++) {
			MembershipListener membershipListener = (MembershipListener) membershipListeners
					.get(i);
		...
		if (getFirstInterceptor() != null
			...
			try {
				...
				throw new ChannelException(
						"Unable to add MessageDispatchInterceptor to interceptor chain.",
						x);
			}
	...
	protected void checkOptionFlags() throws ChannelException {
		...
		while (first != null) {
			...
			if (flag != 0) {
				...
				while (next != null) {
					...
					if (nflag != 0
							&& (((flag & nflag) == flag) || ((flag & nflag) == nflag))) {
						conflicts.append("[");
		...
		if (conflicts.length() > 0)
			throw new ChannelException("Interceptor option flag conflict: "
					+ conflicts.toString());
	 ...
	 * is invoked then the channel will start an internal thread to invoke
	 * <code>Channel.heartbeat()</code> every <code>getHeartbeatSleeptime</code>
	 * milliseconds
	 * 
	 * @param heartbeat
	 *            boolean
