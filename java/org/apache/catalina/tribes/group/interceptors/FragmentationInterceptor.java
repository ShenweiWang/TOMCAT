...
/**
 *
 * The fragmentation interceptor splits up large messages into smaller messages
 * and assembles them on the other end. This is very useful when you don't want
 * large messages hogging the sending sockets and smaller messages can make it
 * through.
 * 
 * <br>
...
public class FragmentationInterceptor extends ChannelInterceptorBase {
	private static final org.apache.juli.logging.Log log = org.apache.juli.logging.LogFactory
			.getLog(FragmentationInterceptor.class);
	...
	public void messageReceived(ChannelMessage msg) {
		boolean isFrag = XByteBuffer.toBoolean(msg.getMessage()
				.getBytesDirect(), msg.getMessage().getLength() - 1);
	...
	public void frag(Member[] destination, ChannelMessage msg,
		...
		for (int i = 0; i < count; i++) {
			...
			tmp.getMessage().append(msg.getMessage().getBytesDirect(), offset,
					length);
			// add the msg nr
	...
	public void heartbeat() {
		try {
			...
			if (log.isErrorEnabled()) {
				log.error(
						"Unable to perform heartbeat clean up in the frag interceptor",
						x);
			}
		}
		super.heartbeat();
	...
	public static class FragCollection {
		...
		public FragCollection(ChannelMessage msg) {
			...
			int count = XByteBuffer.toInt(msg.getMessage().getBytesDirect(),
					msg.getMessage().getLength() - 4);
		...
		public void addMessage(ChannelMessage msg) {
			...
			// get the msg nr
			int nr = XByteBuffer.toInt(msg.getMessage().getBytesDirect(), msg
					.getMessage().getLength() - 4);
			// remove the msg nr
			msg.getMessage().trim(4);
		...
		public ChannelMessage assemble() {
			...
			msg.setMessage(buf);
			for (int i = 0; i < frags.length; i++) {
				msg.getMessage().append(frags[i].getBytesDirect(), 0,
						frags[i].getLength());
			}
			return msg;
		}
