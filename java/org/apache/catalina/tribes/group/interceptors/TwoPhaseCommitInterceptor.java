...
public class TwoPhaseCommitInterceptor extends ChannelInterceptorBase {
	...
	private static final org.apache.juli.logging.Log log = org.apache.juli.logging.LogFactory
			.getLog(TwoPhaseCommitInterceptor.class);
	...
	public void sendMessage(Member[] destination, ChannelMessage msg,
		...
		if (okToProcess(msg.getOptions())) {
			...
			confirmation.getMessage().append(msg.getUniqueId(), 0,
					msg.getUniqueId().length);
	...
	public void messageReceived(ChannelMessage msg) {
		if (okToProcess(msg.getOptions())) {
			if (msg.getMessage().getLength() == (START_DATA.length
					+ msg.getUniqueId().length + END_DATA.length)
					&& Arrays.contains(msg.getMessage().getBytesDirect(), 0,
							START_DATA, 0, START_DATA.length)
					&& Arrays.contains(msg.getMessage().getBytesDirect(),
				...
				if (original != null) {
					...
					log.warn("Received a confirmation, but original message is missing. Id:"
	...
	public void heartbeat() {
		try {
			...
			log.warn(
					"Unable to perform heartbeat on the TwoPhaseCommit interceptor.",
					x);
		} finally {
			super.heartbeat();
		}
