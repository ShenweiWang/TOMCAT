 ...
 * ensure ordering. This setting is useful to avoid
 ...
 * what to do when a message has expired or the queue has grown larger than the
 * maxQueue value. true means that the message is sent up the stack to the
 * receiver that will receive and out of order message false means, forget the
 * message and reset the message counter. <b>default=true</b>
 * 
 * 
 * @author Filip Hanik
 * @version 1.1
...
public class OrderInterceptor extends ChannelInterceptorBase {
	...
	public void messageReceived(ChannelMessage msg) {
		...
		int msgnr = XByteBuffer.toInt(msg.getMessage().getBytesDirect(), msg
				.getMessage().getLength() - 4);
		msg.getMessage().trim(4);
		MessageOrder order = new MessageOrder(msgnr,
				(ChannelMessage) msg.deepclone());
		try {
