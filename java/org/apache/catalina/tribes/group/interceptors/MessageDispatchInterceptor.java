...
public class MessageDispatchInterceptor extends ChannelInterceptorBase
	...
	private static final Log log = LogFactory
			.getLog(MessageDispatchInterceptor.class);
			...
			if ((getCurrentSize() + msg.getMessage().getLength()) > maxQueueSize) {
				if (alwaysSend) {
					...
					throw new ChannelException(
							"Asynchronous queue is full, reached its limit of "
	...
	public void startQueue() {
		...
		msgDispatchThread
				.setName("MessageDispatchInterceptor.MessageDispatchThread");
		...
		try {
			...
			try {
				if (link.getHandler() != null)
					link.getHandler().handleCompletion(
			...
			try {
				if (link.getHandler() != null)
					link.getHandler().handleError(cx,
							new UniqueId(msg.getUniqueId()));
			} catch (Exception ex) {
