 ...
 *
 * <p>
 * Description: The TcpFailureDetector is a useful interceptor that adds
 * reliability to the membership layer.
 * </p>
 * <p>
 * If the network is busy, or the system is busy so that the membership receiver
 * thread is not getting enough time to update its table, members can be
 * &quot;timed out&quot; This failure detector will intercept the
 * memberDisappeared message(unless its a true shutdown message) and connect to
 * the member using TCP.
 * </p>
 * <p>
...
public class TcpFailureDetector extends ChannelInterceptorBase {
	...
	private static final org.apache.juli.logging.Log log = org.apache.juli.logging.LogFactory
			.getLog(TcpFailureDetector.class);

	protected static byte[] TCP_FAIL_DETECT = new byte[] { 79, -89, 115, 72,
			121, -126, 67, -55, -97, 111, -119, -128, -95, 91, 7, 20, 125, -39,
			82, 91, -21, -15, 67, -102, -73, 126, -66, -113, -127, 103, 30,
			-74, 55, 21, -66, -121, 69, 126, 76, -88, -65, 10, 77, 19, 83, 56,
			21, 50, 85, -10, -108, -73, 58, -6, 64, 120, -111, 4, 125, -41,
			114, -124, -64, -43 };

	@Deprecated
	protected boolean performConnectTest = true;// Unused - will be removed in
												// Tomcat 8.0.x
		...
		try {
			...
			for (int i = 0; i < mbrs.length; i++) {
				if (mbrs[i].getCause() != null
						&& (!(mbrs[i].getCause() instanceof RemoteProcessException))) {// RemoteProcessException's
																						// are
																						// ok
	...
	public void messageReceived(ChannelMessage msg) {
		...
		if (okToProcess(msg.getOptions())) {
			...
			process = ((msg.getMessage().getLength() != TCP_FAIL_DETECT.length) || (!Arrays
					.equals(TCP_FAIL_DETECT, msg.getMessage().getBytes())));
		}// end if
	...
	public void memberDisappeared(Member member) {
		...
		if (!shutdown)
			if (log.isInfoEnabled())
				log.info("Received memberDisappeared[" + member
						+ "] message. Will verify.");
		synchronized (membership) {
			if (!membership.contains(member)) {
				if (log.isInfoEnabled())
					log.info("Verification complete. Member already disappeared["
							+ member + "]");
				return;
			}
	...
	public void checkMembers(boolean checkAll) {
		...
		try {
			...
			log.warn("Unable to perform heartbeat on the TcpFailureDetector.",
					x);
		} finally {
	...
	protected void performBasicCheck() {
		...
		MemberImpl[] keys = removeSuspects.keySet().toArray(
				new MemberImpl[removeSuspects.size()]);
