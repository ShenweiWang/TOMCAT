 ...
 * thread at a time
 *
 * This is a state machine, handled by the process method States are: -
 * NOT_CONNECTED -> connect() -> CONNECTED - CONNECTED -> setMessage() -> READY
 * TO WRITE - READY_TO_WRITE -> write() -> READY TO WRITE | READY TO READ -
...
public class NioSender extends AbstractSender {
	...
	public boolean process(SelectionKey key, boolean waitForAck)
		...
		if ((!isConnected()) && (!connecting))
			throw new IOException(
					"Sender has been disconnected, can't selection key.");
		if (!key.isValid())
			throw new IOException(
					"Key is not valid, it must have been cancelled.");
	...
	protected boolean read(SelectionKey key) throws IOException {
		...
		if (current == null)
			return true;
		int read = isUdpBased() ? dataChannel.read(readbuf) : socketChannel
				.read(readbuf);
		// end of stream
		if (read == -1)
			throw new IOException(
					"Unable to receive an ack message. EOF on socket channel has been reached.");
		// no data read
		else if (read == 0)
			return false;
		readbuf.flip();
	...
	protected boolean write(SelectionKey key) throws IOException {
		if ((!isConnected())
				|| (this.socketChannel == null && this.dataChannel == null)) {
			throw new IOException(
					"NioSender is not connected, this should not occur.");
		}
		if (current != null) {
			if (remaining > 0) {
	 ...
	 * @throws IOException
	 *             TODO Implement this
	 *             org.apache.catalina.tribes.transport.IDataSender method
	 */
	@Override
	public synchronized void connect() throws IOException {
		...
		if (isUdpBased()) {
			...
			if (dataChannel != null)
				throw new IOException(
						"Datagram channel has already been established. Connection might be in progress.");
			...
			if (socketChannel != null)
				throw new IOException(
						"Socket channel has already been established. Connection might be in progress.");
	...
	public void disconnect() {
		try {
			...
			if (log.isDebugEnabled())
				log.debug(
						"Unable to disconnect NioSender. msg=" + x.getMessage(),
						x);
		} finally {
		}
