...
public class NioReplicationTask extends AbstractRxTask {
	...
	private static final org.apache.juli.logging.Log log = org.apache.juli.logging.LogFactory
			.getLog(NioReplicationTask.class);
		...
		try {
			...
			if (e instanceof CancelledKeyException) {
				...
				if (log.isDebugEnabled())
					...
					log.warn("IOException in replication worker, unable to drain channel. Probable cause: Keep alive socket closed["
							+ e.getMessage() + "].");
			} else if (log.isErrorEnabled()) {
				// this is a real error, log it.
				log.error(
						"Exception caught in TcpReplicationThread.drainChannel.",
						e);
			}
			cancelKey(key);
		} finally {
	...
	protected void drainChannel(final SelectionKey key, ObjectReader reader)
		...
		if (channel instanceof SocketChannel) {
			...
			buffer.flip(); // make buffer readable
			if (buffer.hasArray())
				reader.append(buffer.array(), 0,
						buffer.limit() - buffer.position(), false);
			else
		...
		for (int i = 0; i < msgs.length; i++) {
			 ...
			 * server before completing the request This is considered an
			...
			try {
				if (Logs.MESSAGES.isTraceEnabled()) {
					try {
						Logs.MESSAGES
								.trace("NioReplicationThread - Received msg:"
										+ new UniqueId(msgs[i].getUniqueId())
										+ " at "
				 ...
				 * server before sending the ack to the remote server This is
				 * considered a synchronized request
				 */
	...
	protected void sendAck(SelectionKey key, WritableByteChannel channel,
		...
		try {
			...
			log.warn("Unable to send ACK back through channel, channel disconnected?: "
					+ x.getMessage());
		}
	}
