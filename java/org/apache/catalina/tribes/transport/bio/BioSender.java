...
public class BioSender extends AbstractSender {
	...
	protected XByteBuffer ackbuf = new XByteBuffer(
	...
	public void disconnect() {
		...
		if (connect) {
			if (log.isDebugEnabled())
				log.debug(sm.getString("IDataSender.disconnect", getAddress()
	...
	public void sendMessage(byte[] data, boolean waitForAck) throws IOException {
		...
		try {
			...
			if (log.isTraceEnabled())
				log.trace(sm.getString("IDataSender.send.again", getAddress()
						.getHostAddress(), new Integer(getPort())), x);
	...
	protected void openSocket() throws IOException {
		...
		try {
			...
			if (log.isDebugEnabled())
				log.debug(sm.getString("IDataSender.openSocket", getAddress()
						.getHostAddress(), new Integer(getPort()), new Long(0)));
	...
	protected void closeSocket() {
		if (isConnected()) {
			...
			setConnected(false);
			if (log.isDebugEnabled())
				log.debug(sm.getString("IDataSender.closeSocket", getAddress()
						.getHostAddress(), new Integer(getPort()), new Long(0)));
		}
	}

	/**
	 * Push messages with only one socket at a time Wait for ack is needed and
	 * make auto retry when write message is failed. After sending error close
	 * and reopen socket again.
	 * 
	...
	protected void waitForAck() throws java.io.IOException {
		try {
			...
			String errmsg = sm.getString("IDataSender.ack.missing",
					getAddress(), new Integer(socket.getLocalPort()), new Long(
							getTimeout()));
