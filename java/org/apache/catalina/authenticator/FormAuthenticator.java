...
public class FormAuthenticator extends AuthenticatorBase {
	 ...
	 *            performed
	 * @throws IOException
	 *             If the forward to the error page fails and the call to
	 *             {@link HttpServletResponse#sendError(int, String)} throws an
	 *             {@link IOException}
	 */
	protected void forwardToErrorPage(Request request,
		...
		RequestDispatcher disp = context.getServletContext()
				.getRequestDispatcher(config.getErrorPage());
		try {
			if (context.fireRequestInitEvent(request)) {
				disp.forward(request.getRequest(), response);
				context.fireRequestDestroyEvent(request);
	...
	protected boolean restoreRequest(Request request, Session session)
		...
		SavedRequest saved = (SavedRequest) session
				.getNote(Constants.FORM_REQUEST_NOTE);
		...
		if (body != null) {
			request.getCoyoteRequest().action(ActionCode.REQ_SET_BODY_REPLAY,
					body);
		...
		request.getCoyoteRequest().queryString()
				.setString(saved.getQueryString());

		request.getCoyoteRequest().requestURI()
				.setString(saved.getRequestURI());
		return (true);
