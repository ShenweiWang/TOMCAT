 ...
 * web application is propagated to other web applications in the same security
...
public class SingleSignOn extends ValveBase implements SessionListener {
	...
	public void sessionEvent(SessionEvent event) {
		...
		if (((session.getMaxInactiveInterval() > 0) && (System
				...
				|| (Session.SESSION_PASSIVATED_EVENT.equals(event.getType()))
				|| (!session.getManager().getContainer().getState()
						.isAvailable())) {
	...
	protected boolean reauthenticate(String ssoId, Realm realm, Request request) {
		...
		if (entry != null && entry.getCanReauthenticate()) {
			...
			if (username != null) {
				Principal reauthPrincipal = realm.authenticate(username,
						entry.getPassword());
