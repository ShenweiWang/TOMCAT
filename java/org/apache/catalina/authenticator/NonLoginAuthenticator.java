...
public final class NonLoginAuthenticator extends AuthenticatorBase {
	 ...
	 *
	 * <p>
	 * This method is called by the AuthenticatorBase super class to establish a
	 * Principal for the user BEFORE the container security constraints are
	 * examined, i.e. it is not yet known whether the user will eventually be
	 * permitted to access the requested resource. Therefore, it is necessary to
	 * always return <code>true</code> to indicate the user has not failed
	 * authentication.
	 * </p>
	 *
	 * <p>
	 * There are two cases:
	 * <ul>
	 * <li>without SingleSignon: a Session instance does not yet exist and there
	 * is no <code>auth-method</code> to authenticate the user, so leave
	 * Request's Principal as null. Note: AuthenticatorBase will later examine
	 * the security constraints to determine whether the resource is accessible
	 * by a user without a security Principal and Role (i.e. unauthenticated).</li>
	 * <li>with SingleSignon: if the user has already authenticated via another
	 * container (using its own login configuration), then associate this
	 * Session with the SSOEntry so it inherits the already-established security
	 * Principal and associated Roles. Note: This particular session will become
	 * a full member of the SingleSignOnEntry Session collection and so will
	 * potentially keep the SSOE "alive", even if all the other properly
	 * authenticated Sessions expire first... until it expires too.</li>
	 * </ul>
	 * </p>
	 *
	 * @param request
		...
		if (principal != null) {
			...
			if (cache) {
				...
				String ssoId = (String) request
						.getNote(Constants.REQ_SSOID_NOTE);
				if (ssoId != null) {
					if (containerLog.isDebugEnabled())
						containerLog
								.debug("User authenticated by existing SSO");
