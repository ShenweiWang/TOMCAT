 ...
 * its parents! Please, use the MBeanFactory operation to generate valves and
 * realms.
 * </p>
 * <p>
...
public class JMXAccessorCreateTask extends JMXAccessorTask {
	...
	protected String jmxCreate(MBeanServerConnection jmxServerConnection,
		...
		if (classLoader != null && !"".equals(classLoader)) {
			...
			if (args == null)
				jmxServerConnection.createMBean(className,
						new ObjectName(name), new ObjectName(classLoader));
			else
				jmxServerConnection.createMBean(className,
						new ObjectName(name), new ObjectName(classLoader),
						argsA, sigA);
			...
			if (args == null)
				jmxServerConnection
						.createMBean(className, new ObjectName(name));
			else
				jmxServerConnection.createMBean(className,
						new ObjectName(name), argsA, sigA);
		}
		return error;
	}
