...
/**
 * <p>
 * Servlet filter to integrate "X-Forwarded-For" and "X-Forwarded-Proto" HTTP
 * headers.
 * </p>
 * <p>
 * Most of the design of this Servlet Filter is a port of <a
 * href="http://httpd.apache.org/docs/trunk/mod/mod_remoteip.html"
 * >mod_remoteip</a>, this servlet filter replaces the apparent client remote IP
 * address and hostname for the request with the IP address list presented by a
 * proxy or a load balancer via a request headers (e.g. "X-Forwarded-For").
 * </p>
 * <p>
 * Another feature of this servlet filter is to replace the apparent scheme
 ...
 * preceding load balancer or proxy in the given request's Http header named
 * <code>$remoteIpHeader</code> (default value <code>x-forwarded-for</code>).
 * Values are processed in right-to-left order.</li>
 * <li>For each ip/host of the list:
 * <ul>
 * <li>if it matches the internal proxies list, the ip/host is swallowed</li>
 * <li>if it matches the trusted proxies list, the ip/host is added to the
 * created proxies header</li>
 ...
 * <code>x-forwarded-for</code>) equals to the value of
 * <code>protocolHeaderHttpsValue</code> configuration parameter (default
 * <code>https</code>) then <code>request.isSecure = true</code>,
 * <code>request.scheme = https</code> and <code>request.serverPort = 443</code>
 * . Note that 443 can be overwritten with the <code>$httpsServerPort</code>
 ...
 * of traversed IP addresses starting from the requesting client</td>
 * <td>RemoteIPHeader</td>
 ...
 * <td>Regular expression that matches the IP addresses of internal proxies. If
 * they appear in the <code>remoteIpHeader</code> value, they will be trusted
 * and will not appear in the <code>proxiesHeader</code> value</td>
 * <td>RemoteIPInternalProxy</td>
 * <td>Regular expression (in the syntax supported by
 * {@link java.util.regex.Pattern java.util.regex})</td>
 * <td>
 * 10\.\d{1,3}\.\d{1,3}\.\d{1,3}|192\.168\.\d{1,3}\.\d{1,3}|169\.254\.\d{1,3}
 * \.\d{1,3}|127\.\d{1,3}\.\d{1,3}\.\d{1,3} <br/>
 * By default, 10/8, 192.168/16, 169.254/16 and 127/8 are allowed ; 172.16/12
 * has not been enabled by default because it is complex to describe with
 ...
 * they appear in the <code>remoteIpHeader</code> value, they will be trusted
 * and will appear in the <code>proxiesHeader</code> value</td>
 ...
 * <code>protocolHeader</code> indicates <code>http</code> protocol</td>
 * <td>N/A</td>
 * <td>integer</td>
 * <td>80</td>
 * </tr>
 ...
 * <td>Value returned by {@link ServletRequest#getServerPort()} when the
 * <code>protocolHeader</code> indicates <code>https</code> protocol</td>
 * <td>N/A</td>
 * <td>integer</td>
 * <td>443</td>
 * </tr>
 ...
 * <code>192.168/16</code>) to configure <code>RemoteIPInternalProxy</code> and
 * <code>RemoteIPTrustedProxy</code> ; as the JVM doesn't have a library similar
 * to <a href=
 * "http://apr.apache.org/docs/apr/1.3/group__apr__network__io.html#gb74d21b8898b7c40bf7fd07ad3eb993d"
 * >apr_ipsubnet_test</a>, we rely on regular expressions.
 * </p>
 * <hr/>
 * <p>
 ...
 * proxies as been traversed by the request. <code>x-forwarded-by</code> is null
 ...
 * that come in <code>x-forwarded-for</code> header, they both are migrated in
 * <code>x-forwarded-by</code> header. <code>x-forwarded-by</code> is null
 * because all the proxies are trusted or internal.
 * </p>
 * <hr/>
 * <p>
 ...
 * internal proxy, it does not appear in <code>x-forwarded-by</code>.
 ...
 * trusted and thus, we can not trust that <code>untrusted-proxy</code> is the
 * actual remote ip. <code>request.remoteAddr</code> is
...
public class RemoteIpFilter implements Filter {
	public static class XForwardedRequest extends HttpServletRequestWrapper {
		...
		public Enumeration<String> getHeaders(String name) {
			...
			if (header == null || header.getValue() == null) {
				return Collections
						.enumeration(Collections.<String> emptyList());
			}
	...
	public void doFilter(HttpServletRequest request,
		...
		if (internalProxies != null
			...
			for (Enumeration<String> e = request.getHeaders(remoteIpHeader); e
					.hasMoreElements();) {
			...
			// continue to loop on remoteIpHeaderValue to build the new value of
			// the remoteIpHeader
			...
			if (protocolHeader != null) {
				...
				if (protocolHeaderValue == null) {
					...
					// of the request
						...
						.equalsIgnoreCase(protocolHeaderValue)) {
			...
			if (log.isDebugEnabled()) {
				log.debug("Incoming request "
						...
						+ protocolHeader
						+ "]='"
						+ (protocolHeader == null ? null : request
								.getHeader(protocolHeader))
						+ "' will be seen as newRemoteAddr='"
						...
						+ "]='" + xRequest.getHeader(remoteIpHeader)
						+ "', new[" + proxiesHeader + "]='"
	 ...
	 * the http header <code>x-forwareded-for</code> is not empty.
	 */
	@Override
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		if (filterConfig.getInitParameter(INTERNAL_PROXIES_PARAMETER) != null) {
			setInternalProxies(filterConfig
					.getInitParameter(INTERNAL_PROXIES_PARAMETER));
		}

		if (filterConfig.getInitParameter(PROTOCOL_HEADER_PARAMETER) != null) {
			setProtocolHeader(filterConfig
					.getInitParameter(PROTOCOL_HEADER_PARAMETER));
		}

		if (filterConfig
				.getInitParameter(PROTOCOL_HEADER_HTTPS_VALUE_PARAMETER) != null) {
			setProtocolHeaderHttpsValue(filterConfig
					.getInitParameter(PROTOCOL_HEADER_HTTPS_VALUE_PARAMETER));
		}
		...
		if (filterConfig.getInitParameter(CHANGE_LOCAL_PORT_PARAMETER) != null) {
			setChangeLocalPort(Boolean.parseBoolean(filterConfig
					.getInitParameter(CHANGE_LOCAL_PORT_PARAMETER)));
		}

		if (filterConfig.getInitParameter(PROXIES_HEADER_PARAMETER) != null) {
			setProxiesHeader(filterConfig
					.getInitParameter(PROXIES_HEADER_PARAMETER));
		}

		if (filterConfig.getInitParameter(REMOTE_IP_HEADER_PARAMETER) != null) {
			setRemoteIpHeader(filterConfig
					.getInitParameter(REMOTE_IP_HEADER_PARAMETER));
		}

		if (filterConfig.getInitParameter(TRUSTED_PROXIES_PARAMETER) != null) {
			setTrustedProxies(filterConfig
					.getInitParameter(TRUSTED_PROXIES_PARAMETER));
		}

		if (filterConfig.getInitParameter(HTTP_SERVER_PORT_PARAMETER) != null) {
			try {
				setHttpServerPort(Integer.parseInt(filterConfig
						.getInitParameter(HTTP_SERVER_PORT_PARAMETER)));
		...
		if (filterConfig.getInitParameter(HTTPS_SERVER_PORT_PARAMETER) != null) {
			try {
				setHttpsServerPort(Integer.parseInt(filterConfig
						.getInitParameter(HTTPS_SERVER_PORT_PARAMETER)));
			} catch (NumberFormatException e) {
				throw new NumberFormatException("Illegal "
	...
	/**
	 * <p>
	 * If <code>true</code>, the return values for both
	 * {@link ServletRequest#getLocalPort()} and
	 * {@link ServletRequest#getServerPort()} wil be modified by this Filter
	 * rather than just {@link ServletRequest#getServerPort()}.
	 * </p>
	 * <p>
	 ...
	 * {@link #protocolHeader} is not null and has a value different of
	 ...
	 * <p>
	 * Default value :
	 * 10\.\d{1,3}\.\d{1,3}\.\d{1,3}|192\.168\.\d{1,3}\.\d{1,3}|169
	 * \.254.\d{1,3}.\d{1,3}|127\.\d{1,3}\.\d{1,3}\.\d{1,3}
	 * </p>
	 */
	 ...
	 * will collect a list of all of the intermediate client IP addresses
	 * trusted to resolve the actual remote IP. Note that intermediate
	 ...
	 * </p>
	 * <p>
	 * Name of the http header that holds the list of trusted proxies that has
	 * been traversed by the http request.
	 * </p>
	 * <p>
	 ...
	 *            <code>true</code> causes the attributes to be set,
	 *            <code>false</code> disables the setting of the attributes.
	 */
