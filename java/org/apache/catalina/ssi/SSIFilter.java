...
public class SSIFilter implements Filter {
	...
	protected Pattern shtmlRegEx = Pattern
			.compile("text/x-server-parsed-html(;.*)?");
	...
	public void init(FilterConfig config) throws ServletException {
		...
		if (config.getInitParameter("contentType") != null) {
			contentTypeRegEx = Pattern.compile(config
					.getInitParameter("contentType"));
		} else {
			contentTypeRegEx = shtmlRegEx;
		}

		isVirtualWebappRelative = Boolean.parseBoolean(config
