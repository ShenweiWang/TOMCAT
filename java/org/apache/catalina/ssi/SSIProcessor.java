...
public class SSIProcessor {
	...
	protected HashMap<String, SSICommand> commands = new HashMap<String, SSICommand>();
	protected int debug;
	...
	/**
	 * Process a file with server-side commands, reading from reader and writing
	 * the processed version to writer. NOTE: We really should be doing this in
	 * a streaming way rather than converting it to an array first.
	 * 
	 * @param reader
			...
			while (index < fileContents.length()) {
				...
				if (!inside) {
					...
					if (c == COMMAND_END.charAt(0)
						...
						SSICommand ssiCommand = commands.get(strCmd
