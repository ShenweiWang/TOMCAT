 ...
 * appropriate request property (selected based on which subclass you choose to
 * configure into your Container's pipeline) against the regular expressions
 ...
 * <ul>
 * <li>The subclass extracts the request property to be filtered, and calls the
 * common <code>process()</code> method.
 * <li>If there is a deny expression configured, the property will be compared
 * to the expression. If a match is found, this request will be rejected with a
 * "Forbidden" HTTP response.</li>
 * <li>If there is a allow expression configured, the property will be compared
 * to each such expression. If a match is found, this request will be allowed to
 * pass through to the next Valve in the current pipeline.</li>
 * <li>If a deny expression was specified but no allow expression, allow this
 * request to pass through (because none of the deny expressions matched it).
...
public abstract class RequestFilterValve extends ValveBase {
	...
	/**
	 * Helper variable to catch configuration errors. It is <code>true</code> by
	 * default, but becomes <code>false</code> if there was an attempt to assign
	 * an invalid value to the <code>allow</code> pattern.
	 */
	 ...
	 * not apply successfully. E.g. if the pattern is syntactically invalid.
	 */
	 ...
	 *
	 * @param property
	 *            The request property on which to filter
	 * @param request
	 *            The servlet request to be processed
	 * @param response
