...
/**
 * <p>
 * This Tomcat extension logs server access directly to a database, and can be
 * used instead of the regular file-based access log implemented in
 * AccessLogValve. To use, copy into the server/classes directory of the Tomcat
 * installation and configure in server.xml as:
 * 
 * <pre>
 ...
 * <code>driverName</code> and <code>connectionURL</code>), the table name (
 * <code>tableName</code>) and the field names (corresponding to the get/set
 * method names). The same options as AccessLogValve are supported, such as
 * <code>resolveHosts</code> and <code>pattern</code> ("common" or "combined"
 * only).
 * </p>
 * <p>
 * When Tomcat is started, a database connection (with autoReconnect option) is
 * created and used for all the log activity. When Tomcat is shutdown, the
 * database connection is closed. This logger can be used at the level of the
 * Engine context (being shared by all the defined hosts) or the Host context
 ...
 * </pre>
 * <p>
 * Set JDBCAccessLogValve attribute useLongContentLength="true" as you have more
 * then 4GB outputs. Please, use long SQL datatype at access.bytes attribute.
 * The datatype of bytes at oracle is <i>number</i> and other databases use
 * <i>bytes BIGINT NOT NULL</i>.
 * </p>
 *
 * <p>
 * If the table is created as above, its name and the field names don't need to
 * be defined.
 * </p>
 * <p>
...
public final class JDBCAccessLogValve extends ValveBase implements AccessLog {
	...
	/**
	 * Class constructor. Initializes the fields with the default values. The
	 * defaults are:
	 * 
	 * <pre>
	 * driverName = null;
	...
	/**
	 * Sets the name of the field containing the virtual host information (this
	 * is in fact the server name).
	 *
	 * @param virtualHostField
	 ...
	 *
	 * @param queryField
	 *            The name of the field containing the URL part of the HTTP
	 *            query.
	 */
	 ...
	 * file-based "common" and "combined". These are translated into the use of
	 * tables containing either set of fields.
	 * <P>
			...
			while (numberOfTries > 0) {
				try {
					...
					container.getLogger().error(
							sm.getString("jdbcAccessLogValve.exception"), e);
	...
	protected void open() throws SQLException {
		...
		if (logPattern.equals("common")) {
			ps = conn.prepareStatement("INSERT INTO " + tableName + " ("
					...
					+ timestampField + ", " + queryField + ", " + statusField
					+ ", " + bytesField + ") VALUES(?, ?, ?, ?, ?, ?)");
		} else if (logPattern.equals("combined")) {
			ps = conn.prepareStatement("INSERT INTO " + tableName + " ("
					+ remoteHostField + ", " + userField + ", "
					+ timestampField + ", " + queryField + ", " + statusField
					+ ", " + bytesField + ", " + virtualHostField + ", "
					+ methodField + ", " + refererField + ", " + userAgentField
					+ ") VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		}
	}
	...
	protected void close() {
		...
		try {
			...
			container.getLogger().error(
					sm.getString("jdbcAccessLogValeve.close"), e); // Just log
																	// it here
		} finally {
			this.conn = null;
		}
