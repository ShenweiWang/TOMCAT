...
public class AccessLogValve extends ValveBase implements AccessLog {
	 ...
	 * length of the range is configurable. It is implemented based on a cyclic
	 * buffer. New entries shift the range.
	 * </p>
	 *
	 * <p>
	 * There is one cache for the CLF format (the access log standard format)
	 * and a HashMap of caches for additional formats used by SimpleDateFormat.
	 * </p>
	 *
	 * <p>
	 * Although the cache supports specifying a locale when retrieving a
	 * formatted timestamp, each format will always use the locale given when
	 * the format was first used. New locales can only be used for new formats.
	...
	protected static class DateFormatCache {
		...
		protected class Cache {
			...
			private String getFormatInternal(long time) {
				...
				/*
				 * First step: if we have seen this timestamp during the
				 * previous call, and we need CLF, return the previous value.
				 */
				...
				} else if (seconds >= last + cacheSize
						|| seconds <= first - cacheSize) {
					first = seconds;
				...
				/*
				 * Last step: format new timestamp either using parent cache or
				 * locally.
				 */
				if (parent != null) {
					synchronized (parent) {
		...
		public void addElement(StringBuilder buf, Date date, Request request,
			...
			if (length <= 0) {
				...
				Object start = request
						.getAttribute(Globals.SENDFILE_FILE_START_ATTR);
				if (start instanceof Long) {
					Object end = request
							.getAttribute(Globals.SENDFILE_FILE_END_ATTR);
	 ...
	 */
	protected AccessLogElement createAccessLogElement(String header,
			char pattern) {
		switch (pattern) {
		case 'i':
