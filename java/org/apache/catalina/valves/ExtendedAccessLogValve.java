 ...
 * <li><code>x-P(...)</code>: Call request.getParameter(...) and URLencode it.
 * Helpful to capture certain POST parameters.</li>
 ...
 * <code>condition</code> property. If the value returned from
 * ServletRequest.getAttribute(condition) yields a non-null value, the logging
 * will be skipped.
 * </p>
 *
 * <p>
...
public class ExtendedAccessLogValve extends AccessLogValve {
	...
	private static final Log log = LogFactory
			.getLog(ExtendedAccessLogValve.class);
	...
	/**
	 * Wrap the incoming value into quotes and escape any inner quotes with
	 * double quotes.
	 *
	 * @param value
	 *            - The value to wrap quotes around
	 * @return '-' if empty of null. Otherwise, toString() will be called on the
	 *         object and the value will be wrapped in quotes and any quotes
	 *         will be escaped with 2 sets of quotes.
	 */
	...
	protected static class DateElement implements AccessLogElement {
		...
		public void addElement(StringBuilder buf, Date date, Request request,
			...
			if (date.getTime() > (millis + INTERVAL - 1)
					|| date.getTime() < millis) {
				eds.currentTimestamp.setTime(date.getTime()
						- (date.getTime() % INTERVAL));
				eds.currentTimestampString = eds.currentTimestampFormat
						.format(eds.currentTimestamp);
			}
