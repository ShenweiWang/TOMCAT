 ...
 * >mod_remoteip</a>, this valve replaces the apparent client remote IP address
 * and hostname for the request with the IP address list presented by a proxy or
 * a load balancer via a request headers (e.g. "X-Forwarded-For").
 * </p>
 * <p>
 * Another feature of this valve is to replace the apparent scheme (http/https)
 * and server port with the scheme presented by a proxy or a load balancer via a
 * request header (e.g. "X-Forwarded-Proto").
 * </p>
 * <p>
 ...
 * <tr>
 * <td>proxiesHeader</td>
 * <td>Name of the http header created by this valve to hold the list of proxies
 * that have been processed in the incoming <code>remoteIpHeader</code></td>
 * <td>proxiesHeader</td>
 * <td>Compliant http header name</td>
 ...
 * <td>Name of the http header read by this valve that holds the flag that this
 * request</td>
 * <td>N/A</td>
 ...
 * <td>Value returned by {@link javax.servlet.ServletRequest#getServerPort()}
 * when the <code>protocolHeader</code> indicates <code>http</code> protocol</td>
 * <td>N/A</td>
 * <td>integer</td>
 * <td>80</td>
 * </tr>
 ...
 * <td>Value returned by {@link javax.servlet.ServletRequest#getServerPort()}
 * when the <code>protocolHeader</code> indicates <code>https</code> protocol</td>
 * <td>N/A</td>
 * <td>integer</td>
 * <td>443</td>
 * </tr>
 ...
 * the filtering you wish to perform.
 * </p>
 * <p>
 ...
 * <code>RemoteIPTrustedProxy</code> ; as Tomcat doesn't have a library similar
 * to <a href=
 * "http://apr.apache.org/docs/apr/1.3/group__apr__network__io.html#gb74d21b8898b7c40bf7fd07ad3eb993d"
 * >apr_ipsubnet_test</a>, <code>RemoteIpValve</code> uses regular expression to
 * configure <code>internalProxies</code> and <code>trustedProxies</code> in the
 * same fashion as {@link RequestFilterValve} does.
 * </p>
 * <hr/>
 * <p>
...
public class RemoteIpValve extends ValveBase {
	...
	/**
	 * Obtain the name of the HTTP header used to override the value returned by
	 * {@link Request#getServerPort()} and (optionally depending on {link
	...
	/**
	 * Set the name of the HTTP header used to override the value returned by
	 * {@link Request#getServerPort()} and (optionally depending on {link
	 * {@link #isChangeLocalPort()} {@link Request#getLocalPort()}.
	 *
	 * @param portHeader
	 ...
	 * @return the value of the protocol header for incoming https request (e.g.
	 *         "https")
	 */
	...
	public void invoke(Request request, Response response) throws IOException,
		...
		if (internalProxies != null
			...
			if (remoteIp != null) {
				...
				// use
				// request.coyoteRequest.mimeHeaders.setValue(str).setString(str)
				// because request.addHeader(str, str) is no-op in Tomcat
				// 6.0
				if (proxiesHeaderValue.size() == 0) {
					request.getCoyoteRequest().getMimeHeaders()
							.removeHeader(proxiesHeader);
				} else {
					String commaDelimitedListOfProxies = listToCommaDelimitedString(proxiesHeaderValue);
					request.getCoyoteRequest().getMimeHeaders()
							.setValue(proxiesHeader)
							.setString(commaDelimitedListOfProxies);
				}
				if (newRemoteIpHeaderValue.size() == 0) {
					request.getCoyoteRequest().getMimeHeaders()
							.removeHeader(remoteIpHeader);
				} else {
					String commaDelimitedRemoteIpHeaderValue = listToCommaDelimitedString(newRemoteIpHeaderValue);
					request.getCoyoteRequest().getMimeHeaders()
							.setValue(remoteIpHeader)
			...
			if (protocolHeader != null) {
				...
				if (protocolHeaderValue == null) {
					...
					// request.setScheme() because request.setScheme() is no-op
					// in Tomcat 6.0
			...
			if (log.isDebugEnabled()) {
				log.debug("Incoming request " + request.getRequestURI()
						...
						+ "', originalSecure='" + originalSecure
						+ "', originalScheme='" + originalScheme
						+ "' will be seen as newRemoteAddr='"
						+ request.getRemoteAddr() + "', newRemoteHost='"
						+ request.getRemoteHost() + "', newScheme='"
						+ request.getScheme() + "', newSecure='"
						+ request.isSecure() + "'");
			}
		} else {
			if (log.isDebugEnabled()) {
	...
	/**
	 * <p>
	 * Server Port value if the {@link #protocolHeader} is not <code>null</code>
