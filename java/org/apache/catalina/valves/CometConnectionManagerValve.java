...
/**
 * <p>
 * Implementation of a Valve that tracks Comet connections, and closes them when
 * the associated session expires or the webapp is reloaded.
 * </p>
 *
 * <p>
...
public class CometConnectionManagerValve extends ValveBase implements
	 ...
	 */
	protected final List<Request> cometRequests = Collections
		...
		if (Lifecycle.BEFORE_STOP_EVENT.equals(event.getType())) {
			...
			while (iterator.hasNext()) {
				...
				try {
					...
					cometEvent
							.setEventSubType(CometEvent.EventSubType.WEBAPP_RELOAD);
					...
					try {
						...
						container
								.getLogger()
								.warn(sm.getString("cometConnectionManagerValve.event"),
										e);
					}
				}
			}
	...
	public void invoke(Request request, Response response) throws IOException,
		...
		if (request.isComet() && !response.isClosed()) {
			...
			synchronized (session) {
				Request[] requests = (Request[]) session
	...
	public void event(Request request, Response response, CometEvent event)
		...
		try {
			...
			if (!ok
					...
					|| (event.getEventType() == CometEvent.EventType.ERROR && !(event
	...
	public void sessionDestroyed(HttpSessionEvent se) {
		...
		Request[] reqs = (Request[]) se.getSession().getAttribute(
				cometRequestsAttribute);
		if (reqs != null) {
			for (int i = 0; i < reqs.length; i++) {
				Request req = reqs[i];
				try {
					...
					((CometProcessor) req.getWrapper().getServlet())
							.event(event);
					event.close();
				} catch (Exception e) {
					req.getWrapper()
							.getParent()
							.getLogger()
							.warn(sm.getString("cometConnectionManagerValve.listenerEvent"),
									e);
				}
			}
		}
