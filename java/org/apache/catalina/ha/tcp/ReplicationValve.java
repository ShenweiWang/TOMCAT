 ...
 * Request (before processing) and the corresponding Response (after
 * processing). It is especially useful in debugging problems related to headers
 * and cookies.
 * </p>
 *
 * <p>
 * This Valve may be attached to any Container, depending on the granularity of
 * the logging you wish to perform.
 * </p>
 *
 * <p>
 * primaryIndicator=true, then the request attribute
 * <i>org.apache.catalina.ha.tcp.isPrimarySession.</i> is set true, when request
 * processing is at sessions primary node.
 * </p>
 *
...
public class ReplicationValve extends ValveBase implements ClusterValve {
	 ...
	 */
	protected ThreadLocal<ArrayList<DeltaSession>> crossContextSessions = new ThreadLocal<ArrayList<DeltaSession>>();
	...
	public void setFilter(String filter) {
		...
		if (filter == null || filter.length() == 0) {
			...
			try {
				...
				log.error(
						sm.getString("ReplicationValve.filter.failure", filter),
						pse);
			}
		}
	}
			...
			if (!sessions.contains(session)) {
				if (log.isDebugEnabled())
					log.debug(sm.getString(
							...
							session.getIdInternal(), session.getManager()
	...
	public void invoke(Request request, Response response) throws IOException,
		...
		try {
			...
			if (context != null && cluster != null
				...
				ClusterManager clusterManager = (ClusterManager) context
						.getManager();
			...
			if (isCrossContext) {
				if (log.isDebugEnabled())
					log.debug(sm
							.getString("ReplicationValve.crossContext.remove"));
	...
	protected void sendCrossContextSession(CatalinaCluster containerCluster) {
		...
		if (sessions != null && sessions.size() > 0) {
			for (Iterator<DeltaSession> iter = sessions.iterator(); iter
					.hasNext();) {
				Session session = iter.next();
				if (log.isDebugEnabled())
					log.debug(sm.getString(
							"ReplicationValve.crossContext.sendDelta", session
	...
	protected void createPrimaryIndicator(Request request) throws IOException {
		...
		if ((id != null) && (id.length() > 0)) {
			...
			if (session instanceof ClusterSession) {
				...
				if (log.isDebugEnabled())
					log.debug(sm.getString(
							"ReplicationValve.session.indicator", request
									.getContext().getName(), id,
							primaryIndicatorName, Boolean.valueOf(cses
									.isPrimarySession())));
				...
				if (log.isDebugEnabled()) {
					if (session != null) {
						log.debug(sm.getString(
								"ReplicationValve.session.found", request
										.getContext().getName(), id));
					} else {
						log.debug(sm.getString(
								"ReplicationValve.session.invalid", request
										.getContext().getName(), id));
					}
				}
			}
