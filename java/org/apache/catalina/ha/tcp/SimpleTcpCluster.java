 ...
 */
public class SimpleTcpCluster extends LifecycleMBeanBase implements
		CatalinaCluster, LifecycleListener, IDynamicProperty,
	...
	private Map<Member, ObjectName> memberOnameMap = new ConcurrentHashMap<Member, ObjectName>();
	...
	public ClusterListener[] findClusterListeners() {
		if (clusterListeners.size() > 0) {
			ClusterListener[] listener = new ClusterListener[clusterListeners
					.size()];
	...
	public boolean setProperty(String name, Object value) {
		...
		log.warn("Dynamic setProperty("
				+ name
				+ ",value) has been disabled, please use explicit properties for the element you are trying to identify");
		return false;
	}
	...
	public synchronized Manager createManager(String name) {
		...
		try {
			...
			log.error(
					"Unable to clone cluster manager, defaulting to org.apache.catalina.ha.session.DeltaManager",
					x);
	...
	/**
	 * Stop Cluster and implement the requirements of
	...
	public void send(ClusterMessage msg, Member dest) {
		try {
			...
			if (msg instanceof SessionMessage
				...
				sendOptions = Channel.SEND_OPTIONS_SYNCHRONIZED_ACK
	...
	public void messageReceived(ClusterMessage message) {
		...
		if (message != null) {
			for (Iterator<ClusterListener> iter = clusterListeners.iterator(); iter
					.hasNext();) {
