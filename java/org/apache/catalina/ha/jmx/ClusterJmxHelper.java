...
public class ClusterJmxHelper {
	...
	protected static boolean initMetaData(Class<?> clazz) {
		try {
			if (clazz == null)
				return false;
			getRegistry().loadMetadata(
					clazz.getResourceAsStream("mbeans-descriptors.xml"));
		} catch (Exception x) {
	...
	public static boolean registerDefaultCluster(SimpleTcpCluster cluster) {
		try {
			...
			log.warn(
					"Unable to register default cluster implementation with JMX",
					x);
			return false;
		}
	}
	...
	public static boolean unregisterDefaultCluster(SimpleTcpCluster cluster) {
		try {
			...
			log.warn(
					"Unable to unregister default cluster implementation with JMX",
					x);
			return false;
		}
	}
