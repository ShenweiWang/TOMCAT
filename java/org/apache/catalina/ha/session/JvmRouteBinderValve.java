...
public class JvmRouteBinderValve extends ValveBase implements ClusterValve {
	...
	public void invoke(Request request, Response response) throws IOException,
		...
		if (getEnabled() && request.getContext() != null
			...
			if (manager != null
					&& ((manager instanceof ClusterManager
							&& getCluster() != null && getCluster().getManager(
							((ClusterManager) manager).getName()) != null) || (manager instanceof PersistentManager)))
	...
	protected Manager getManager(Request request) {
		...
		if (log.isDebugEnabled()) {
			if (manager != null)
				...
				log.debug(sm.getString("jvmRoute.notFoundManager", request
						.getContext().getName()));
		}
		return manager;
	}
	 ...
	 *            local jvmRoute
	 */
	protected void handleJvmRoute(Request request, String sessionId,
		...
		if (requestJvmRoute != null && !requestJvmRoute.equals(localJvmRoute)) {
			...
			if (catalinaSession != null) {
				...
				try {
					catalinaSession = getManager(request).findSession(
							newSessionID);
				} catch (IOException e) {
					// Hups!
				}
	 ...
	 *            new session id for node migration
	 */
	protected void changeRequestSessionID(Request request, String sessionId,
			String newSessionID) {
		...
		if (sessionIdAttribute != null && !"".equals(sessionIdAttribute)) {
			if (log.isDebugEnabled()) {
				log.debug(sm.getString("jvmRoute.set.orignalsessionid",
	 ...
	 * 
	 * @see JvmRouteSessionIDBinderListener#messageReceived(org.apache.catalina.ha.ClusterMessage)
	 * @param sessionId
