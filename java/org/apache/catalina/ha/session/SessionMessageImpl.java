...
public class SessionMessageImpl extends ClusterMessageBase implements
	...
	private SessionMessageImpl(String contextName, int eventtype,
	 ...
	 * send that all session state information are transfered after
	 * GET_ALL_SESSION received from this sender.<BR>
	 * <B>EVT_CHANGE_SESSION_ID</B><BR>
	 * send original sessionID and new sessionID.<BR>
	 * <B>EVT_ALL_SESSION_NOCONTEXTMANAGER</B><BR>
	 * send that context manager does not exist after GET_ALL_SESSION received
	 * from this sender.<BR>
	 * 
	 * @param contextName
	 *            - the name of the context (application
	 * @param eventtype
	 *            - one of the 8 event type defined in this class
	 * @param session
	 *            - the serialized byte array of the session itself
	 * @param sessionID
	 *            - the id that identifies this session
	 * @param uniqueID
	 *            - the id that identifies this message
	 */
	public SessionMessageImpl(String contextName, int eventtype,
			byte[] session, String sessionID, String uniqueID) {
