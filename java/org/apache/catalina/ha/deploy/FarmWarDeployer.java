...
public class FarmWarDeployer extends ClusterListener implements
	...
	protected HashMap<String, FileMessageFactory> fileFactories = new HashMap<String, FileMessageFactory>();
	...
	public void start() throws Exception {
		...
		try {
			...
			log.error(
					sm.getString("farmWarDeployer.mbeanNameFail",
		...
		if (watchEnabled) {
			...
			if (log.isInfoEnabled()) {
				log.info(sm
						.getString("farmWarDeployer.watchDir", getWatchDir()));
			}
		}
			...
			if (msg instanceof FileMessage) {
				...
				try {
					...
					if (!isServiced(contextName)) {
						...
						if (log.isDebugEnabled())
							log.debug(sm.getString(
									"farmWarDeployer.undeployEnd", contextName));
					} else
						log.error(sm
								.getString("farmWarDeployer.servicingUneploy",
										contextName));
				} catch (Exception ex) {
					log.error(ex);
				}
	 ...
	 * with the removed <code>Context</code> as an argument. Deletes the web
	 * application war file and/or directory if they exist in the Host's
	 * appBase.
	 * 
	 * @param contextName
	...
	public void remove(String contextName, boolean undeploy) throws IOException {
		if (getCluster().getMembers().length > 0) {
			...
			UndeployMessage msg = new UndeployMessage(localMember,
					System.currentTimeMillis(), "Undeploy:" + contextName + ":"
	...
	public void fileModified(File newWar) {
		try {
			...
			if (deployWar.exists()
					&& deployWar.lastModified() > newWar.lastModified()) {
				if (log.isInfoEnabled())
