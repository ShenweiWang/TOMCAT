...
public class NamingResources extends LifecycleMBeanBase implements Serializable {
	...
	private Server getServer() {
		...
		if (container instanceof Context) {
			...
			Engine engine = (Engine) ((Context) container).getParent()
					.getParent();
	...
	public MessageDestinationRef[] findMessageDestinationRefs() {
		...
		synchronized (mdrs) {
			MessageDestinationRef results[] = new MessageDestinationRef[mdrs
					.size()];
	...
	public ContextResourceEnvRef[] findResourceEnvRefs() {
		...
		synchronized (resourceEnvRefs) {
			ContextResourceEnvRef results[] = new ContextResourceEnvRef[resourceEnvRefs
					.size()];
	...
	protected void initInternal() throws LifecycleException {
		...
		for (ContextResource cr : resources.values()) {
			try {
				...
				log.warn(
						sm.getString("namingResources.mbeanCreateFail",
								cr.getName()), e);
			}
		}
	...
	private void cleanUp() {
		...
		try {
			...
			log.warn(
					sm.getString("namingResources.cleanupNoContext", container),
					e);
			return;
		}
		for (ContextResource cr : resources.values()) {
	...
	protected void destroyInternal() throws LifecycleException {
		...
		for (ContextResourceLink crl : resourceLinks.values()) {
			try {
				...
				log.warn(
						sm.getString("namingResources.mbeanDestroyFail",
								crl.getName()), e);
			}
		}
	...
	private boolean checkResourceType(ResourceBase resource) {
		...
		Class<?> compatibleClass = getCompatibleType(context, resource,
				typeClass);
	...
	private Class<?> getCompatibleType(Context context, ResourceBase resource,
			Class<?> typeClass) {
		...
		for (InjectionTarget injectionTarget : resource.getInjectionTargets()) {
			Class<?> clazz = Introspection.loadClass(context,
					injectionTarget.getTargetClass());
			if (clazz == null) {
