...
public class CoyoteAdapter implements Adapter {
	...
	private static final EnumSet<SessionTrackingMode> SSL_ONLY = EnumSet
	...
	public boolean event(org.apache.coyote.Request req,
		...
		try {
			...
			if (!error
				...
				// An unexpected exception occurred while processing the event,
				// so
	...
	public boolean asyncDispatch(org.apache.coyote.Request req,
		...
		AsyncContextImpl asyncConImpl = (AsyncContextImpl) request
				.getAsyncContext();
		req.getRequestProcessor().setWorkerThreadName(
				Thread.currentThread().getName());
		try {
		...
		try {
			...
			if (request.mappingData != null) {
				if (request.mappingData.context != null) {
					logged = true;
					((Context) request.mappingData.context).logAccess(request,
							response, time, true);
				} else if (request.mappingData.host != null) {
					logged = true;
					((Host) request.mappingData.host).logAccess(request,
							response, time, true);
				}
			}
			if (!logged) {
				connector.getService().getContainer()
						.logAccess(request, response, time, true);
			}
		} catch (Throwable t) {
			ExceptionUtils.handleThrowable(t);
	...
	protected boolean postParseRequest(org.apache.coyote.Request req,
		...
		try {
			...
			connector.getService().getContainer()
					.logAccess(request, response, 0, true);
			return false;
		}
		// Normalization
		...
		if (!redirectPathMB.isNull()) {
			...
			if (request.isRequestedSessionIdFromURL()) {
				...
				redirectPath = redirectPath
						+ ";"
						+ SessionConfig.getSessionUriParamName(request
								.getContext()) + "="
