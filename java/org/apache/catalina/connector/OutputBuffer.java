 ...
 * OutputBuffer, with the removal of some of the state handling (which in Coyote
 * is mostly the Processor's responsibility).
 *
 * @author Costin Manolache
 * @author Remy Maucherat
 */
public class OutputBuffer extends Writer implements
	 ...
	 * List of encoders.
	 */
	protected HashMap<String, C2BConverter> encoders = new HashMap<String, C2BConverter>();
	...
	/**
	 * Close the output buffer. This tries to calculate the response size if the
	...
	public void close() throws IOException {
		...
		// If there are chars, flush all of them to the byte buffer now as bytes
		// are used to
	...
	/**
	 * Sends the buffer data to the client output, checking the state of
	 * Response and calling the right interceptors.
	 *
	 * @param buf
	 ...
	 * 
	 * @param buf
	 *            Char buffer to be written to the response
	 * @param off
	 *            Offset
	 * @param len
	 *            Length
	...
	public void realWriteChars(char buf[], int off, int len) throws IOException {
		...
		while (outputCharChunk.getLength() > 0) {
			...
			if (bb.getLength() == 0) {
				// Break out of the loop if more chars are needed to produce any
				// output
				break;
			}
			if (outputCharChunk.getLength() > 0) {
				if (bb.getBuffer().length == bb.getEnd()
						&& bb.getLength() < bb.getLimit()) {
	...
	/**
	 * True if this buffer hasn't been used ( since recycle() ) - i.e. no chars
	 * or bytes have been added to the buffer.
	 */
