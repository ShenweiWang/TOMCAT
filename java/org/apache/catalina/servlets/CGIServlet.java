...
public final class CGIServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		if (getServletConfig().getInitParameter("parameterEncoding") != null) {
			parameterEncoding = getServletConfig().getInitParameter(
					"parameterEncoding");
		}
	...
	/**
	 * Encapsulates the CGI environment and rules to derive that environment
	 * from the servlet container and request information.
	 *
	 * <p>
	 * </p>
	 *
	...
	protected class CGIEnvironment {
		...
		/**
		 * Creates a CGIEnvironment and derives the necessary environment, query
		 * parameters, working directory, cgi command, etc.
		 *
		 * @param req
		 *            HttpServletRequest for information provided by the Servlet
		 *            API
		 * @param context
		...
		protected void setupFromRequest(HttpServletRequest req)
			...
			if (isIncluded) {
				this.contextPath = (String) req
						.getAttribute(RequestDispatcher.INCLUDE_CONTEXT_PATH);
				this.servletPath = (String) req
						.getAttribute(RequestDispatcher.INCLUDE_SERVLET_PATH);
				this.pathInfo = (String) req
			...
			if (req.getMethod().equals("GET") || req.getMethod().equals("POST")
				...
				if (isIncluded) {
					qs = (String) req
							.getAttribute(RequestDispatcher.INCLUDE_QUERY_STRING);
				} else {
				...
				if (qs != null && qs.indexOf("=") == -1) {
					...
					while (qsTokens.hasMoreTokens()) {
						cmdLineParameters.add(URLDecoder.decode(
								qsTokens.nextToken(), parameterEncoding));
					}
				}
			}
		}
		 ...
		 * &lt;my-webapp-root&gt; and find the first non-directory in the
		 * getPathTranslated("/"), reading/searching from left-to-right.
		 * </p>
		 * <p>
		 * The CGI search path will start at webAppRootDir + File.separator +
		 * cgiPathPrefix (or webAppRootDir alone if cgiPathPrefix is null).
		 ...
		 *
		 * @param pathInfo
		 *            String from HttpServletRequest.getPathInfo()
		 * @param webAppRootDir
		 *            String from context.getRealPath("/")
		 * @param contextPath
		 *            String as from HttpServletRequest.getContextPath()
		 * @param servletPath
		 *            String as from HttpServletRequest.getServletPath()
		 * @param cgiPathPrefix
		 *            subdirectory of webAppRootDir below which the web app's
		 *            CGIs may be stored; can be null. The CGI search path will
		 *            start at webAppRootDir + File.separator + cgiPathPrefix
		 *            (or webAppRootDir alone if cgiPathPrefix is null).
		 *            cgiPathPrefix is defined by setting the servlet's
		 *            cgiPathPrefix init parameter.
		 *
		 *
		 * @return <ul>
		 *         <li>
		 *         <code>path</code> - full file-system path to valid cgi
		 *         script, or null if no cgi was found
		 *         <li>
		 *         <code>scriptName</code> - CGI variable SCRIPT_NAME; the full
		 *         URL path to valid cgi script or null if no cgi was found
		 *         <li>
		 *         <code>cgiName</code> - servlet pathInfo fragment
		 *         corresponding to the cgi script itself, or null if not found
		 *         <li>
		 *         <code>name</code> - simple name (no directories) of the cgi
		 *         script, or null if no cgi was found
		 *         </ul>
		 *
		 * @since Tomcat 4.0
		...
		protected String[] findCGI(String pathInfo, String webAppRootDir,
			...
			if ((webAppRootDir != null)
					&& (webAppRootDir.lastIndexOf(File.separator) == (webAppRootDir
							.length() - 1))) {
		 ...
		 *
		 * @param req
		 *            request associated with the CGI Invocation
		 *
		...
		protected boolean setCGIEnvironment(HttpServletRequest req)
			...
			Integer iPort = (port == 0 ? Integer.valueOf(-1) : Integer
					.valueOf(port));
			...
			String sContentLength = (contentLength <= 0 ? "" : (Integer
					.valueOf(contentLength)).toString());
		...
		protected void expandCGIScript() {
			...
			// create directories
			String dirPath = destPath.toString().substring(0,
					destPath.toString().lastIndexOf("/"));
		...
		protected String blanksToString(String couldBeBlank, String subForBlanks) {
			return (("".equals(couldBeBlank) || couldBeBlank == null) ? subForBlanks
					: couldBeBlank);
		}
	...
	/**
	 * Encapsulates the knowledge of how to run a CGI script, given the script's
	 * desired environment and (optionally) input/output streams
	 *
	 * <p>
	 *
	 ...
	 * <p>
	 *
	 * The CGI environment and settings are derived from the information passed
	 * to the constructor.
	 *
	 * </p>
	 * <p>
	 *
	 * The input and output streams can be set by the <code>setInput</code> and
	 * <code>setResponse</code> methods, respectively.
	 * </p>
	 */
	protected class CGIRunner {
		 ...
		 * @param params
		 *            ArrayList with the script's query command line parameters
		 *            as strings
		 */
		...
		protected void updateReadyStatus() {
			if (command != null && env != null && wd != null && params != null
					&& response != null) {
				readyToRun = true;
			} else {
				readyToRun = false;
		 ...
		 * Gets ready status
		 *
		 * @return false if not ready (<code>run</code> will throw an
		 *         exception), true if ready
		 */
		...
		/**
		 * Sets HttpServletResponse object used to set headers and send output
		 * to
		 *
		 * @param response
		 ...
		 * requestURI before determining the contextPath, servletPath and
		 * pathInfo);
		...
		protected void run() throws IOException {
			 ...
			 * http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4216884 with
			 * major modifications by Martin Dengler
			 */
			Runtime rt = null;
			...
			try {
				...
				if (!"".equals(sContentLength)) {
					commandsStdIn = new BufferedOutputStream(
				...
				commandsStdErr = new BufferedReader(new InputStreamReader(
						proc.getErrorStream()));
				...
				InputStream cgiHeaderStream = new HTTPHeaderInputStream(
						proc.getInputStream());
				cgiHeaderReader = new BufferedReader(new InputStreamReader(
						cgiHeaderStream));
