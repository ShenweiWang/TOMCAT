 ...
 * directory with the specified name in their home directories. The context path
 * of each deployed application will be set to <code>~xxxxx</code>, where xxxxx
 * is the username of the owning user for that web application
 *
		...
		for (Future<?> result : results) {
			try {
				...
				host.getLogger().error(
						sm.getString("userConfig.deploy.threaded.error"), e);
			}
		}
	}
