...
public class WebAnnotationSet {
	...
	protected static void loadClassAnnotation(Context context,
		...
		/*
		 * Process EJB annotation. Ref JSR 224, equivalent to the ejb-ref or
		 * ejb-local-ref element in the deployment descriptor. { EJB annotation
		 * = classClass.getAnnotation(EJB.class); if (annotation != null) {
		 * 
		 ...
		 */
		/*
		 * Process WebServiceRef annotation. Ref JSR 224, equivalent to the
		 * service-ref element in the deployment descriptor. The service-ref
		 * registration is not implemented { WebServiceRef annotation =
		 * classClass .getAnnotation(WebServiceRef.class); if (annotation !=
		 * null) { ContextService service = new ContextService();
		 * 
		 ...
		 */
		/*
		 * Process DeclareRoles annotation. Ref JSR 250, equivalent to the
		 * security-role element in the deployment descriptor
		 */
		{
				...
				if (annotation != null) {
					if (!Introspection.isValidSetter(method)) {
						throw new IllegalArgumentException(
								sm.getString("webAnnotationSet.invalidInjection"));
					}
	...
	/**
	 * Process a Resource annotation to set up a Resource. Ref JSR 250,
	 * equivalent to the resource-ref, message-destination-ref, env-ref,
	 * resource-env-ref or service-ref element in the deployment descriptor.
	 */
	...
	protected static void addResource(Context context, Resource annotation,
		...
		if (type.equals("java.lang.String")
			...
			context.getNamingResources().addMessageDestinationRef(resource);
				...
				|| type.equals("javax.transaction.UserTransaction") || true) {
