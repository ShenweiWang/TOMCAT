...
/**
 * Rule to check that the <code>jsp-config</code> is occurring only 1 time
 * within the web.xml
 */
...
/**
 * Rule to check that the <code>session-config</code> is occurring only 1 time
 * within the web.xml
 */
...
final class SetAuthConstraintRule extends Rule {
	...
	public void begin(String namespace, String name, Attributes attributes)
			throws Exception {
		SecurityConstraint securityConstraint = (SecurityConstraint) digester
				.peek();
...
final class ServletDefCreateRule extends Rule {
	...
	public void begin(String namespace, String name, Attributes attributes)
		...
		if (digester.getLogger().isDebugEnabled())
			digester.getLogger()
					.debug("new " + servletDef.getClass().getName());
	}
	...
	public void end(String namespace, String name) throws Exception {
		...
		if (digester.getLogger().isDebugEnabled())
			digester.getLogger()
					.debug("pop " + servletDef.getClass().getName());
	}
 ...
 * needed (used for addServletMapping).
 */
...
final class CallMethodMultiRule extends CallMethodRule {
	...
	public CallMethodMultiRule(String methodName, int paramCount,
			int multiParamIndex) {
			...
			if (i != multiParamIndex) {
				...
				if (parameters[i] == null
					...
					paramValues[i] = IntrospectionUtils.convert(
							(String) parameters[i], paramTypes[i]);
				} else {
		...
		for (int j = 0; j < multiParams.size(); j++) {
			...
			if (param == null
					|| (param instanceof String && !String.class
							.isAssignableFrom(paramTypes[multiParamIndex]))) {
				paramValues[multiParamIndex] = IntrospectionUtils.convert(
						(String) param, paramTypes[multiParamIndex]);
			} else {
...
final class NameRule extends Rule {
	...
	public void begin(String namespace, String name, Attributes attributes)
		...
		if (isNameSet) {
			throw new IllegalArgumentException(
					WebRuleSet.sm.getString("webRuleSet.nameCount"));
		}
		isNameSet = true;
	}
...
final class AbsoluteOrderingRule extends Rule {
	...
	public void begin(String namespace, String name, Attributes attributes)
		...
		if (isAbsoluteOrderingSet) {
			throw new IllegalArgumentException(
					WebRuleSet.sm.getString("webRuleSet.absoluteOrderingCount"));
		} else {
...
final class RelativeOrderingRule extends Rule {
	...
	public void begin(String namespace, String name, Attributes attributes)
		...
		if (isRelativeOrderingSet) {
			throw new IllegalArgumentException(
					WebRuleSet.sm.getString("webRuleSet.relativeOrderingCount"));
		} else {
...
/**
 * A rule that fails if more than one post construct or pre destroy methods are
