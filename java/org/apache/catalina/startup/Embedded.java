...
/**
 * Convenience class to embed a Catalina servlet container environment inside
 * another application. You must call the methods of this class in the following
 ...
 * associated with the newly created Engine, and then call its property setters
 ...
 * associated with each newly created Host, and then call its property setters
...
public class Embedded extends StandardService {
	...
	/**
	 * The default realm to be used by all containers associated with this
	 * component.
	 */
	...
	public Connector createConnector(String address, int port, String protocol) {
		...
		if (log.isDebugEnabled()) {
			log.debug("Creating connector for address='"
					+ ((address == null) ? "ALL" : address) + "' port='" + port
					+ "' protocol='" + protocol + "'");
		}
	 ...
	 * requests received from one of the associated Connectors, based on the
	 * specified properties.
	 */
	...
	/**
	 * Create and return a class loader manager that can be customized, and then
	 * attached to a Context, before it is started.
	 *
	 * @param parent
	 *            ClassLoader that will be the parent of the one created by this
	 *            Loader
	 */
	 ...
	 * the set of defined Hosts for its associated Engine. If this is the last
	 ...
	 * and if something is returned assume false ? XXX We have a major problem
	...
	protected void initDirs() {
		...
		if (System.getProperty(Globals.CATALINA_BASE_PROP) == null) {
			...
			String catalinaBase = System
					.getProperty(Globals.CATALINA_BASE_PROP);
