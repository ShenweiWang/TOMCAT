 ...
 * </pre>
 * 
 * . These methods create a webapp context, configure it, and then add it to a
 * {@link Host}. They do not use a global default web.xml; rather, they add a
 * lifecycle listener that adds the standard DefaultServlet, JSP processing, and
 * welcome files.
 * 
 ...
 * </pre>
 * 
 * methods, you may want to call two methods of this class:
...
public class Tomcat {
	 ...
	 * catalina.base, catalina.home - $HOME/tomcat.$PORT ( /tmp doesn't seem a
	 * good choice for security ).
	 * 
	 *
	 * TODO: better default ? Maybe current dir ? TODO: disable work dir if not
	 * needed ( no jsp, etc ).
	 */
	...
	/**
	 * Equivalent with <servlet><servlet-name><servlet-class>.
	 * 
	 ...
	 * 
	 * @param contextPath
	 *            Context to add Servlet to
	 * @param servletName
	 *            Servlet name (used in mappings)
	 * @param servletClass
	 *            The class to be used for the Servlet
	 * @return The wrapper for the servlet
	 */
	public Wrapper addServlet(String contextPath, String servletName,
			String servletClass) {
	 ...
	 * @return The wrapper for the servlet
	 */
	public static Wrapper addServlet(Context ctx, String servletName,
			String servletClass) {
	...
	/**
	 * Set a custom realm for auth. If not called, a simple default will be
	 * used, using an internal map.
	 * 
	...
	public Context addContext(Host host, String contextPath,
			String contextName, String dir) {
	 ...
	 * obtain the simple realm created by default.
	 * 
	 ...
	 * 
	 * @param silent
	 *            <code>true</code> sets the log level to WARN for the loggers
	 *            that log information on Tomcat start up. This prevents the
	 *            usual startup information being logged. <code>false</code>
	...
	public void enableNaming() {
		...
		String oldValue = System
				.getProperty(javax.naming.Context.URL_PKG_PREFIXES);
		if (oldValue != null) {
		...
		value = System
				.getProperty(javax.naming.Context.INITIAL_CONTEXT_FACTORY);
		if (value == null) {
	...
	public static void initWebappDefaults(Context ctx) {
		// Default servlet
		Wrapper servlet = addServlet(ctx, "default",
	...
	private URL getWebappConfigFileFromDirectory(File docBase, String url) {
		...
		if (webAppContextXml.exists()) {
			try {
				...
				Logger.getLogger(getLoggerName(getHost(), url)).log(
						Level.WARNING,
