...
public class WsFrame {
	 ...
	 *
	 * @param first
	 *            First byte of data for this frame
	 * @param processor
	 *            Processor associated with the WebSocket connection on which
	 *            the frame has been sent
	 *
	 * @throws IOException
	 *             If a problem occurs processing the frame. Any exception will
	 *             trigger the closing of the WebSocket connection.
	 */
	private WsFrame(byte first, UpgradeProcessor<?> processor)
			throws IOException {
	 ...
	 *
	 * @param block
	 *            Should this method block until a frame is presented if no data
	 *            is currently available to process. Note that is a single byte
	 *            is available, this method will block until the complete frame
	 *            (excluding payload for non-control frames) is available.
	 *
	 * @throws IOException
	 *             If a problem occurs processing the frame. Any exception will
	 *             trigger the closing of the WebSocket connection.
	 */
	public static WsFrame nextFrame(UpgradeProcessor<?> processor, boolean block)
			throws IOException {
		...
		if (read == 1) {
			...
			throw new IOException(sm.getString("frame.readFailed",
