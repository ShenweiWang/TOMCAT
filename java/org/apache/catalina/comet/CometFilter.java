 ...
 * either the request to a resource (a Comet servlet), or on the response from a
 * resource, or both. <br>
 * <br>
 * Filters perform filtering in the <code>doFilterEvent</code> method. Every
 * Filter has access to a FilterConfig object from which it can obtain its
...
public interface CometFilter extends Filter {
	...
	/**
	 * The <code>doFilterEvent</code> method of the CometFilter is called by the
	 * container each time a request/response pair is passed through the chain
	 * due to a client event for a resource at the end of the chain. The
	 * CometFilterChain passed in to this method allows the Filter to pass on
	 * the event to the next entity in the chain.
	 * <p>
	 * A typical implementation of this method would follow the following
	 * pattern:- <br>
	 * 1. Examine the request<br>
	 * 2. Optionally wrap the request object contained in the event with a
	 * custom implementation to filter content or headers for input filtering
	 * and pass a CometEvent instance containing the wrapped request to the next
	 * filter<br>
	 * 3. Optionally wrap the response object contained in the event with a
	 * custom implementation to filter content or headers for output filtering
	 * and pass a CometEvent instance containing the wrapped request to the next
	 * filter<br>
	 ...
	 *
	 * @param event
	 *            the event that is being processed. Another event may be passed
	 *            along the chain.
	 * @param chain
	 * @throws IOException
