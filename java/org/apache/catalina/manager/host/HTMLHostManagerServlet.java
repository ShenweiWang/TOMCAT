...
/**
 * Servlet that enables remote management of the virtual hosts deployed on the
 ...
 * <p>
 * The difference between the <code>HostManagerServlet</code> and this Servlet
 * is that this Servlet prints out a HTML interface which makes it easier to
 * administrate.
 * <p>
 * However if you use a software that parses the output of
 * <code>HostManagerServlet</code> you won't be able to upgrade to this Servlet
 * since the output are not in the same format as from
...
public final class HTMLHostManagerServlet extends HostManagerServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (command == null) {
			...
			message = smClient.getString("hostManagerServlet.postCommand",
					command);
		} else {
			message = smClient.getString("hostManagerServlet.unknownCommand",
					command);
		}
	...
	public void list(HttpServletRequest request, HttpServletResponse response,
			String message, StringManager smClient) throws IOException {
		...
		args[3] = response
				.encodeURL(request.getContextPath()
						+ "/"
						+ smClient
								.getString("htmlHostManagerServlet.helpHtmlManagerFile"));
		...
		String hostsStart = smClient
				.getString("htmlHostManagerServlet.hostsStart");
		String hostsStop = smClient
				.getString("htmlHostManagerServlet.hostsStop");
		String hostsRemove = smClient
				.getString("htmlHostManagerServlet.hostsRemove");

		Iterator<Map.Entry<String, String>> iterator = sortedHostNamesMap
				.entrySet().iterator();
		while (iterator.hasNext()) {
			...
			if (host != null) {
				...
				if (host == this.installedHost) {
					...
					writer.print(MessageFormat.format(HOSTS_ROW_BUTTON_SECTION,
							args));
				}
			}
		}
		...
		args[0] = smClient
				.getString("htmlHostManagerServlet.addDeployOnStartup");
