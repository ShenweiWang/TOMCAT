 ...
 * during testing.
 * <p>
 * The difference between the <code>ManagerServlet</code> and this Servlet is
 * that this Servlet prints out a HTML interface which makes it easier to
 * administrate.
 * <p>
 * However if you use a software that parses the output of
 * <code>ManagerServlet</code> you won't be able to upgrade to this Servlet
 * since the output are not in the same format ar from
 * <code>ManagerServlet</code>
 *
 * @author Bip Thelin
...
public final class HTMLManagerServlet extends ManagerServlet {
	...
	protected String upload(HttpServletRequest request, StringManager smClient) {
		...
		try {
			while (true) {
				...
				if (warPart == null) {
					message = smClient
							.getString("htmlManagerServlet.deployUploadNoFile");
					break;
				}
				String filename = extractFilename(warPart
	 ...
	 *
	 * @param config
	 *            URL of the context configuration file to be deployed
	 * @param cn
	...
	protected void list(HttpServletRequest request,
			HttpServletResponse response, String message, StringManager smClient)
			throws IOException {
		...
		String appsUndeploy = smClient
				.getString("htmlManagerServlet.appsUndeploy");
		...
		for (String contextName : contextNames) {
			...
			if (ctxt != null) {
				...
				args[9] = appsExpire;
				args[10] = smClient
						.getString("htmlManagerServlet.expire.explain");
				if (manager == null) {
					args[11] = smClient
							.getString("htmlManagerServlet.noManager");
				} else {
					args[11] = Integer.valueOf(ctxt.getManager()
		...
		args[4] = smClient
				.getString("htmlManagerServlet.diagnosticsLeakButton");
	 ...
	 *
	 * @param cn
	 *            Name of the application to be undeployed
	 * @param smClient
	 *            StringManager for the client's locale
	 * @return message String
	 */
	 ...
	 *
	 * @param cn
	 *            Name of the application to list session information
	 * @param idle
	 *            Expire all sessions with idle time &ge; idle for this context
	 * @param smClient
	 *            StringManager for the client's locale
	 * @return message String
	 */
	...
	protected String findleaks(StringManager smClient) {
		...
		if (writerText.length() > 0) {
			if (!writerText.startsWith("FAIL -")) {
				msg.append(smClient
						.getString("htmlManagerServlet.findleaksList"));
			}
	 ...
	 * 
	 * @param cn
	 *            Name of the application from which to expire sessions
	 * @param req
	 * @param smClient
	 *            StringManager for the client's locale
	 */
	...
	protected void displaySessionDetailPage(HttpServletRequest req,
		...
		getServletContext().getRequestDispatcher(
				resp.encodeURL(sessionDetailJspPath)).include(req, resp);
	}
	...
	protected Comparator<Session> getComparator(String sortBy) {
		...
		if ("CreationTime".equalsIgnoreCase(sortBy)) {
			...
			comparator = new BaseSessionComparator<Date>() {
				...
				public Comparable<Date> getComparableObject(Session session) {
					return new Date(
							SessionUtils.getInactiveTimeForSession(session));
				}
			};
