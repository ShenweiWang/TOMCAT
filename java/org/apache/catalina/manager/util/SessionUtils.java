...
public class SessionUtils {
	 ...
	 * NOTE: this method has explicit support for Tapestry 3, Struts 1.x and
	...
	public static Locale guessLocaleFromSession(final HttpSession in_session) {
		...
		try {
			...
			for (int i = 0; i < LOCALE_TEST_ATTRIBUTES.length; ++i) {
				...
				obj = in_session.getAttribute(LOCALE_TEST_ATTRIBUTES[i]
			...
			// TODO: Tapestry 4+
			{
				...
				for (Enumeration<String> enumeration = in_session
						.getAttributeNames(); enumeration.hasMoreElements();) {
				...
				if (tapestryArray.size() == 1) {
					...
					if (null != probableEngine) {
						try {
							Method readMethod = probableEngine.getClass()
									.getMethod("getLocale", (Class<?>[]) null);//$NON-NLS-1$
							if (null != readMethod) {
								// Call the property getter and return the value
								Object possibleLocale = readMethod.invoke(
										probableEngine, (Object[]) null);
	...
	public static Object guessUserFromSession(final Session in_session) {
		...
		try {
			...
			for (int i = 0; i < USER_TEST_ATTRIBUTES.length; ++i) {
				...
				obj = httpSession.getAttribute(USER_TEST_ATTRIBUTES[i]
						.toLowerCase(Locale.ENGLISH));
				if (null != obj) {
					user = obj;
					break;
			...
			// If there is only one, consider it to be /the/ user
			{
				...
				for (Enumeration<String> enumeration = httpSession
						.getAttributeNames(); enumeration.hasMoreElements();) {
					String name = enumeration.nextElement();
					Object obj = httpSession.getAttribute(name);
					if (null != obj
							&& (obj instanceof Principal || obj instanceof Subject)) {
	...
	public static long getTTLForSession(Session in_session) {
		try {
			long diffMilliSeconds = (1000 * in_session.getMaxInactiveInterval())
					- (System.currentTimeMillis() - in_session
							.getThisAccessedTime());
			return diffMilliSeconds;
		} catch (IllegalStateException ise) {
			// ignore: invalidated session
	...
	public static long getInactiveTimeForSession(Session in_session) {
		try {
			long diffMilliSeconds = System.currentTimeMillis()
					- in_session.getThisAccessedTime();
			return diffMilliSeconds;
		} catch (IllegalStateException ise) {
			// ignore: invalidated session
			return -1;
