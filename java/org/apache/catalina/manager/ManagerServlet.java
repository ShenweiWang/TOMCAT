 ...
 * the application.</li>
 * <li><b>/deploy?config={config-url}&war={war-url}/</b> - Install and start a
 * new web application, based on the contents of the context configuration file
 * found at <code>{config-url}</code>, overriding the <code>docBase</code>
 ...
 * host.</li>
 * <li><b>/expire?path=/xxx&idle=mm</b> - Expire sessions for the context path
 * <code>/xxx</code> which were idle for at least mm minutes.</li>
 ...
 * changes.</li>
 * <li><b>jar:file:/absolute/path/to/a/warfile.war!/</b> - You can specify a URL
 * to a local web application archive file. The syntax must conform to the rules
 * specified by the <code>JarURLConnection</code> class for a reference to an
 * entire JAR file.</li>
 * <li><b>jar:http://hostname:port/path/to/a/warfile.war!/</b> - You can specify
 * a URL to a remote (HTTP-accessible) web application archive file. The syntax
 * must conform to the rules specified by the <code>JarURLConnection</code>
 * class for a reference to an entire JAR file.</li>
 * </ul>
 * <p>
...
public class ManagerServlet extends HttpServlet implements ContainerServlet {
	...
	static {
		String lastAccessAtStart = System
				.getProperty("org.apache.catalina.session.StandardSession.LAST_ACCESS_AT_START");
		if (lastAccessAtStart == null) {
			LAST_ACCESS_AT_START = Globals.STRICT_SERVLET_COMPLIANCE;
		} else {
			LAST_ACCESS_AT_START = Boolean.valueOf(lastAccessAtStart)
					.booleanValue();
		}
	}
	...
	protected void findleaks(boolean statusLine, PrintWriter writer,
		...
		String[] results = ((StandardHost) host)
				.findReloadedContextMemoryLeaks();

		if (results.length > 0) {
			if (statusLine) {
				writer.println(smClient
						.getString("managerServlet.findleaksList"));
			}
	...
	protected synchronized void save(PrintWriter writer, String path,
		...
		if (!(server instanceof StandardServer)) {
			writer.println(smClient
					.getString("managerServlet.saveFail", server));
			return;
		}
		...
		if ((path == null) || path.length() == 0 || !path.startsWith("/")) {
			...
			try {
				...
				writer.println(smClient.getString(
						"managerServlet.savedContext", path));
			} catch (Exception e) {
		...
		if (tag == null) {
			if (update) {
				...
				if (uploadedWar.exists() && !uploadedWar.delete()) {
					writer.println(smClient.getString(
							"managerServlet.deleteFail", uploadedWar));
				}
			} else {
				...
				try {
					...
					if (update && tag == null) {
						if (deployedWar.exists() && !deployedWar.delete()) {
							writer.println(smClient.getString(
									"managerServlet.deleteFail", deployedWar));
							return;
						}
		...
		for (int i = 0; i < contexts.length; i++) {
			...
			if (context != null) {
				...
				if (context.getState().isAvailable()) {
					writer.println(smClient.getString(
							"managerServlet.listitem", displayPath, "running",
							"" + context.getManager().findSessions().length,
							context.getDocBase()));
				} else {
					writer.println(smClient.getString(
							"managerServlet.listitem", displayPath, "stopped",
							"0", context.getDocBase()));
				}
			}
		}
	}
	...
	protected void printResources(PrintWriter writer, String prefix,
			javax.naming.Context namingContext, String type, Class<?> clazz,
			StringManager smClient) {
	...
	protected void sessions(PrintWriter writer, ContextName cn, int idle,
		...
		if (debug >= 1) {
			log("sessions: Session information for web application '" + cn
					+ "'");
			if (idle >= 0)
				log("sessions: Session expiration for " + idle + " minutes '"
						+ cn + "'");
		}
		...
		try {
			...
			for (int i = 0; i < sessions.length; i++) {
				...
				if (LAST_ACCESS_AT_START) {
					time = (int) ((now - sessions[i]
							.getLastAccessedTimeInternal()) / 1000L);
				} else {
					time = (int) ((now - sessions[i]
							.getThisAccessedTimeInternal()) / 1000L);
				}
			...
			if (timeout[0] > 0)
				writer.println(smClient.getString(
						"managerServlet.sessiontimeout", "<" + histoInterval,
						"" + timeout[0]));
			for (int i = 1; i < maxCount - 1; i++) {
				if (timeout[i] > 0)
					writer.println(smClient.getString(
							"managerServlet.sessiontimeout", "" + (i)
									* histoInterval + " - <" + (i + 1)
									* histoInterval, "" + timeout[i]));
			}
			if (timeout[maxCount - 1] > 0)
				writer.println(smClient.getString(
						"managerServlet.sessiontimeout", ">=" + maxCount
								* histoInterval, "" + timeout[maxCount - 1]));
			if (notimeout > 0)
				writer.println(smClient.getString(
			...
			if (idle >= 0)
				writer.println(smClient.getString(
						"managerServlet.sessiontimeout.expired", "" + idle, ""
								+ expired));
		} catch (Throwable t) {
			ExceptionUtils.handleThrowable(t);
	 ...
	 *
	 * @deprecated Use
	 *             {@link #sessions(PrintWriter, ContextName, int, StringManager)}
	 */
	@Deprecated
	...
	protected void stop(PrintWriter writer, ContextName cn,
		...
		try {
			...
			context.stop();
			writer.println(smClient.getString("managerServlet.stopped",
					displayPath));
		} catch (Throwable t) {
			ExceptionUtils.handleThrowable(t);
	...
	protected boolean isDeployed(String name) throws Exception {
		...
		Boolean result = (Boolean) mBeanServer.invoke(oname, "isDeployed",
				params, signature);
