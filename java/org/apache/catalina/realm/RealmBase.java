...
/**
 * Simple implementation of <b>Realm</b> that reads an XML file to configure the
 * valid users, passwords, and roles. The file format (and default file
...
public abstract class RealmBase extends LifecycleMBeanBase implements Realm {
	...
	/**
	 * Gets the name of the class that will be used to extract user names from
	...
	/**
	 * Sets the name of the class that will be used to extract user names from
	 * X509 client certificates. The class must implement (@link
		...
		if (!validated) {
			if (containerLog.isTraceEnabled()) {
				containerLog.trace(sm.getString(
						"realmBase.authenticateFailure", username));
			}
			return null;
		}
		if (containerLog.isTraceEnabled()) {
	...
	public Principal authenticate(String username, String clientDigest,
		...
		if (log.isDebugEnabled()) {
			log.debug("Digest : " + clientDigest + " Username:" + username
					...
					+ " realm:" + realm + "md5a2:" + md5a2 + " Server digest:"
					+ serverDigest);
		}
	...
	protected boolean compareCredentials(String userCredentials,
		...
		if (hasMessageDigest()) {
			...
			if (serverCredentials.startsWith("{MD5}")
				...
				// digested password.
				byte[] serverDigestPlusSaltBytes = Base64
						.decodeBase64(serverDigestPlusSalt);
					...
					if (pattern.startsWith("/") && pattern.endsWith("/*")
						...
						if (pattern.length() == 2) {
							...
							length = pattern.length();
								...
								pattern.length() - 1)
								|| (pattern.length() - 2 == uri.length() && pattern
										.regionMatches(0, uri, 0,
												pattern.length() - 2))) {
							matched = true;
							length = pattern.length();
						}
				...
				for (int k = 0; k < patterns.length && !matched; k++) {
					...
					if (pattern.startsWith("*.")) {
						...
						if (slash >= 0 && dot > slash
							...
							if (pattern.regionMatches(1, uri, dot, uri.length()
									- dot)) {
								matched = true;
								pos = j;
							}
	...
	public boolean hasResourcePermission(Request request, Response response,
		...
		if (!denyfromall && allRolesMode != AllRolesMode.STRICT_MODE && !status
				&& principal != null) {
			if (log.isDebugEnabled()) {
			...
			for (int i = 0; i < constraints.length; i++) {
				...
				if (constraint.getAllRoles()) {
					...
					// For AllRolesMode.STRICT_AUTH_ONLY_MODE there must be zero
					// roles
					roles = request.getContext().findSecurityRoles();
					if (roles.length == 0
							&& allRolesMode == AllRolesMode.STRICT_AUTH_ONLY_MODE) {
						if (log.isDebugEnabled()) {
	...
	public boolean hasUserDataPermission(Request request, Response response,
		...
		if ((requestedSessionId != null)
			...
			file.append(";");
			file.append(SessionConfig.getSessionUriParamName(request
					.getContext()));
			file.append("=");
	...
	public static void main(String args[]) {
		...
		if (args.length > firstCredentialArg && args[0].equalsIgnoreCase("-a")) {
			...
			System.out
					.println("Usage: RealmBase -a <algorithm> [-e <encoding>] <credentials>");
		}
	...
	protected static class AllRolesMode {
		 ...
		 */
		public static final AllRolesMode STRICT_MODE = new AllRolesMode(
				"strict");
		/**
		 * Allow any authenticated user
		 */
		public static final AllRolesMode AUTH_ONLY_MODE = new AllRolesMode(
				"authOnly");
		/**
		 * Allow any authenticated user only if there are no
		 * web-app/security-roles
		 */
		public static final AllRolesMode STRICT_AUTH_ONLY_MODE = new AllRolesMode(
				"strictAuthOnly");
		...
		static AllRolesMode toMode(String name) {
			...
			if (name.equalsIgnoreCase(STRICT_MODE.name))
				...
				throw new IllegalStateException(
						"Unknown mode, must be one of: strict, authOnly, strictAuthOnly");
			return mode;
		}
	...
	private static X509UsernameRetriever createUsernameRetriever(
			String className) throws LifecycleException {
		...
		try {
			...
			throw new LifecycleException(sm.getString(
					"realmBase.createUsernameRetriever.ClassNotFoundException",
					className), e);
		} catch (InstantiationException e) {
			throw new LifecycleException(sm.getString(
					"realmBase.createUsernameRetriever.InstantiationException",
					className), e);
		} catch (IllegalAccessException e) {
			throw new LifecycleException(sm.getString(
					"realmBase.createUsernameRetriever.IllegalAccessException",
					className), e);
		} catch (ClassCastException e) {
			throw new LifecycleException(sm.getString(
					"realmBase.createUsernameRetriever.ClassCastException",
					className), e);
		}
	}
}
