 ...
 * and/or "{2}" the value of an attribute from the user's directory entry (the
 * attribute is specified by the <code>userRoleAttribute</code> property), of
 * the authenticated user for which roles will be retrieved.</li>
...
public class JNDIRealm extends RealmBase {
	...
	/**
	 * The name of the attribute inside the users directory entry where the
	 * value will be taken to search for roles This attribute is not used during
	 * a nested search
	 */
	 ...
	 * the spot where the distinguished name of the user goes. The "{1}" and
	 * "{2}" are described in the Configuration Reference.
	 */
