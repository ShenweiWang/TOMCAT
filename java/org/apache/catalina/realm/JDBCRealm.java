...
public class JDBCRealm extends RealmBase {
	...
	public synchronized Principal authenticate(Connection dbConnection,
		...
		if (validated) {
			...
			if (containerLog.isTraceEnabled())
				containerLog.trace(sm.getString(
						"jdbcRealm.authenticateFailure", username));
			return (null);
		}
			...
			try {
				...
				if (rs != null) {
					try {
						...
						containerLog.warn(sm
								.getString("jdbcRealm.abnormalCloseResultSet"));
					}
				}
			}
				...
				try {
					...
					if (rs != null) {
						try {
							...
							containerLog
									.warn(sm.getString("jdbcRealm.abnormalCloseResultSet"));
						}
					}
					dbConnection.commit();
				}
