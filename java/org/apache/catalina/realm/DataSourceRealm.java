...
/**
 *
 * Implementation of <b>Realm</b> that works with any JDBC JNDI DataSource. See
 * the JDBCRealm.howto for more details on how to set up the database and for
...
public class DataSourceRealm extends RealmBase {
		...
		if (validated) {
			if (containerLog.isTraceEnabled())
				containerLog.trace(sm.getString(
						"dataSourceRealm.authenticateSuccess", username));
		} else {
			if (containerLog.isTraceEnabled())
				containerLog.trace(sm.getString(
						"dataSourceRealm.authenticateFailure", username));
			return (null);
		}
	...
	protected void close(Connection dbConnection) {
		...
		try {
			...
			containerLog.error(
					"Exception committing connection before closing:", e);
		}
		...
		try {
			...
			containerLog.error(sm.getString("dataSourceRealm.close"), e); // Just
																			// log
																			// it
																			// here
		}
	 ...
	 *            The database connection to be used
	 * @param username
	 *            Username for which password should be retrieved
	 */
	...
	protected Principal getPrincipal(String username) {
		...
		try {
			return (new GenericPrincipal(username, getPassword(dbConnection,
					username), getRoles(dbConnection, username)));
		} finally {
			close(dbConnection);
		}
	 ...
	 * retrieve user credentials for the specified username.
	 *
	 * @param dbConnection
	 *            The database connection to be used
	 * @param username
	...
	private PreparedStatement credentials(Connection dbConnection,
		...
		PreparedStatement credentials = dbConnection
				.prepareStatement(preparedCredentials);
