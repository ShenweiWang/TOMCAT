 ...
 * either JDK 1.4 (which includes it as part of the standard platform) or JDK
 * 1.3 (with the plug-in <code>jaas.jar</code> file).
 * </p>
 *
 * <p>
 * The value configured for the <code>appName</code> property is passed to the
 * <code>javax.security.auth.login.LoginContext</code> constructor, to specify
 * the <em>application name</em> used to select the set of relevant
 ...
 * <code>Subject.getPrincipals()</code> method. However, it provides no guidance
 * on how to distinguish Principals that describe the individual user (and are
 * thus appropriate to return as the value of request.getUserPrincipal() in a
 * web application) from the Principal(s) that describe the authorized roles for
 * this user. To maintain as much independence as possible from the underlying
 ...
 * <code>Principals</code> representing the security roles authorized for this
 * user.</li>
 * <li>On the <code>Principal</code> representing the user, the Principal name
 * is an appropriate value to return via the Servlet API method
 * <code>HttpServletRequest.getRemoteUser()</code>.</li>
 * <li>On the <code>Principals</code> representing the security roles, the name
 * is the name of the authorized security role.</li>
 * <li>This Realm will be configured with two lists of fully qualified Java
 * class names of classes that implement <code>java.security.Principal</code> -
 * one that identifies class(es) representing a user, and one that identifies
 ...
 * <li>As this Realm iterates over the <code>Principals</code> returned by
 * <code>Subject.getPrincipals()</code>, it will accumulate the set of all
 * <code>Principals</code> matching the "role classes" list as identifying the
 * security roles for this user.</li>
 * <li>It is a configuration error for the JAAS login method to return a
 * validated <code>Subject</code> without a <code>Principal</code> that matches
 * the "user classes" list.</li>
 * <li>By default, the enclosing Container's name serves as the application name
 * used to obtain the JAAS LoginContext ("Catalina" in a default installation).
 * Tomcat must be able to find an application with this name in the JAAS
 * configuration file. Here is a hypothetical JAAS configuration file entry for
 * a database-oriented login module that uses a Tomcat-managed JNDI database
 * resource: <blockquote>
 * 
 * <pre>
 * Catalina {
 ...
 * 
 * </blockquote></li>
 * <li>To set the JAAS configuration file location, set the
 * <code>CATALINA_OPTS</code> environment variable similar to the following:
 * <blockquote>
 * <code>CATALINA_OPTS="-Djava.security.auth.login.config=$CATALINA_HOME/conf/jaas.config"</code>
 * </blockquote></li>
 * <li>As part of the login process, JAASRealm registers its own
 * <code>CallbackHandler</code>, called (unsurprisingly)
 * <code>JAASCallbackHandler</code>. This handler supplies the HTTP requests's
 * username and credentials to the user-supplied <code>LoginModule</code></li>
 * <li>As with other <code>Realm</code> implementations, digested passwords are
 * supported if the <code>&lt;Realm&gt;</code> element in
 * <code>server.xml</code> contains a <code>digest</code> attribute;
 * <code>JAASCallbackHandler</code> will digest the password prior to passing it
 * back to the <code>LoginModule</code></li>
 * </ul>
 *
...
public class JAASRealm extends RealmBase {
	...
	/**
	 * The application name passed to the JAAS <code>LoginContext</code>, which
	 * uses it to select the set of relevant <code>LoginModule</code>s.
	 */
	...
	/**
	 * Whether to use context ClassLoader or default ClassLoader. True means use
	 * context ClassLoader, and True is the default value.
	 */
	...
	/**
	 * Sets whether to use the context or default ClassLoader. True means use
	 * context ClassLoader.
	 *
	 * @param useContext
	...
	/**
	 * Returns whether to use the context or default ClassLoader. True means to
	...
	/**
	 * Comma-delimited list of <code>java.security.Principal</code> classes that
	 ...
	 *
	 * @param classNamesString
	 *            a comma-delimited list of fully qualified class names.
	 * @param classNamesList
	 *            the list in which the class names will be stored. The list is
	 ...
	 * The classes in the list must implement
	 ...
	 *            Username of the <code>Principal</code> to look up
	 * @param clientDigest
	 *            Digest to use in authenticating this username
	 * @param nonce
	...
	protected Principal authenticate(String username,
		...
		try {
			...
			try {
				...
				loginContext = new LoginContext(appName, null, callbackHandler,
						config);
			} catch (Throwable e) {
			...
			try {
				...
				if (log.isDebugEnabled())
					log.debug(sm
							.getString("jaasRealm.accountExpired", username));
				return (null);
	 ...
	 * <code>Subject</code>. The Principal is constructed by scanning the list
	 * of Principals returned by the JAASLoginModule. The first
	 * <code>Principal</code> object that matches one of the class names
	 * supplied as a "user class" is the user Principal. This object is returned
	 * to the caller. Any remaining principal objects returned by the
	 * LoginModules are mapped to roles, but only if their respective classes
	 * match one of the "role class" classes. If a user Principal cannot be
	 * constructed, return <code>null</code>.
	 * 
	 * @param subject
	 *            The <code>Subject</code> representing the logged-in user
	 * @param loginContext
	 *            Associated with the Principal so {@link LoginContext#logout()}
	 *            can be called later
	 */
	...
	/**
	 * Ensure the given name is legal for JAAS configuration. Added for Bugzilla
	 * 30869, made protected for easy customization in case my implementation is
	 * insufficient, which I think is very likely.
	 *
	 * @param src
	...
	protected Configuration getConfig() {
		try {
			...
			synchronized (this) {
				...
				Constructor<Configuration> constructor = sunConfigFile
