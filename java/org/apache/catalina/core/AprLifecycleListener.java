...
/**
 * Implementation of <code>LifecycleListener</code> that will init and and
 * destroy APR.
 *
...
public class AprLifecycleListener implements LifecycleListener {
	...
	private static final Log log = LogFactory
			.getLog(AprLifecycleListener.class);
	...
	private static void init() {
		...
		if (apver < rqver) {
			log.error(sm.getString("aprListener.tcnInvalid", major + "."
					+ minor + "." + patch, TCN_REQUIRED_MAJOR + "."
					+ TCN_REQUIRED_MINOR + "." + TCN_REQUIRED_PATCH));
			try {
		...
		if (apver < rcver) {
			log.info(sm.getString("aprListener.tcnVersion", major + "." + minor
					+ "." + patch, TCN_REQUIRED_MAJOR + "."
					+ TCN_RECOMMENDED_MINOR + "." + TCN_RECOMMENDED_PV));
		}

		log.info(sm.getString("aprListener.tcnValid", major + "." + minor + "."
				+ patch, Library.APR_MAJOR_VERSION + "."
		...
		if (!(null == FIPSMode || "off".equalsIgnoreCase(FIPSMode))) {
			...
			if (enterFipsMode) {
				...
				if (fipsModeState != FIPS_ON) {
					...
					String message = sm
							.getString("aprListener.initializeFIPSFailed");
					log.error(message);
