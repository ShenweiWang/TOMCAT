...
public class JreMemoryLeakPreventionListener implements LifecycleListener {
	...
	private static final Log log = LogFactory
			.getLog(JreMemoryLeakPreventionListener.class);
	...
	public void lifecycleEvent(LifecycleEvent event) {
		...
		if (Lifecycle.BEFORE_INIT_EVENT.equals(event.getType())) {
			...
			try {
				 ...
				 * java.beans.Introspector.flushCaches() in 1.6.0_15 to
				 * 1.7.0_01. From 1.7.0_02 onwards use of AppContext by
				 * Introspector.flushCaches() was replaced with
				 * ThreadGroupContext - others TBD
				 * 
				...
				if (gcDaemonProtection) {
					try {
						...
						if (Globals.IS_ORACLE_JVM) {
							log.error(sm
									.getString("jreLeakListener.gcDaemonFail"),
									e);
						} else {
							log.debug(sm
									.getString("jreLeakListener.gcDaemonFail"),
				...
				if (securityPolicyProtection) {
					try {
						...
						log.warn(
								sm.getString("jreLeakListener.authPolicyFail"),
								e);
					} catch (IllegalArgumentException e) {
						log.warn(
								sm.getString("jreLeakListener.authPolicyFail"),
				...
				if (urlCacheProtection) {
					try {
						...
						log.error(
								sm.getString("jreLeakListener.jarUrlConnCacheFail"),
								e);
					} catch (IOException e) {
						log.error(
								sm.getString("jreLeakListener.jarUrlConnCacheFail"),
				...
				if (ldapPoolProtection) {
					try {
						...
						if (Globals.IS_ORACLE_JVM) {
							log.error(
									sm.getString("jreLeakListener.ldapPoolManagerFail"),
									e);
						} else {
							log.debug(
									sm.getString("jreLeakListener.ldapPoolManagerFail"),
