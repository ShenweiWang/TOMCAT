 ...
 * the request and/or response objects that are passed on to the called
 * resource, as long as the wrapping classes extend
...
final class ApplicationDispatcher implements AsyncDispatcher, RequestDispatcher {
	...
	static {
		...
		String wrapSameObject = System
				.getProperty("org.apache.catalina.core.ApplicationDispatcher.WRAP_SAME_OBJECT");
	...
	protected class PrivilegedForward implements
	 ...
	 *            <code>null</code>
	 */
	public ApplicationDispatcher(Wrapper wrapper, String requestURI,
			String servletPath, String pathInfo, String queryString, String name) {
	...
	/**
	 * The Wrapper associated with the resource that will be forwarded to or
	 * included.
	 */
	...
	private void processRequest(ServletRequest request,
			ServletResponse response, State state) throws IOException,
			ServletException {

		DispatcherType disInt = (DispatcherType) request
				.getAttribute(Globals.DISPATCHER_TYPE_ATTR);
		if (disInt != null) {
	...
	public void dispatch(ServletRequest request, ServletResponse response)
		...
		if (Globals.IS_SECURITY_ENABLED) {
			try {
				PrivilegedDispatch dp = new PrivilegedDispatch(request,
						response);
				AccessController.doPrivileged(dp);
			} catch (PrivilegedActionException pe) {
				Exception e = pe.getException();
	 ...
	 * associated request, and create (or append to) the associated response.
	 * <p>
		...
		try {
			...
			wrapper.getLogger().error(
					sm.getString("applicationDispatcher.allocateException",
							wrapper.getName()), e);
			servletException = new ServletException(sm.getString(
					"applicationDispatcher.allocateException",
					wrapper.getName()), e);
			servlet = null;
		}
		...
		try {
			...
			wrapper.getLogger().error(
					sm.getString("applicationDispatcher.deallocateException",
							wrapper.getName()), e);
			servletException = new ServletException(sm.getString(
					"applicationDispatcher.deallocateException",
					wrapper.getName()), e);
		}
	...
	private void unwrapRequest(State state) {
		...
		if (state.outerRequest.isAsyncStarted()) {
			if (!state.outerRequest.getAsyncContext()
		...
		while (current != null) {
			...
			if (current == state.wrapRequest) {
				ServletRequest next = ((ServletRequestWrapper) current)
						.getRequest();
				if (previous == null)
	...
	private void unwrapResponse(State state) {
		...
		while (current != null) {
			...
			if (current == state.wrapResponse) {
				ServletResponse next = ((ServletResponseWrapper) current)
						.getResponse();
				if (previous == null)
	...
	private ServletRequest wrapRequest(State state) {
		...
		if ((current instanceof ApplicationHttpRequest)
			...
			if ((state.outerRequest instanceof ApplicationHttpRequest)
				...
				Object contextPath = houterRequest
	...
	private ServletResponse wrapResponse(State state) {
		...
		if ((current instanceof ApplicationHttpResponse)
				...
				|| (current instanceof HttpServletResponse))
			wrapper = new ApplicationHttpResponse(
					(HttpServletResponse) current, state.including);
		else
	...
	private void checkSameObjects(ServletRequest appRequest,
		...
		ServletRequest originalRequest = ApplicationFilterChain
				.getLastServicedRequest();
		ServletResponse originalResponse = ApplicationFilterChain
		...
		while (originalRequest instanceof ServletRequestWrapper
			...
			originalRequest = ((ServletRequestWrapper) originalRequest)
					.getRequest();
		}
		...
		while (!same) {
			...
			if (!same && dispatchedRequest instanceof ServletRequestWrapper) {
				dispatchedRequest = ((ServletRequestWrapper) dispatchedRequest)
						.getRequest();
			} else {
				break;
			}
		...
		if (!same) {
			throw new ServletException(
					sm.getString("applicationDispatcher.specViolation.request"));
		}
		...
		while (!same) {
			...
			if (!same && dispatchedResponse instanceof ServletResponseWrapper) {
				dispatchedResponse = ((ServletResponseWrapper) dispatchedResponse)
						.getResponse();
			} else {
				break;
			}
		...
		if (!same) {
			throw new ServletException(
					sm.getString("applicationDispatcher.specViolation.response"));
		}
	}
