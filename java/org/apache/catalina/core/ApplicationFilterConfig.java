...
public final class ApplicationFilterConfig implements FilterConfig,
	...
	private static final org.apache.juli.logging.Log log = LogFactory
			.getLog(ApplicationFilterConfig.class);
		...
		if (this.filter != null) {
			try {
				...
				context.getLogger().error(
						sm.getString("applicationFilterConfig.release",
			...
			if (!context.getIgnoreAnnotations()) {
				try {
					((StandardContext) context).getInstanceManager()
							.destroyInstance(this.filter);
				} catch (Exception e) {
					Throwable t = ExceptionUtils
							.unwrapInvocationTargetException(e);
					ExceptionUtils.handleThrowable(t);
					context.getLogger().error(
							"ApplicationFilterConfig.preDestroy", t);
				}
			}
		}
		this.filter = null;
