...
public class AsyncContextImpl implements AsyncContext, AsyncContextCallback {
	...
	public boolean timeout() {
		...
		if (result.get()) {
			...
			ClassLoader newCL = request.getContext().getLoader()
					.getClassLoader();
			try {
				Thread.currentThread().setContextClassLoader(newCL);
				List<AsyncListenerWrapper> listenersCopy = new ArrayList<AsyncListenerWrapper>();
	...
	public void dispatch(ServletContext context, String path) {
		...
		final RequestDispatcher requestDispatcher = context
	...
	public void setErrorState(Throwable t, boolean fireOnError) {
		...
		if (result.get()) {
			...
			request.getCoyoteRequest()
					.action(ActionCode.ASYNC_IS_ERROR, result);
			if (result.get()) {
	...
	private void logDebug(String method) {
		...
		if (request == null) {
			...
			org.apache.coyote.Request coyoteRequest = request
					.getCoyoteRequest();
		...
		String msg = String
				.format("Req: %1$8s  CReq: %2$8s  RP: %3$8s  Stage: %4$s  "
						+ "Thread: %5$20s  State: %6$20s  Method: %7$11s  URI: %8$s",
						rHashCode, crHashCode, rpHashCode, stage, threadName,
						"N/A", method, uri);
	...
	private InstanceManager getInstanceManager() {
		if (instanceManager == null) {
			if (context instanceof StandardContext) {
				instanceManager = ((StandardContext) context)
						.getInstanceManager();
			} else {
				instanceManager = new DefaultInstanceManager(null,
						new HashMap<String, Map<String, String>>(), context,
	...
	private void check() {
		if (request == null) {
			...
			throw new IllegalStateException(
					sm.getString("asyncContextImpl.requestEnded"));
		}
	}
