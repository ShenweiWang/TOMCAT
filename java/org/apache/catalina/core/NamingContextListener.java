...
public class NamingContextListener implements LifecycleListener,
	...
	private static final Log log = LogFactory
			.getLog(NamingContextListener.class);
	 ...
	 */
	protected HashMap<String, ObjectName> objectNames = new HashMap<String, ObjectName>();
	...
	public void lifecycleEvent(LifecycleEvent event) {
		...
		if (Lifecycle.CONFIGURE_START_EVENT.equals(event.getType())) {
			...
			try {
				...
				ContextBindings
						.bindContext(container, namingContext, container);
				...
				namingContext
						.setExceptionOnFailedWrite(getExceptionOnFailedWrite());
				...
				try {
					...
					logger.error(sm.getString(
							"naming.namingContextCreationFailed", e));
				}
				...
				if (container instanceof Server) {
					org.apache.naming.factory.ResourceLinkFactory
							.setGlobalContext(namingContext);
					try {
						ContextBindings.bindClassLoader(container, container,
					...
					if (container instanceof StandardServer) {
						((StandardServer) container)
								.setGlobalNamingContext(namingContext);
					}
				}
			...
			try {
				...
				ContextAccessController
						.unsetSecurityToken(getName(), container);
				ContextAccessController
						.unsetSecurityToken(container, container);
	...
	public void containerEvent(ContainerEvent event) {
		...
		if (type.equals("addEjb")) {
			...
			if (environmentName != null) {
				ContextEnvironment env = namingResources
						.findEnvironment(environmentName);
				addEnvironment(env);
			}
			...
			if (localEjbName != null) {
				ContextLocalEjb localEjb = namingResources
						.findLocalEjb(localEjbName);
			...
			if (resourceName != null) {
				ContextResource resource = namingResources
			...
			if (resourceEnvRefName != null) {
				ContextResourceEnvRef resourceEnvRef = namingResources
						.findResourceEnvRef(resourceEnvRefName);
			...
			if (serviceName != null) {
				ContextService service = namingResources
	...
	private void createNamingContext() throws NamingException {
		...
		// Resources Env
		ContextResourceEnvRef[] resourceEnvRefs = namingResources
				.findResourceEnvRefs();
		...
		if (container instanceof Context) {
			try {
				...
				ContextTransaction transaction = namingResources
						.getTransaction();
				if (transaction != null) {
					Iterator<String> params = transaction.listProperties();
					while (params.hasNext()) {
						String paramName = params.next();
						String paramValue = (String) transaction
	...
	protected ObjectName createObjectName(ContextResource resource)
		...
		if (container instanceof Server) {
			name = new ObjectName(domain + ":type=DataSource" + ",class="
			...
			name = new ObjectName(domain + ":type=DataSource" + ",context="
					+ contextName + ",host=" + host.getName() + ",class="
					+ resource.getType() + ",name=" + quotedResourceName);
		}
	...
	public void addEjb(ContextEjb ejb) {
		...
		Reference ref = new EjbRef(ejb.getType(), ejb.getHome(),
				ejb.getRemote(), ejb.getLink());
	...
	public void addService(ContextService service) {
		...
		if (service.getWsdlfile() != null) {
			...
			if (wsdlURL == null) {
				try {
					wsdlURL = ((Context) container).getServletContext()
		...
		Reference ref = new ServiceRef(service.getName(), service.getType(),
		...
		while (portcomponent.hasNext()) {
			...
			StringRefAddr refAddr = new StringRefAddr(
					ServiceRef.SERVICEENDPOINTINTERFACE, serviceendpoint);
			ref.add(refAddr);
		...
		while (handlers.hasNext()) {
			...
			while (params.hasNext()) {
				...
				handlerRef.add(new StringRefAddr(HandlerRef.HANDLER_PARAMNAME,
						paramName));
				handlerRef.add(new StringRefAddr(HandlerRef.HANDLER_PARAMVALUE,
						paramValue));
			}
		...
		try {
			if (logger.isDebugEnabled()) {
				logger.debug("  Adding service ref " + service.getName() + "  "
						+ ref);
			}
	...
	public void addResource(ContextResource resource) {
		...
		// Create a reference to the resource.
		Reference ref = new ResourceRef(resource.getType(),
				resource.getDescription(), resource.getScope(),
		...
		try {
			if (logger.isDebugEnabled()) {
				logger.debug("  Adding resource ref " + resource.getName()
						+ "  " + ref);
			}
	...
	public void addResourceLink(ContextResourceLink resourceLink) {
		...
		// Create a reference to the resource.
		Reference ref = new ResourceLinkRef(resourceLink.getType(),
				resourceLink.getGlobal(), resourceLink.getFactory(), null);
		...
		javax.naming.Context ctx = "UserTransaction".equals(resourceLink
				.getName()) ? compCtx : envCtx;
		try {
			if (logger.isDebugEnabled())
