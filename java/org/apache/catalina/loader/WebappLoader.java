...
public class WebappLoader extends LifecycleMBeanBase implements Loader,
	...
	protected void startInternal() throws LifecycleException {
		...
		try {
			...
			classLoader.setSearchExternalFirst(searchExternalFirst);
			if (container instanceof StandardContext) {
				classLoader.setAntiJARLocking(((StandardContext) container)
						.getAntiJARLocking());
				classLoader
						.setClearReferencesStatic(((StandardContext) container)
								.getClearReferencesStatic());
				classLoader
						.setClearReferencesStopThreads(((StandardContext) container)
								.getClearReferencesStopThreads());
				classLoader
						.setClearReferencesStopTimerThreads(((StandardContext) container)
								.getClearReferencesStopTimerThreads());
				classLoader
						.setClearReferencesHttpClientKeepAliveThread(((StandardContext) container)
			...
			ObjectName cloname = new ObjectName(MBeanUtils.getDomain(ctx)
	...
	public void propertyChange(PropertyChangeEvent event) {
		...
		if (event.getPropertyName().equals("reloadable")) {
			try {
				...
				log.error(sm.getString("webappLoader.reloadable", event
	...
	private void setClassPath() {
		...
		if (container instanceof StandardContext) {
			String baseClasspath = ((StandardContext) container)
