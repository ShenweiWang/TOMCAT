 ...
 *
 * See http://docs.oracle.com/javase/1.4.2/docs/guide/extensions/spec.html for a
 * detailed explanation of the extension mechanism in Java.
 *
 * @author Greg Murray
...
public final class ExtensionValidator {
	 ...
	 * <code>DirContext</code>. It locates Web Application MANIFEST.MF file in
	 * the /META-INF/ directory of the application and all MANIFEST.MF files in
	 * each JAR file located in the WEB-INF/lib directory and creates an
	 * <code>ArrayList</code> of
	 * <code>ManifestResorce<code> objects. These objects are then passed 
	 * to the validateManifestResources method for validation.
	 *
	 * @param dirContext
	 *            The JNDI root of the Web Application
	 * @param context
	 *            The context from which the Logger and path to the application
	 *
	 * @return true if all required extensions satisfied
	 */
	public static synchronized boolean validateApplication(
			DirContext dirContext, Context context) throws IOException {
		...
		try {
			NamingEnumeration<Binding> wne = dirContext
					.listBindings("/META-INF/");
			Binding binding = wne.nextElement();
			if (binding.getName().toUpperCase(Locale.ENGLISH)
					.equals("MANIFEST.MF")) {
				Resource resource = (Resource) dirContext.lookup("/META-INF/"
						+ binding.getName());
	 ...
	 * objects. This method requires an application name (which is the context
	 * root of the application at runtime).
	 *
	 * <code>false</false> is returned if the extension dependencies
	 * represented by any given <code>ManifestResource</code> objects is not
	 * met.
	 *
	 * This method should also provide static validation of a Web Application if
	 * provided with the necessary parameters.
	 *
	 * @param appName
	 *            The name of the Application that will appear in the error
	 *            messages
	 * @param resources
	 *            A list of <code>ManifestResource</code> objects to be
	 *            validated.
	 *
	 ...
	 * 
	 * NOTE: A list is built only if there is a dependency that needs to be
	 ...
	 *
	 * @param inStream
	 *            Input stream to a WAR or JAR file
		...
		if (extensionsDir != null) {
			...
			while (extensionsTok.hasMoreTokens()) {
				...
				for (int i = 0; i < files.length; i++) {
					if (files[i].getName().toLowerCase(Locale.ENGLISH)
							.endsWith(".jar")
							&& files[i].isFile()) {
						try {
