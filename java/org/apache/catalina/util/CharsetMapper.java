 ...
 * character set to be used for interpreting input text (or generating output
 * text) when the Content-Type header does not include one. You can customize
 * the behavior of this class by modifying the mapping data it loads, or by
 * subclassing it (to change the algorithm) and then using your own version for
...
public class CharsetMapper {
	 ...
	 *                if the specified properties resource could not be loaded
	 *                for any reason.
	 */
	...
	/**
	 * The deployment descriptor can have a locale-encoding-mapping-list element
	 * which describes the webapp's desired mapping from locale to charset. This
	 * method gets called when processing the web.xml file for a context
	 *
	 * @param locale
