...
/**
 * This class implements a local SAX's <code>EntityResolver</code>. All DTDs and
 * schemas used to validate the web.xml file will re-directed to a local file
 * stored in the servlet-api.jar and jsp-api.jar.
 *
...
public class SchemaResolver implements EntityResolver {
	...
	/**
	 * Create a new <code>EntityResolver</code> that will redirect all remote
	 * dtds and schema to a local destination.
	 * 
	 * @param digester
	 ...
	 * <code>parse()</code>.
	 *
	 * When adding a schema file (*.xsd), only the name of the file will get
	 * added. If two schemas with the same name are added, only the last one
	 * will be stored.
	 *
	 * @param publicId
	 ...
	 *
	 * @param publicId
	 *            The public identifier of the entity being referenced
	 * @param systemId
