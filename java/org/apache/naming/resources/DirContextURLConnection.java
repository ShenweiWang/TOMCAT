 ...
 * queries using the right names. Content handler is also not used; the content
...
public class DirContextURLConnection extends URLConnection {
	...
	/**
	 * Connect to the DirContext, and retrieve the bound object, as well as its
	 * attributes. If no object is bound with the name specified in the URL,
	...
	public long getLastModified() {
		...
		Attribute lastModified = attributes
				.get(ResourceAttributes.LAST_MODIFIED);
	...
	public Object getContent() throws IOException {
		...
		throw new FileNotFoundException(getURL() == null ? "null" : getURL()
				.toString());
	...
	public InputStream getInputStream() throws IOException {
		...
		if (resource == null) {
			throw new FileNotFoundException(getURL() == null ? "null"
					: getURL().toString());
		}
	...
	public Enumeration<String> list() throws IOException {
		...
		if (collection != null) {
			try {
				NamingEnumeration<NameClassPair> enumeration = collection
						.list("/");
				while (enumeration.hasMoreElements()) {
					NameClassPair ncp = enumeration.nextElement();
					String s = ncp.getName();
					result.addElement(URL_ENCODER.encodeURL(s, 0, s.length())
							.toString());
				}
			} catch (NamingException e) {
