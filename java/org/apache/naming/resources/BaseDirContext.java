...
public abstract class BaseDirContext implements DirContext {
	...
	public void addAlias(String path, BaseDirContext dirContext) {
		if (!path.startsWith("/")) {
			throw new IllegalArgumentException(sm.getString(
					"resources.invalidAliasPath", path));
		}
	...
	public String getAliases() {
		...
		Iterator<Entry<String, BaseDirContext>> iter = aliases.entrySet()
				.iterator();
		boolean first = true;
		while (iter.hasNext()) {
			if (first) {
	...
	public void setAliases(String theAliases) {
		...
		for (String kvp : kvps) {
			...
			if (kv.length != 2)
				throw new IllegalArgumentException(sm.getString(
						"resources.invalidAliasMapping", kvp));
			...
			if (kv[0].equals("/")) {
				throw new IllegalArgumentException(sm.getString(
						"resources.invalidAliasNotAllowed", kv[0]));
			}
			File aliasLoc = new File(kv[1]);
			if (!aliasLoc.exists()) {
				throw new IllegalArgumentException(sm.getString(
						"resources.invalidAliasNotExist", kv[1]));
			}
			...
			if (kv[1].endsWith(".war") && !(aliasLoc.isDirectory())) {
				...
				throw new IllegalArgumentException(sm.getString(
						"resources.invalidAliasFile", kv[1]));
			}
	...
	public String getRealPath(String name) {
		...
		for (DirContext altDirContext : altDirContexts) {
			if (altDirContext instanceof BaseDirContext) {
				path = ((BaseDirContext) altDirContext)
						.getRealPath("/META-INF/resources" + name);
				if (path != null)
					return path;
			}
		}
	...
	public NamingEnumeration<NameClassPair> list(String name)
		...
		for (DirContext altDirContext : altDirContexts) {
			if (altDirContext instanceof BaseDirContext) {
				altBindings = ((BaseDirContext) altDirContext)
						.doListBindings("/META-INF/resources" + name);
			}
			if (altBindings != null) {
				if (bindings == null) {
	...
	public final Attributes getAttributes(String name, String[] attrIds)
		...
		if (!aliases.isEmpty()) {
			AliasResult result = findAlias(name);
			if (result.dirContext != null) {
				return result.dirContext.getAttributes(result.aliasName,
						attrIds);
			}
		}
	 ...
	 */
	@Override
	public abstract void modifyAttributes(String name, int mod_op,
