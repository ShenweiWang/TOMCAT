...
public class NioSelectorPool {
	...
	protected static final boolean SHARED = Boolean.valueOf(
			System.getProperty("org.apache.tomcat.util.net.NioSelectorShared",
					"true")).booleanValue();
	...
	public void put(Selector s) throws IOException {
		...
		if (enabled)
			active.decrementAndGet();
		if (enabled
				&& (maxSpareSelectors == -1 || spare.get() < Math.min(
						maxSpareSelectors, maxSelectors))) {
	...
	/**
	 * Performs a blocking write using the bytebuffer for data to be written and
	 * a selector to block. If the <code>selector</code> parameter is null, then
	 * it will perform a busy write that could take up a lot of CPU cycles.
	 * 
	 * @param buf
	 *            ByteBuffer - the buffer containing the data, we will write as
	 *            long as <code>(buf.hasRemaining()==true)</code>
	 * @param socket
	 *            SocketChannel - the socket to write data to
	 * @param selector
	 *            Selector - the selector to use for blocking, if null then a
	 *            busy write will be initiated
	 * @param writeTimeout
	 *            long - the timeout for this write operation in milliseconds,
	 *            -1 means no timeout
	 ...
	 * selector to block. If the <code>selector</code> parameter is null, then
	 * it will perform a busy read that could take up a lot of CPU cycles.
	 * 
	 * @param buf
	 *            ByteBuffer - the buffer containing the data, we will read as
	 *            until we have read at least one byte or we timed out
	 * @param socket
	 *            SocketChannel - the socket to write data to
	 * @param selector
	 *            Selector - the selector to use for blocking, if null then a
	 *            busy read will be initiated
	 * @param readTimeout
	 *            long - the timeout for this read operation in milliseconds, -1
	 *            means no timeout
	 ...
	 *             if an IO Exception occurs in the underlying socket logic
	 */
	public int read(ByteBuffer buf, NioChannel socket, Selector selector,
	 ...
	 * to register for events should you have the block=true. If the
	 * <code>selector</code> parameter is null, then it will perform a busy read
	 * that could take up a lot of CPU cycles.
	 * 
	 * @param buf
	 *            ByteBuffer - the buffer containing the data, we will read as
	 *            until we have read at least one byte or we timed out
	 ...
	 *            means no timeout
	 * @param block
	 *            - true if you want to block until data becomes available or
	 ...
	 */
	public int read(ByteBuffer buf, NioChannel socket, Selector selector,
			long readTimeout, boolean block) throws IOException {
		if (SHARED && block) {
