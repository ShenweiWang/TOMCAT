...
public class JIoEndpoint extends AbstractEndpoint<Socket> {
	...
	protected class AsyncTimeout implements Runnable {
		...
		public void run() {
			...
			while (running) {
				...
				Iterator<SocketWrapper<Socket>> sockets = waitingRequests
						.iterator();
			...
			synchronized (socket) {
				try {
					...
					if (launch) {
						try {
							getExecutor().execute(
									new SocketProcessor(socket,
											SocketStatus.OPEN_READ));
						} catch (RejectedExecutionException x) {
							log.warn(
									"Socket reprocessing request was rejected for:"
											+ socket, x);
							try {
	...
	public void bind() throws Exception {
		...
		if (serverSocketFactory == null) {
			if (isSSLEnabled()) {
				serverSocketFactory = handler.getSslImplementation()
						.getServerSocketFactory(this);
			} else {
