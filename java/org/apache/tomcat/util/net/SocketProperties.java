 ...
 * All properties are prefixed with &quot;socket.&quot; and are currently only
...
public class SocketProperties {
	/**
	 * Enable/disable key cache, this bounded cache stores KeyAttachment objects
	 * to reduce GC Default is 500 -1 is unlimited 0 is disabled
	 */
	protected int keyCache = 500;

	/**
	 * Enable/disable socket processor cache, this bounded cache stores
	 * SocketProcessor objects to reduce GC Default is 500 -1 is unlimited 0 is
	 * disabled
	 */
	protected int processorCache = 500;

	/**
	 * Enable/disable poller event cache, this bounded cache stores PollerEvent
	 * objects to reduce GC for the poller Default is 500 -1 is unlimited 0 is
	 * disabled >0 the max number of objects to keep in cache.
	 */
	protected int eventCache = 500;

	/**
	 * Enable/disable direct buffers for the network buffers Default value is
	 * enabled
	 */
	protected boolean directBuffer = false;

	/**
	 * Socket receive buffer size in bytes (SO_RCVBUF). JVM default used if not
	 * set.
	 */
	protected Integer rxBufSize = null;

	/**
	 * Socket send buffer size in bytes (SO_SNDBUF). JVM default used if not
	 * set.
	 */
	...
	/**
	 * NioChannel pool size for the endpoint, this value is how many channels -1
	 * means unlimited cached, 0 means no cache Default value is 500
	 */
	protected int bufferPool = 500;

	/**
	 * Buffer pool size in bytes to be cached -1 means unlimited, 0 means no
	 * cache Default value is 100MB (1024*1024*100 bytes)
	 */
	...
	/**
	 * SO_LINGER option, paired with the <code>soLingerTime</code> value. JVM
	 * defaults used unless both attributes are set.
	 */
	protected Boolean soLingerOn = null;

	/**
	 * SO_LINGER option, paired with the <code>soLingerOn</code> value. JVM
	 * defaults used unless both attributes are set.
	 */
	 ...
	 * /api/java/net/Socket.html#setPerformancePreferences(int,%20int,%20int)
	...
	public void setPerformanceConnectionTime(int performanceConnectionTime) {
		this.performanceConnectionTime = Integer
				.valueOf(performanceConnectionTime);
	}
