...
public class NioBlockingSelector {
	...
	/**
	 * Performs a blocking write using the bytebuffer for data to be written If
	 * the <code>selector</code> parameter is null, then it will perform a busy
	 * write that could take up a lot of CPU cycles.
	 * 
	 * @param buf
		...
		try {
			while ((!timedout) && buf.hasRemaining()) {
				...
				try {
					...
					if (writeTimeout < 0) {
						att.awaitWriteLatch(Long.MAX_VALUE,
	...
	/**
	 * Performs a blocking read using the bytebuffer for data to be read If the
