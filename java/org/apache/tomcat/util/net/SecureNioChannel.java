...
public class SecureNioChannel extends NioChannel {
	...
	public int handshake(boolean read, boolean write) throws IOException {
		...
		while (!handshakeComplete) {
			...
			case NEED_UNWRAP: {
				...
				if (handshake.getStatus() == Status.OK) {
					...
					throw new IOException("Invalid handshake status:"
							+ handshakeStatus + " during handshake UNWRAP.");
				}// switch
				break;
			}
			case NEED_TASK: {
			...
			// return 0 if we are complete, otherwise reregister for any
			// activity that
			// would cause this method to be called again.
		return handshakeComplete ? 0
	...
	public void rehandshake(long timeout) throws IOException {
		...
		if (netInBuffer.position() > 0
			...
			throw new IOException(
					"Network input buffer still contains data. Handshake will fail.");
		if (netOutBuffer.position() > 0
				&& netOutBuffer.position() < netOutBuffer.limit())
			throw new IOException(
					"Network output buffer still contains data. Handshake will fail.");
		if (getBufHandler().getReadBuffer().position() > 0
				&& getBufHandler().getReadBuffer().position() < getBufHandler()
						.getReadBuffer().limit())
			throw new IOException(
					"Application input buffer still contains data. Data would have been lost.");
		if (getBufHandler().getWriteBuffer().position() > 0
				&& getBufHandler().getWriteBuffer().position() < getBufHandler()
						.getWriteBuffer().limit())
			throw new IOException(
					"Application output buffer still contains data. Data would have been lost.");
		reset();
	 ...
	 *             bufHandler.getReadBuffer() TODO Implement this
