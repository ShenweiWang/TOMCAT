 ...
 * passing the top object (child) as an argument. It is important to remember
 * that this rule acts on <code>end</code>.
 * </p>
 *
 * <p>
 * This rule now supports more flexible method matching by default. It is
 * possible that this may break (some) code written against release 1.1.1 or
 * earlier. See {@link #isExactMatch()} for more details.
 * </p>
 */
...
public class SetRootRule extends Rule {
	...
	/**
	 * Construct a "set root" rule with the specified method name. The method's
	 ...
	 *             {@link Digester#addRule} method. Use
	 *             {@link #SetRootRule(String methodName)} instead.
	 */
	@Deprecated
	 ...
	 *            Method name of the parent method to call
	 * @param paramType
	 *            Java class of the parent method's argument (if you wish to use
	 *            a primitive type, specify the corresponding Java wrapper class
	 *            instead, such as <code>java.lang.Boolean</code> for a
	 *            <code>boolean</code> parameter)
	 *
	 * @deprecated The digester instance is now set in the
	 *             {@link Digester#addRule} method. Use
	 *             {@link #SetRootRule(String methodName,String paramType)}
	 *             instead.
	 */
	@Deprecated
	 ...
	 * introspect the relevant objects so that the right method can be called.
	 * Originally, <code>MethodUtils.invokeExactMethod</code> was used. This
	 * matches methods very strictly and so may not find a matching method when
	 * one exists. This is still the behaviour when exact matching is enabled.
	 * </p>
	 *
	 * <p>
	 * When exact matching is disabled, <code>MethodUtils.invokeMethod</code> is
	 * used. This method finds more methods but is less precise when there are
	 * several methods with correct signatures. So, if you want to choose an
	 * exact signature you might need to enable this property.
	 * </p>
	 *
	 * <p>
		...
		if (digester.log.isDebugEnabled()) {
			if (parent == null) {
				digester.log.debug("[SetRootRule]{" + digester.match
						+ "} Call [NULL ROOT]." + methodName + "(" + child
						+ ")");
			} else {
				digester.log.debug("[SetRootRule]{" + digester.match
