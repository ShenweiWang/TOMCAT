 ...
 * new object which it pushes onto the object stack. When the element is
 * complete, the object will be popped.
 * </p>
 *
 * <p>
 * This rule is intended in situations where the element's attributes are needed
 * before the object can be created. A common scenario is for the
 * ObjectCreationFactory implementation to use the attributes as parameters in a
 * call to either a factory method or to a non-empty constructor.
 */
...
public class FactoryCreateRule extends Rule {
	...
	/**
	 * <p>
	 * Construct a factory create rule that will use the specified class name to
	 * create an {@link ObjectCreationFactory} which will then be used to create
	 * an object and push it on the stack.
	 * </p>
	 *
	 * <p>
	 ...
	 * (possibly overridden by the specified attribute if present) to create an
	 * {@link ObjectCreationFactory}, which will then be used to instantiate an
	 ...
	 * @param attributeName
	 *            Attribute name which, if present, contains an override of the
	 *            class name of the object creation factory to create.
	 */
	 ...
	 *            if true, exceptions thrown by the object creation factory will
	 *            be ignored.
	 */
	 ...
	 * required based on the attributes specified in the matched XML element.
	 */
	...
	public void begin(String namespace, String name, Attributes attributes)
		...
		if (ignoreCreateExceptions) {
			...
			try {
				Object instance = getFactory(attributes).createObject(
						attributes);
				...
				if (digester.log.isInfoEnabled()) {
					digester.log
							.info("[FactoryCreateRule] Create exception ignored: "
		...
		if (ignoreCreateExceptions && exceptionIgnoredStack != null
			...
			if ((exceptionIgnoredStack.pop()).booleanValue()) {
				...
				if (digester.log.isTraceEnabled()) {
					digester.log
							.trace("[FactoryCreateRule] No creation so no push so no pop");
				}
				return;
			}
		}
