...
/**
 * <p>
 * Default implementation of the <code>Rules</code> interface that supports the
 * standard rule matching behavior. This class can also be used as a base class
 * for specialized <code>Rules</code> implementations.
 * </p>
 *
 * <p>
...
public class RulesBase implements Rules {
	...
	/**
	 * The set of registered Rule instances, keyed by the matching pattern. Each
	 * value is a List containing the Rules for that pattern, in the order that
	...
	/**
	 * The namespace URI for which subsequently added <code>Rule</code> objects
	 * are relevant, or <code>null</code> for matching independent of
	 * namespaces.
	 */
	 ...
	 * than one Rule instance matches, they <strong>must</strong> be returned in
	 * the order originally registered through the <code>add()</code> method.
	 *
	 * @param namespaceURI
	 *            Namespace URI for which to select matching rules, or
	 *            <code>null</code> to match regardless of namespace URI
	 * @param pattern
	...
	/**
	 * Return a List of all registered Rule instances, or a zero-length List if
	 * there are no registered Rule instances. If more than one Rule instance
	 * has been registered, they <strong>must</strong> be returned in the order
	...
	/**
	 * Return a List of Rule instances for the specified pattern that also match
	 * the specified namespace URI (if any). If there are no such rules, return
	 * <code>null</code>.
	 *
	 * @param namespaceURI
	 *            Namespace URI to match, or <code>null</code> to select
	 *            matching rules regardless of namespace URI
	 * @param pattern
