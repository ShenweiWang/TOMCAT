 ...
 * <code>XmlMapper</code> class that was part of Tomcat 3.0 and 3.1, but is
 * organized somewhat differently.
 * </p>
 *
 * <p>
 * See the <a href="package-summary.html#package_description">Digester Developer
 * Guide</a> for more information.
 * </p>
 *
 * <p>
 * <strong>IMPLEMENTATION NOTE</strong> - A single Digester instance may only be
 * used within the context of a single thread at a time, and a call to
 * <code>parse()</code> must be completed before another can be initiated even
 * from the same thread.
 * </p>
 *
 * <p>
 * <strong>IMPLEMENTATION NOTE</strong> - A bug in Xerces 2.0.2 prevents the
 * support of XML schema. You need Xerces 2.1/2.3 and up to make this class
...
public class Digester extends DefaultHandler2 {
	...
	static {
		String className = System
				.getProperty("org.apache.tomcat.util.digester.PROPERTY_SOURCE");
		if (className != null) {
			...
			for (int i = 0; i < cls.length && (!initialized); i++) {
				try {
					...
					LogFactory.getLog(
							...
							"Unable to load property source[" + className
									+ "].", t);
				}
			}
			if (initialized)
	 ...
	 * James House (james@interobjective.com). This may help in places where you
	 * are able to load JAXP 1.1 classes yourself.
	 */
	 ...
	 * allows Digester to be used in environments which are unfriendly to
	 * JAXP1.1 (such as WebLogic 6.0). Note that if you use this option you have
	 * to configure namespace and validation support yourself, as these
	 ...
	 * objects as returned from Rules.getMatch(). As each xml element in the
	 * input is entered, the matching rules are pushed onto this stack. After
	 * the end tag is reached, the matches are popped again. The depth of is
	 * stack is therefore exactly the same as the current "nesting" level of the
	 * input xml.
	 *
	 * @since 1.6
	 */
	 ...
	 * ArrayStack of the namespace URIs this prefix has been mapped to -- the
	 * top Stack element is the most current one. (This architecture is required
	 * because documents can declare nested uses of the same prefix for
	 * different Namespace URIs).
	 */
	protected HashMap<String, ArrayStack<String>> namespaces = new HashMap<String, ArrayStack<String>>();
	...
	/**
	 * The "root" element of the stack (in other words, the last object that was
	 * popped.
	 */
	 ...
	 * established before the first rule is added, a default implementation will
	 * be provided.
	 */
	...
	protected Log log = LogFactory
			.getLog("org.apache.tomcat.util.digester.Digester");

	/**
	 * The Log to which all SAX event related logging calls will be made.
	 */
	protected Log saxLog = LogFactory
			.getLog("org.apache.tomcat.util.digester.Digester.sax");

	/** Stacks used for interrule communication, indexed by name String */
	private HashMap<String, ArrayStack<Object>> stacksByName = new HashMap<String, ArrayStack<Object>>();

	// ------------------------------------------------------------- Properties

	/**
	 * Return the currently mapped namespace URI for the specified prefix, if
	 * any; otherwise return <code>null</code>. These mappings come and go
	 ...
	 * the underlying implementation of <code>org.xml.sax.XMLReader</code>. See
	 * <a href=
	 * "http://www.saxproject.org/apidoc/xml/sax/package-summary.html#package-description"
	 * http://www.saxproject.org/apidoc/xml/sax/package-summary.html#package-
	 * description</a> for information about the standard SAX2 feature flags.
	 *
	 * @param feature
	 ...
	 * "http://www.saxproject.org/apidoc/xml/sax/package-summary.html#package-description"
	 * http://www.saxproject.org/apidoc/xml/sax/package-summary.html#package-
	 * description</a> for information about the standard SAX2 feature flags. In
	 * order to be effective, this method must be called <strong>before</strong>
	 * the <code>getParser()</code> method is called for the first time, either
	 ...
	 *            Namespace URI that must match on all subsequently added rules,
	 *            or <code>null</code> for matching regardless of the current
	 *            namespace URI
	 */
	 ...
	 * <code>XMLReader</code> implementation. See <a href=
	 * "http://www.saxproject.org/apidoc/xml/sax/package-summary.html#package-description"
	 * http://www.saxproject.org/apidoc/xml/sax/package-summary.html#package-
	 * description</a> for information about the standard SAX2 properties.
	 *
	 * @param property
	 ...
	 *
	 * FIX ME: there is a bug in JAXP/XERCES that prevent the use of a parser
	...
	/**
	 * Process notification of ignorable whitespace received from the body of an
	 * XML element.
	 *
	 * @param buffer
	 *            The characters from the XML document
	...
	/**
	 * Set the <code>EntityResolver</code> used by SAX when resolving public id
	 * and system id. This must be called before the first call to
	 * <code>parse()</code>.
	 * 
	 * @param entityResolver
	 *            a class that implement the <code>EntityResolver</code>
	 *            interface.
	 */
			...
			if (systemId == null) {
				...
				if (baseURI != null) {
					try {
						...
						if (!uri.isAbsolute()) {
							entityURL = new URI(baseURI).resolve(uri)
									.toString();
						}
	...
	public void error(SAXParseException exception) throws SAXException {
		...
		log.error(
				"Parse Error at line " + exception.getLineNumber() + " column "
	...
	public void fatalError(SAXParseException exception) throws SAXException {
		...
		log.error(
				"Parse Fatal Error at line " + exception.getLineNumber()
	...
	/**
	 * Forward notification of a parse warning to the application supplied error
	 * handler (if any).
	 *
	 * @param exception
	...
	/**
	 * Parse the content of the specified file using this Digester. Returns the
	 * root element from the object stack (if any).
	 *
	 * @param file
	...
	/**
	 * Parse the content of the specified reader using this Digester. Returns
	...
	/**
	 * Parse the content of the specified URI using this Digester. Returns the
	 * root element from the object stack (if any).
	 *
	 * @param uri
	...
	/**
	 * <p>
	 * Register the specified DTD URL for the specified public identifier. This
	 * must be called before the first call to <code>parse()</code>.
	 * </p>
	 * <p>
	 * <code>Digester</code> contains an internal <code>EntityResolver</code>
	 * implementation. This maps <code>PUBLICID</code>'s to URLs (from which the
	 * resource will be loaded). A common use case for this method is to
	 * register local URLs (possibly computed at runtime by a classloader) for
	 * DTDs. This allows the performance advantage of using a local version
	 * without having to ensure every <code>SYSTEM</code> URI on every processed
	 * xml document is local. This implementation provides only basic
	 * functionality. If more sophisticated features are required, using
	 * {@link #setEntityResolver} to set a custom resolver is recommended.
	 * </p>
	 * <p>
	 ...
	 * <code>Digester</code> property on the rule.
	 * </p>
	 *
	 * @param pattern
	 ...
	 *            Method name to be called
	 * @param paramCount
	 *            Number of expected parameters (or zero for a single parameter
	...
	/**
	 * Add an "call method" rule for the specified parameters. If
	 * <code>paramCount</code> is set to zero the rule will use the body of the
	 * matched element as the single argument of the method, unless
	 ...
	 *            from the body of this element)
	 * @param paramTypes
	 *            Set of Java class names for the types of the expected
	 *            parameters (if you wish to use a primitive type, specify the
	 *            corresponding Java wrapper class instead, such as
	 *            <code>java.lang.Boolean</code> for a <code>boolean</code>
	 *            parameter)
	 * @see CallMethodRule
	 */
	public void addCallMethod(String pattern, String methodName,
	 ...
	 *            from the body of this element)
	 * @param paramTypes
	 *            The Java class names of the arguments (if you wish to use a
	 *            primitive type, specify the corresponding Java wrapper class
	 *            instead, such as <code>java.lang.Boolean</code> for a
	 ...
	 *            Element matching pattern
	 * @param paramIndex
	 *            Zero-relative parameter index to set (from the body of this
	 *            element)
	 * @see CallParamRule
	 */
	 ...
	 *            Element matching pattern
	 * @param paramIndex
	 *            Zero-relative parameter index to set (from the specified
	 *            attribute)
	 * @param attributeName
	...
	/**
	 * Add a "call parameter" rule. This will either take a parameter from the
	 * stack or from the current element body text.
	 *
	 * @param paramIndex
	...
	/**
	 * Add a "call parameter" rule that sets a parameter from the stack. This
	 * takes a parameter from the given position on the stack.
	 *
	 * @param paramIndex
	 *            The zero-relative parameter number
	 * @param stackIndex
	 *            set the call parameter to the stackIndex'th object down the
	 *            stack, where 0 is the top of the stack, 1 the next element
	 *            down and so on
	 * @see CallMethodRule
	 */
	 ...
	 * <code>Digester</code> matching path. This is sometimes useful when using
	 * rules that support wildcards.
	 *
	 * @param pattern
	...
	/**
	 * Add a "call parameter" rule that sets a parameter from a caller-provided
	 * object. This can be used to pass constants such as strings to methods; it
	 * can also be used to pass mutable objects, providing ways for objects to
	 * do things like "register" themselves with some shared object.
	 * <p>
	 * Note that when attempting to locate a matching method to invoke, the true
	 * type of the paramObj is used, so that despite the paramObj being passed
	 * in here as type Object, the target method can declare its parameters as
	 * being the true type of the object (or some ancestor type, according to
	...
	/**
	 * Add a "factory create" rule for the specified parameters. Exceptions
	 * thrown during the object creation process will be propagated.
	 *
	 * @param pattern
	 *            Element matching pattern
	 * @param className
	 ...
	 *            Java class name of the object creation factory class
	 * @param attributeName
	 *            Attribute name which, if present, overrides the value
	 ...
	 *            when <code>true</code> any exceptions thrown during object
	...
	public void addFactoryCreate(String pattern, String className,
		...
		addRule(pattern, new FactoryCreateRule(className,
	...
	public void addFactoryCreate(String pattern, String className,
		...
		addRule(pattern, new FactoryCreateRule(className, attributeName,
				ignoreCreateExceptions));
	...
	public void addFactoryCreate(String pattern, Class<?> clazz,
		...
		addRule(pattern, new FactoryCreateRule(clazz, attributeName,
				ignoreCreateExceptions));
	...
	public void addFactoryCreate(String pattern,
		...
		addRule(pattern, new FactoryCreateRule(creationFactory,
				ignoreCreateExceptions));
	 ...
	 * @param attributeName
	 *            Attribute name that optionally overrides the default Java
	 ...
	 * @param clazz
	 *            Default Java class to be created the default Java class name
	 *            to be created
	 * @see ObjectCreateRule
	 */
	public void addObjectCreate(String pattern, String attributeName,
			Class<?> clazz) {
	 ...
	 *            Method name to call on the parent element
	 * @param paramType
	 *            Java class name of the expected parameter type (if you wish to
	 *            use a primitive type, specify the corresponding Java wrapper
	 ...
	 *            Element matching pattern
	 * @param name
	 *            Attribute name containing the property name to be set
	 * @param value
	...
	/**
	 * Pushes the given object onto the stack with the given name. If no stack
	 * already exists with the given name then one will be created.
	 * 
	 * @param stackName
	...
	/**
	 * <p>
	 * Gets the top object from the stack with the given name. This method does
	 * not remove the object from the stack.
	 * </p>
	 * <p>
	 * <strong>Note:</strong> a stack is considered empty if no objects have
	 * been pushed onto it yet.
	 ...
	 * allows you to access the root object that has been created after parsing.
	 * 
	 ...
	 * instance. The default implementation does nothing, but subclasses can
	 * override as needed. Digester (by default) only calls this method once.
	 * </p>
	 *
	 * <p>
	 * <strong>Note</strong> This method will be called by {@link #configure}
	 * only when the {@link #configured} flag is false. Subclasses that override
	 * <code>configure</code> or who set <code>configured</code> may find that
	 ...
	 * element and [getCount()-1] is the bottom element. If the specified index
	...
	/**
	 * Return a new StringBuilder containing the same contents as the input
	 * buffer, except that data of form ${varname} have been replaced by the
	 * value of that var as defined in the system property.
	 */
