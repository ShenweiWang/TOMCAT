...
public class StandardJarScanner implements JarScanner {
	 ...
	 * @param classloader
	 *            The classloader - used to access JARs not in WEB-INF/lib
	 * @param callback
	 *            The handler to process any JARs found
	 * @param jarsToSkip
	 *            List of JARs to ignore. If this list is null, a default list
	 *            will be read from the system property defined by
			...
			while (it.hasNext()) {
				...
				if (path.endsWith(Constants.JAR_EXT)
					...
					if (log.isDebugEnabled()) {
						log.debug(sm
								.getString("jarScan.webinflibJarScan", path));
					}
					URL url = null;
					try {
			...
			while (loader != null && loader != stopLoader) {
				if (loader instanceof URLClassLoader) {
					...
					for (int i = 0; i < urls.length; i++) {
						...
						if (jarName != null
								&& !(Matcher.matchName(ignoredJars, jarName) || urls[i]
										.toString()
										.contains(
				...
				if (urlStr.endsWith(Constants.JAR_EXT)) {
					...
					try {
						...
						if (f.isFile() && scanAllFiles) {
							...
							URL jarURL = new URL("jar:" + urlStr + "!/");
							callback.scan((JarURLConnection) jarURL
									.openConnection());
