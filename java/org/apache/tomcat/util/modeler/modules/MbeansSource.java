...
/**
 * This will create mbeans based on a config file. The format is an extended
 * version of MLET.
 *
 * Classloading. We don't support any explicit classloader tag. A ClassLoader is
 * just an mbean ( it can be the standard MLetMBean or a custom one ).
 * 
 * XXX add a special attribute to reference the loader mbean, XXX figure out how
 * to deal with private loaders
 *
...
public class MbeansSource extends ModelerSource implements MbeansSourceMBean {
	...
	private HashMap<ObjectName, Node> object2Node = new HashMap<ObjectName, Node>();
	...
	/**
	 * Return the list of mbeans created by this source. It can be used to
	...
	public void execute() throws Exception {
		...
		try {
			...
			if (!loaderLoaded) {
				...
				ObjectName defaultLoader = new ObjectName("modeler", "loader",
						"modeler");
				MLet mlet = new MLet(new URL[0], this.getClass()
						.getClassLoader());
			...
			for (Node mbeanN = firstMbeanN; mbeanN != null; mbeanN = DomUtil
					.getNext(mbeanN, null, Node.ELEMENT_NODE)) {
				...
				if ("mbean".equals(nodeName) || "MLET".equals(nodeName)) {
					...
					String objectName = DomUtil.getAttribute(mbeanN,
							"objectName");
					if (objectName == null) {
					...
					try {
						...
						if (!server.isRegistered(oname)) {
							...
							// XXX need to support
							// "StandardMBeanDescriptorsSource"
							String modelMBean = BaseModelMBean.class.getName();
							server.createMBean(modelMBean, oname, new Object[] {
									code, this },
					...
					for (Node descN = firstAttN; descN != null; descN = DomUtil
							.getNext(descN)) {
					...
					String operation = DomUtil
							.getAttribute(mbeanN, "operation");
	...
	public void save() {
		...
		// the triger will work for the first att, but all others will be
		// delayed
	...
	private void processAttribute(MBeanServer server, Node descN,
			String objectName) {
