 ...
 * {@link org.apache.tomcat.util.http.fileupload.FileItem}s associated with a
 * given HTML widget.
 * </p>
 *
 * <p>
...
public abstract class FileUploadBase {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				try {
					...
					throw new IOFileUploadException(String.format(
	...
	public Map<String, List<FileItem>> parseParameterMap(RequestContext ctx)
		...
		final Map<String, List<FileItem>> itemsMap = new HashMap<String, List<FileItem>>(
				items.size());
	...
	private String getFieldName(String pContentDisposition) {
		...
		if (pContentDisposition != null
				&& pContentDisposition.toLowerCase(Locale.ENGLISH).startsWith(
						FORM_DATA)) {
			ParameterParser parser = new ParameterParser();
			parser.setLowerCaseNames(true);
			// Parameter parser can handle null input
	...
	protected FileItemHeaders getParsedHeaders(String headerPart) {
		...
		for (;;) {
			...
			StringBuilder header = new StringBuilder(headerPart.substring(
					start, end));
			start = end + 2;
			while (start < len) {
				int nonWs = start;
				...
				if (requestSize != -1 && requestSize > sizeMax) {
					throw new SizeLimitExceededException(
							String.format(
									...
									Long.valueOf(sizeMax)), requestSize,
							sizeMax);
				}
			...
			for (;;) {
				...
				if (currentFieldName == null) {
					...
					if (fileName != null) {
						currentItem = new FileItemStreamImpl(fileName,
								currentFieldName,
								headers.getHeader(CONTENT_TYPE), false,
