...
public class Procattr {
	 ...
	 * This is NOT a required initializer function. This is useful if you have
	 * already opened a pipe (or multiple files) that you wish to use, perhaps
	 * persistently across multiple process invocations - such as a log file.
	 * You can save some extra function calls by not creating your own pipe
	 * since this creates one in the process space for you.
	 * 
	 * @param attr
	 *            The procattr we care about.
	 * @param in
	 ...
	 *            The procattr we care about.
	 * @param dir
	 *            Which dir to start in. By default, this is the same dir as the
	 *            parent currently resides in, when the createprocess call is
	 *            made.
	 */
	 ...
	 * failures to the caller of apr_proc_create(), rather than find out in the
	 * child.
	 * 
	 * @param attr
	 *            The procattr describing the child process to be created.
	 * @param chk
	 *            Flag to indicate whether or not extra work should be done to
	 *            try to report failures to the caller. <br />
	 *            This flag only affects apr_proc_create() on platforms where
	 *            fork() is used. This leads to extra overhead in the calling
	 *            process, but that may help the application handle such errors
	 *            more gracefully.
	 */
	 ...
	 *            The procattr we care about.
	 * @param addrspace
	 *            Should the child start in its own address space? Default is no
	 *            on NetWare and yes on other platforms.
	 */
	 ...
	 *            The the pool to use.
	 * @param o
	 *            The Object to call in the child process. <br />
	 *            At the present time, it will only be called from
	 *            apr_proc_create() on platforms where fork() is used. It will
	 *            never be called on other platforms, on those platforms
	 *            apr_proc_create() will return the error in the parent process
	 *            rather than invoke the callback in the now-forked child
	 *            process.
	 */
	 ...
	 *            The username used
	 * @param password
	 *            User password if needed. Password is needed on WIN32 or any
	 *            other platform having APR_PROCATTR_USER_SET_REQUIRES_PASSWORD
	 *            set.
	 */
