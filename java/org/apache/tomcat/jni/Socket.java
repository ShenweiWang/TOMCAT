...
public class Socket {
	...
	/**
	 * Set on non-blocking sockets (timeout != 0) on which the previous read()
	 * did not fill a buffer completely. the next apr_socket_recv() will first
	 * call select()/poll() rather than going straight into read(). (Can also be
	 * set by an application to force a select()/poll() call before the next
	 * read, in cases where the app expects that an immediate read would fail.)
	 */
	...
	/*
	 * TODO: Missing: APR_INET APR_UNSPEC APR_INET6
	 */
	 ...
	 * @param cont
	 *            The parent pool to use
	 * @return The new socket that has been set up.
	 */
	public static native long create(int family, int type, int protocol,
			long cont) throws Exception;

	/**
	 * Shutdown either reading, writing, or both sides of a socket. <br />
	 * This does not actually close the socket descriptor, it just controls
	 * which calls are still valid on the socket.
	 * 
	 * @param thesocket
	 *            The socket to close
	 * @param how
	 ...
	 *            The socket to bind
	 * @param sa
	 *            The socket address to bind to This may be where we will find
	 *            out if there is any other process using the selected port.
	 */
	 ...
	 * @return A copy of the socket that is connected to the socket that made
	 *         the connection request. This is the socket which should be used
	 ...
	 * @param sock
	 *            The socket to query
	 * @return True if socket is at the OOB/urgent mark, otherwise return false.
	 */
	 ...
	 *            The socket to send the data over.
	 * @param buf
	 *            The Byte buffer which contains the data to be sent.
	 * @param offset
	 *            The offset within the buffer array of the first buffer from
	 *            which bytes are to be retrieved; must be non-negative and no
	 *            larger than buf.length
	 * @param len
	 *            The maximum number of buffers to be accessed; must be
	 *            non-negative and no larger than buf.length - offset
	 * @return The number of bytes send.
	 *
	 */
	public static native int sendb(long sock, ByteBuffer buf, int offset,
			int len);
	 ...
	 *
	 */
	public static native int sendib(long sock, ByteBuffer buf, int offset,
			int len);
	...
	/**
	 * Send data over a network using internally set ByteBuffer without internal
	 * retry.
	 */
	 ...
	 *            The socket to send the data over.
	 * @param vec
	 *            The array from which to get the data to send.
	 *
	 */
	 ...
	 *            The number of bytes to read (-1) for full array.
	 * @param timeout
	 *            The socket timeout in microseconds.
	 * @return the number of bytes received.
	 */
	 ...
	 * @return the number of bytes received.
	 */
	public static native int recvb(long sock, ByteBuffer buf, int offset,
			int nbytes);
	 ...
	 * @return the number of bytes received.
	 */
	public static native int recvbt(long sock, ByteBuffer buf, int offset,
			int nbytes, long timeout);

	/**
	 * Read data from a network with timeout using internally set ByteBuffer
	 */
	public static native int recvbbt(long sock, int offset, int nbytes,
			long timeout);
	...
	/**
	 * Send a file from an open file descriptor to a socket, along with optional
	 * headers and trailers. <br />
	 * This functions acts like a blocking write by default. To change this
	 * behavior, use apr_socket_timeout_set() or the APR_SO_NONBLOCK socket
	 * option. The number of bytes actually sent is stored in the len parameter.
	 * The offset parameter is passed by reference for no reason; its value will
	 * never be modified by the apr_socket_sendfile() function.
	 * 
	 * @param sock
	 ...
	 * @param headers
	 *            Array containing the headers to send
	 * @param trailers
	 *            Array containing the trailers to send
	 * @param offset
	 ...
	 * @return Number of bytes actually sent, including headers, file, and
	 *         trailers
	 *
	 */
	public static native long sendfile(long sock, long file, byte[][] headers,
			byte[][] trailers, long offset, long len, int flags);
	...
	/**
	 * Set internal send ByteBuffer. This function will preset internal Java
	 * ByteBuffer for consecutive sendbb calls.
	 * 
	 * @param sock
	 *            The socket to use
	 * @param buf
	...
	/**
	 * Set internal receive ByteBuffer. This function will preset internal Java
	 * ByteBuffer for consecutive revcvbb/recvbbt calls.
	 * 
	 * @param sock
	 *            The socket to use
	 * @param buf
