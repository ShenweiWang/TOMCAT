...
public class Proc {
	...
	/**
	 * a restart is occurring, perform any necessary cleanup (including sending
	 * a special signal to child)
	 */
	public static final int APR_OC_REASON_RESTART = 2;
	/**
	 * unregister has been called, do whatever is necessary (including kill the
	 * child)
	 */
	...
	/**
	 * a health check is occurring, for most maintenance functions this is a
	 * no-op.
	 */
	...
	/**
	 * This is currently the only non-portable call in APR. This executes a
	 * standard unix fork.
	 * 
	 * @param proc
	 ...
	 * @param cont
	 *            The pool to use.
	 * @return APR_INCHILD for the child, and APR_INPARENT for the parent or an
	 *         error.
	 */
	public static native int fork(long[] proc, long cont);

	/**
	 * Create a new process and execute a new program within that process. This
	 * function returns without waiting for the new process to terminate; use
	 * apr_proc_wait for that.
	 * 
	 * @param progname
	 *            The program to run
	 * @param args
	 *            The arguments to pass to the new program. The first one should
	 *            be the program name.
	 * @param env
	 *            The new environment table for the new process. This should be
	 *            a list of NULL-terminated strings. This argument is ignored
	 *            for APR_PROGRAM_ENV, APR_PROGRAM_PATH, and APR_SHELLCMD_ENV
	 *            types of commands.
	 * @param attr
	 *            The procattr we should use to determine how to create the new
	 *            process
	 * @param pool
	 *            The pool to use.
	 * @return The resulting process handle.
	 */
	public static native int create(long proc, String progname, String[] args,
			String[] env, long attr, long pool);

	/**
	 * Wait for a child process to die
	 * 
	 * @param proc
	 *            The process handle that corresponds to the desired child
	 *            process
	 * @param exit
	 *            exit[0] The returned exit status of the child, if a child
	 *            process dies, or the signal that caused the child to die. On
	 *            platforms that don't support obtaining this information, the
	 *            status parameter will be returned as APR_ENOTIMPL. exit[1] Why
	 *            the child died, the bitwise or of:
	 * 
	 *            <PRE>
	 ...
	 * </PRE>
	 * @return The childs status is in the return code to this process. It is
	 *         one of:
	 * 
	 *         <PRE>
	...
	/**
	 * Wait for any current child process to die and return information about
	 * that child.
	 * 
	 * @param proc
	 *            Pointer to NULL on entry, will be filled out with child's
	 *            information
	 * @param exit
	 *            exit[0] The returned exit status of the child, if a child
	 *            process dies, or the signal that caused the child to die. On
	 ...
	 */
	public static native int waitAllProcs(long proc, int[] exit, int waithow,
			long pool);
	 ...
	 * 
	 * @param daemonize
	 *            set to non-zero if the process should daemonize and become a
	 *            background process, else it will stay in the foreground.
	 */
