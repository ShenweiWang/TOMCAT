...
public class Pool {
	 ...
	 * 
	 * @param parent
	 *            The parent pool. If this is 0, the new pool is a root pool. If
	 *            it is non-zero, the new pool will inherit all of its parent
	 *            pool's attributes, except the apr_pool_t will be a sub-pool.
	 ...
	 * 
	 * @param pool
	 *            The pool to clear This does not actually free the memory, it
	 *            just allows the pool to re-use this memory for the next
	 *            allocation.
	 */
	 ...
	 * @return True if a is an ancestor of b, NULL is considered an ancestor of
	 *         all pools.
	 */
	 ...
	 * Cleanup
	 * 
	 * Cleanups are performed in the reverse order they were registered. That
	 * is: Last In, First Out. A cleanup function can safely allocate memory
	 * from the pool that is being cleaned up. It can also safely register
	 * additional cleanups which will be run LIFO, directly after the current
	 * cleanup terminates. Cleanups have to take caution in calling functions
	 * that create subpools. Subpools, created during cleanup will NOT
	 * automatically be cleaned up. In other words, cleanups are to clean up
	 * after themselves.
	 */
	 ...
	 * 
	 * @param pool
	 *            The pool register the cleanup with
	 * @param o
	 *            The object to call when the pool is cleared or destroyed
	 ...
	 * 
	 * @param pool
	 *            The pool remove the cleanup from
	 * @param data
	 ...
	 * @param pool
	 *            The current pool <br />
	 *            <b>Warning :</b> The data to be attached to the pool should
	 *            have a life span at least as long as the pool it is being
	 *            attached to. Object attached to the pool will be globally
	 *            referenced until the pool is cleared or dataSet is called with
	 *            the null data.
