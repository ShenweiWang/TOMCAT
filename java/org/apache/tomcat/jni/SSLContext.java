...
public final class SSLContext {
	 ...
	 * </PRE>
	 * 
	 * Callback can use that word to determine application logging level by
	 * intercepting <b>write</b> call. If the <b>bio</b> is set to 0 no error
	 * messages will be displayed. Default is to use the stderr output stream.
	 * 
	 * @param ctx
	 *            Server or Client context to use.
	 * @param bio
	...
	/**
	 * Sets the "quiet shutdown" flag for <b>ctx</b> to be <b>mode</b>. SSL
	 * objects created from <b>ctx</b> inherit the <b>mode</b> valid at the time
	 * and may be 0 or 1. <br />
	 * Normally when a SSL connection is finished, the parties must send out
	 * "close notify" alert messages using L<SSL_shutdown(3)|SSL_shutdown(3)>
	 * for a clean shutdown. <br />
	 * When setting the "quiet shutdown" flag to 1, <b>SSL.shutdown</b> will set
	 ...
	 * to be shutdown, but no "close notify" alert is sent to the peer. This
	 * behaviour violates the TLS standard. The default is normal shutdown
	 * behaviour as described by the TLS standard.
	 * 
	 * @param ctx
	 *            Server or Client context to use.
	 * @param mode
	 ...
	 * consisting of OpenSSL cipher specifications to configure the Cipher Suite
	 * the client is permitted to negotiate in the SSL handshake phase. Notice
	 * that this directive can be used both in per-server and per-directory
	 * context. In per-server context it applies to the standard SSL handshake
	 * when a connection is established. In per-directory context it forces a
	 * SSL renegotiation with the reconfigured Cipher Suite after the HTTP
	...
	/**
	 * Set File of concatenated PEM-encoded CA CRLs or directory of PEM-encoded
	 * CA Certificates for Client Auth <br />
	 ...
	 * files, in order of preference. <br />
	 ...
	 * files there: you also have to create symbolic links named hash-value.N.
	 * And you should always make sure this directory contains the appropriate
	 * symbolic links. Use the Makefile which comes with mod_ssl to accomplish
	 * this task.
	 * 
	 * @param ctx
	 *            Server or Client context to use.
	 ...
	 * the certificates of Certification Authorities (CA) which form the
	 ...
	 * PEM-encoded CA Certificate files, usually in certificate chain order. <br />
	 * But be careful: Providing the certificate chain works only if you are
	 * using a single (either RSA or DSA) based server certificate. If you are
	 * using a coupled RSA+DSA certificate pair, this will work only if actually
	 * both certificates use the same certificate chain. Else the browsers will
	 * be confused in this situation.
	 * 
	 * @param ctx
	 *            Server or Client context to use.
	 * @param file
	 ...
	 * @param skipfirst
	 *            Skip first certificate if chain file is inside certificate
	 *            file.
	 */
	...
	/**
	 * Set Certificate <br />
	 * Point setCertificateFile at a PEM encoded certificate. If the certificate
	 * is encrypted, then you will be prompted for a pass phrase. Note that a
	 * kill -HUP will prompt again. A test certificate can be generated with
	 * `make certificate' under built time. Keep in mind that if you've both a
	 * RSA and a DSA certificate you can configure both in parallel (to also
	 * allow the use of DSA ciphers, etc.) <br />
	 * If the key is not combined with the certificate, use key param to point
	 * at the key file. Keep in mind that if you've both a RSA and a DSA private
	 * key you can configure both in parallel (to also allow the use of DSA
	 * ciphers, etc.)
	 * 
	 * @param ctx
	 *            Server or Client context to use.
	 ...
	 *            Certificate file.
	 * @param key
	 *            Private Key file to use if not in cert.
	 * @param password
	 *            Certificate password. If null and certificate is encrypted,
	...
	public static native boolean setCertificate(long ctx, String cert,
			String key, String password, int idx) throws Exception;
	 ...
	 * preference. This can be used alternatively and/or additionally to path. <br />
	 * The files in this directory have to be PEM-encoded and are accessed
	 * through hash filenames. So usually you can't just place the Certificate
	 * files there: you also have to create symbolic links named hash-value.N.
	 * And you should always make sure this directory contains the appropriate
	 ...
	 * Certificates in Client Certificate verification. <br />
	 * This directive sets the Certificate verification level for the Client
	 * Authentication. Notice that this directive can be used both in per-server
	 * and per-directory context. In per-server context it applies to the client
	 * authentication process used in the standard SSL handshake when a
	 * connection is established. In per-directory context it forces a SSL
	 * renegotiation with the reconfigured client verification level after the
	 * HTTP request was read but before the HTTP response is sent. <br />
	 * The following levels are available for level:
	 * 
	 * <PRE>
	 ...
	 * under <code>setCACertificatePath</code>), etc.
	 * 
	 * @param ctx
	 *            Server or Client context to use.
	 * @param level
