...
public class File {
	...
	/**
	 * Platform dependent support for higher level locked read/write access to
	 * support writes across process/machines
	 */
	 ...
	 * <b>Warning :</b> The APR_LARGEFILE flag only has effect on some platforms
	 * where sizeof(apr_off_t) == 4. Where implemented, it allows opening and
	 * writing to a file which exceeds the size which can be represented by
	 * apr_off_t (2 gigabytes). When a file's size does exceed 2Gb,
	 * apr_file_info_get() will fail with an error on the descriptor, likewise
	 * apr_stat()/apr_lstat() will fail on the filename. apr_dir_read() will
	 * fail with APR_INCOMPLETE on a directory entry for a large file depending
	 * on the particular APR_FINFO_* flags. Generally, it is not recommended to
	 * use this flag.
	 */
	...
	/**
	 * Shared lock. More than one process or thread can hold a shared lock at
	 * any given time. Essentially, this is a "read lock", preventing writers
	 * from establishing an exclusive lock.
	 */
	public static final int APR_FLOCK_SHARED = 1;

	/**
	 * Exclusive lock. Only one process may hold an exclusive lock at any given
	 * time. This is analogous to a "write lock".
	 */
	...
	/*
	 * apr_filetype_e values for the filetype member of the apr_file_info_t
	 * structure <br /><b>Warning :</b>: Not all of the filetypes below can be
	 * determined. For example, a given platform might not correctly report a
	 * socket descriptor as APR_SOCK if that type isn't well-identified on that
	 * platform. In such cases where a filetype exists but cannot be described
	 * by the recognized flags below, the filetype will be APR_UNKFILE. If the
	 ...
	 * @param pool
	 *            The pool to use. If perm is APR_OS_DEFAULT and the file is
	 *            being created, appropriate default permissions will be used.
	 ...
	 * @param flags
	 *            The flags to open the file with. If this is zero, the file is
	 *            opened with APR_CREATE | APR_READ | APR_WRITE | APR_EXCL |
	 *            APR_DELONCLOSE
	 * @param pool
	 *            The pool to allocate the file out of.
	 * @return The apr file to use as a temporary file.
	 *
	 *         This function generates a unique temporary file name from
	 ...
	 *            The full path to the file (using / on all systems)
	 * @param pool
	 *            The pool to use. If the file is open, it won't be removed
	 ...
	 * <b>Warning :</b> If a file exists at the new location, then it will be
	...
	/**
	 * Copy the specified file to another file. The new file does not need to
	 * exist, it will be created if required. <br />
	 * <b>Warning :</b> If the new file already exists, its contents will be
	 * overwritten.
	 * 
	 * @param fromPath
	 *            The full path to the original file (using / on all systems)
	 ...
	 *            The full path to the new file (using / on all systems)
	 * @param perms
	 *            Access permissions for the new file if it is created. In place
	 *            of the usual or'd combination of file permissions, the value
	 *            APR_FILE_SOURCE_PERMS may be given, in which case the source
	...
	/**
	 * Append the specified file to another file. The new file does not need to
	 * exist, it will be created if required.
	 * 
	 * @param fromPath
	 ...
	 *            In place of the usual or'd combination of file permissions,
	 *            the value APR_FILE_SOURCE_PERMS may be given, in which case
	 ...
	 *
	 * Write will write up to the specified number of bytes, but never more. If
	 * the OS cannot write that many bytes, it will write as many as it can. The
	 * third argument is modified to reflect the * number of bytes written.
	 *
	 ...
	 */
	public static native int write(long thefile, byte[] buf, int offset,
			int nbytes);
	 ...
	 * @return The number of bytes written.
	 */
	public static native int writeb(long thefile, ByteBuffer buf, int offset,
			int nbytes);
	 ...
	 *
	 * Write will write up to the specified number of bytes, but never more. If
	 * the OS cannot write that many bytes, the process/thread will block until
	 * they can be written. Exceptional error such as "out of space" or
	 * "pipe closed" will terminate with an error.
	 *
	 * It is possible for both bytes to be written and an error to be returned.
	 * And if *bytes_written is less than nbytes, an accompanying error is
	 * _always_ returned.
	 *
	 * APR_EINTR is never returned.
	 * 
	 ...
	 *            The file descriptor to write to.
	 * @param buf
	 *            The direct ByteBuffer which contains the data.
	 * @param offset
	 *            Start offset in buf
	 * @param nbytes
	 *            The number of bytes to write.
	 * @return The number of bytes written.
	 */
	public static native int writeFullb(long thefile, ByteBuffer buf,
	...
	/**
	 * Write data from array of byte arrays to the specified file, ensuring that
	 * all of the data is written before returning.
	 *
	 * writevFull is available even if the underlying operating system doesn't
	 * provide writev().
	 * 
	 * @param thefile
	 *            The file descriptor to write to.
	 ...
	 *
	 * apr_file_read will read up to the specified number of bytes, but never
	 * more. If there isn't enough data to fill that number of bytes, all of the
	 * available data is read. The third argument is modified to reflect the
	 * number of bytes read. If a char was put back into the stream via ungetc,
	 * it will be the first character returned.
	 *
	 * It is not possible for both bytes to be read and an APR_EOF or other
	 * error to be returned. APR_EINTR is never returned.
	 * 
	 * @param thefile
	 ...
	 */
	public static native int read(long thefile, byte[] buf, int offset,
			int nbytes);
	 ...
	 *            The file descriptor to read from.
	 * @param buf
	 *            The direct Byte buffer to store the data to.
	 * @param offset
	 *            Start offset in buf
	 * @param nbytes
	 ...
	 * @return the number of bytes read.
	 */
	public static native int readb(long thefile, ByteBuffer buf, int offset,
			int nbytes);
	 ...
	 * before returning.
	 *
	 * Read will read up to the specified number of bytes, but never more. If
	 * there isn't enough data to fill that number of bytes, then the
	 * process/thread will block until it is available or EOF is reached. If a
	 * char was put back into the stream via ungetc, it will be the first
	 * character returned.
	 *
	 * It is possible for both bytes to be read and an error to be returned. And
	 * if *bytes_read is less than nbytes, an accompanying error is _always_
	 * returned.
	 *
	 * APR_EINTR is never returned.
	 * 
	 ...
	 *            The file descriptor to read from.
	 * @param buf
	 *            The direct ByteBuffer to store the data to.
	 * @param offset
	 *            Start offset in buf
	 * @param nbytes
	 *            The number of bytes to read.
	 * @return the number of bytes read.
	 */
	public static native int readFullb(long thefile, ByteBuffer buf,
			int offset, int nbytes);

	/**
	 * Read a string from the specified file. The buffer will be NUL-terminated
	 * if any characters are stored.
	 * 
	 * @param buf
	 ...
	 * @return Returns APR_EOF if we are at the end of file, APR_SUCCESS
	 *         otherwise.
	 */
	 ...
	 * permissions are specified which could not be set. <br />
	...
	/**
	 * Set attributes of the specified file. This function should be used in
	 * preference to explicit manipulation of the file permissions, because the
	 * operations to provide these attributes are platform specific and may
	 * involve more than simply setting permission bits. <br />
	 * <b>Warning :</b> Platforms which do not implement this feature will
	 * return APR_ENOTIMPL.
	 * 
	 * @param fname
	...
	/**
	 * Establish a lock on the specified, open file. The lock may be advisory or
	 * mandatory, at the discretion of the platform. The lock applies to the
	 * file as a whole, rather than a specific range. Locks are established on a
	 * per-thread/process basis; a second lock by the same thread will not
	 * block.
	 * 
	 * @param thefile
	 *            The file to lock.
	...
	/**
	 * Retrieve the flags that were passed into apr_file_open() when the file
	 * was opened.
	 * 
	 * @param file
	 ...
	 * 
	 * @param io
	 *            io[0] The file descriptors to use as input to the pipe. io[1]
	 *            The file descriptor to use as output from the pipe.
	 * @param pool
	 ...
	 * 
	 * @param newFile
	 *            The file to duplicate. newFile must point to a valid
	 *            apr_file_t, or point to NULL.
	 * @param oldFile
	 ...
	 * 
	 * @param newFile
	 *            The old file that is to be closed and reused. newFile MUST
	 *            point at a valid apr_file_t. It cannot be NULL.
	 * @param oldFile
	 *            The file to duplicate.
	 * @param pool
	 *            The pool to use for the new file.
	 ...
	 * 
	 * @param finfo
	 *            Where to store the information about the file, which is never
	 *            touched if the call fails.
	 * @param fname
	 *            The name of the file to stat.
	 * @param wanted
	 *            The desired apr_finfo_t fields, as a bit flag of APR_FINFO_
	 *            values
	 * @param pool
