...
public class PojoMessageHandlerWholeBinary extends
	...
	public PojoMessageHandlerWholeBinary(Object pojo, Method method,
		...
		if (maxMessageSize > -1
				&& maxMessageSize > session.getMaxBinaryMessageBufferSize()) {
			if (maxMessageSize > Integer.MAX_VALUE) {
				throw new IllegalArgumentException(
						sm.getString("pojoMessageHandlerWhole.maxBufferSize"));
			}
		...
		try {
			if (decoderClazzes != null) {
				for (Class<? extends Decoder> decoderClazz : decoderClazzes) {
					...
					} else if (BinaryStream.class
							.isAssignableFrom(decoderClazz)) {
			...
			if (decoder instanceof Binary) {
				...
				try {
					...
					throw new DecodeException(
							message,
							sm.getString("pojoMessageHandlerWhole.decodeIoFail"),
							ioe);
				}
			}
		}
