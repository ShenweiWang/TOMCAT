...
/**
 * For a POJO class annotated with {@link javax.websocket.server.ServerEndpoint}
 * , an instance of this class creates and caches the method handler, method
 * information and parameter information for the onXXX calls.
 */
public class PojoMethodMapping {
	...
	private static PojoPathParam[] getPathParams(Method m, MethodType methodType)
		...
		for (int i = 0; i < types.length; i++) {
			...
			if (type.equals(Session.class)) {
				...
				for (Annotation paramAnnotation : paramAnnotations) {
					if (paramAnnotation.annotationType()
							.equals(PathParam.class)) {
						...
						try {
							...
							throw new DeploymentException(
									sm.getString("pojoMethodMapping.invalidPathParamType"),
									iae);
						}
				...
				if (result[i] == null) {
					throw new DeploymentException(sm.getString(
							"pojoMethodMapping.paramWithoutAnnotation", type,
		...
		if (methodType == MethodType.ON_ERROR && !foundThrowable) {
			throw new DeploymentException(sm.getString(
					"pojoMethodMapping.onErrorNoThrowable", m.getName(), m
			...
			if (type.equals(Session.class)) {
				...
				try {
					...
					throw new DecodeException(value, sm.getString(
							"pojoMethodMapping.decodePathParamFail", value,
							type), e);
				}
			}
		}
	...
	private static class MessageHandlerInfo {
		...
		public MessageHandlerInfo(Method m, List<DecoderEntry> decoderEntries) {
			...
			if (indexPong != -1) {
				if (indexPayload != -1) {
					throw new IllegalArgumentException(sm.getString(
							"pojoMethodMapping.pongWithPayload", m.getName(), m
									.getDeclaringClass().getName()));
				} else {
			...
			if (indexPayload == -1) {
				throw new IllegalArgumentException(sm.getString(
						"pojoMethodMapping.noPayload", m.getName(), m
								.getDeclaringClass().getName()));
			}
			if (indexPong != -1 && indexBoolean != -1) {
				throw new IllegalArgumentException(sm.getString(
						"pojoMethodMapping.partialPong", m.getName(), m
								.getDeclaringClass().getName()));
			}
			if (indexReader != -1 && indexBoolean != -1) {
				throw new IllegalArgumentException(sm.getString(
						"pojoMethodMapping.partialReader", m.getName(), m
								.getDeclaringClass().getName()));
			}
			if (indexInputStream != -1 && indexBoolean != -1) {
				throw new IllegalArgumentException(sm.getString(
						"pojoMethodMapping.partialInputStream", m.getName(), m
								.getDeclaringClass().getName()));
			}
			if (decoderMatch != null && decoderMatch.hasMatches()
					&& indexBoolean != -1) {
				throw new IllegalArgumentException(sm.getString(
						"pojoMethodMapping.partialObject", m.getName(), m
								.getDeclaringClass().getName()));
			}
		...
		public Set<MessageHandler> getMessageHandlers(Object pojo,
			...
			for (Map.Entry<Integer, PojoPathParam> entry : indexPathParams
					.entrySet()) {
			...
			if (indexBoolean == -1) {
				...
				if (indexString != -1 || indexPrimitive != -1) {
					MessageHandler mh = new PojoMessageHandlerWholeText(pojo,
							m, session, config, null, params, indexPayload,
							false, indexSession, maxMessageSize);
					results.add(mh);
				} else if (indexReader != -1) {
					MessageHandler mh = new PojoMessageHandlerWholeText(pojo,
							m, session, config, null, params, indexReader,
							true, indexSession, maxMessageSize);
					results.add(mh);
					...
					MessageHandler mh = new PojoMessageHandlerWholePong(pojo,
							m, session, params, indexPong, false, indexSession);
					results.add(mh);
				}
			} else {
				// ASync
