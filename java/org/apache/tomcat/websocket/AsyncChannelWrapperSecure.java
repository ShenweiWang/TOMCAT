...
public class AsyncChannelWrapperSecure implements AsyncChannelWrapper {
	...
	private static final Log log = LogFactory
			.getLog(AsyncChannelWrapperSecure.class);
	...
	public <B, A extends B> void read(ByteBuffer dst, A attachment,
		...
		WrapperFuture<Integer, B> future = new WrapperFuture<Integer, B>(
				handler, attachment);
	...
	public Future<Integer> write(ByteBuffer src) {
		...
		WriteTask writeTask = new WriteTask(new ByteBuffer[] { src }, 0, 1,
				inner);
	...
	@Override
	public <B, A extends B> void write(ByteBuffer[] srcs, int offset,
			int length, long timeout, TimeUnit unit, A attachment,
			CompletionHandler<Long, B> handler) {

		WrapperFuture<Long, B> future = new WrapperFuture<Long, B>(handler,
				attachment);
			...
			try {
				for (int i = offset; i < offset + length; i++) {
					...
					while (src.hasRemaining()) {
						...
						while (toWrite > 0) {
							Future<Integer> f = socketChannel
									.write(socketWriteBuffer);
		...
		public void run() {
			...
			try {
				while (read == 0) {
					...
					if (forceRead) {
						Future<Integer> f = socketChannel
								.read(socketReadBuffer);
						Integer socketRead = f.get();
						if (socketRead.intValue() == -1) {
							throw new EOFException(
									sm.getString("asyncChannelWrapperSecure.eof"));
						}
					}
			...
			try {
				...
				while (handshaking) {
					...
					case NEED_WRAP: {
						...
						checkResult(r, true);
						socketWriteBuffer.flip();
						Future<Integer> fWrite = socketChannel
								.write(socketWriteBuffer);
						fWrite.get();
						break;
					}
					case NEED_UNWRAP: {
						...
						if (socketReadBuffer.position() == 0
							...
							Future<Integer> fRead = socketChannel
									.read(socketReadBuffer);
							fRead.get();
						}
						socketReadBuffer.flip();
