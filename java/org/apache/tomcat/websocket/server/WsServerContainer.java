...
public class WsServerContainer extends WsWebSocketContainer implements
	...
	private static final CloseReason AUTHENTICATED_HTTP_SESSION_CLOSED = new CloseReason(
	...
	private final ConcurrentHashMap<Integer, SortedSet<TemplatePathMatch>> configTemplateMatchMap = new ConcurrentHashMap<Integer, SortedSet<TemplatePathMatch>>();
	private volatile boolean enforceNoAddAfterHandshake = org.apache.tomcat.websocket.Constants.STRICT_SPEC_COMPLIANCE;
	private volatile boolean addAllowed = true;
	private final ConcurrentHashMap<String, Set<WsSession>> authenticatedSessions = new ConcurrentHashMap<String, Set<WsSession>>();
	...
	WsServerContainer(ServletContext servletContext) {
		...
		String value = servletContext
				.getInitParameter(Constants.BINARY_BUFFER_SIZE_SERVLET_CONTEXT_INIT_PARAM);
		if (value != null) {
			setDefaultMaxBinaryMessageBufferSize(Integer.parseInt(value));
		}

		value = servletContext
				.getInitParameter(Constants.TEXT_BUFFER_SIZE_SERVLET_CONTEXT_INIT_PARAM);
		if (value != null) {
			setDefaultMaxTextMessageBufferSize(Integer.parseInt(value));
		}

		value = servletContext
				.getInitParameter(Constants.ENFORCE_NO_ADD_AFTER_HANDSHAKE_CONTEXT_INIT_PARAM);
		if (value != null) {
		...
		value = servletContext
				.getInitParameter(Constants.EXECUTOR_CORE_SIZE_INIT_PARAM);
		if (value != null) {
			executorCoreSize = Integer.parseInt(value);
		}
		value = servletContext
				.getInitParameter(Constants.EXECUTOR_KEEPALIVETIME_SECONDS_INIT_PARAM);
		if (value != null) {
	...
	public void addEndpoint(ServerEndpointConfig sec)
		...
		if (uriTemplate.hasParameters()) {
			...
			SortedSet<TemplatePathMatch> templateMatches = configTemplateMatchMap
					.get(key);
			...
			if (!templateMatches.add(new TemplatePathMatch(sec, uriTemplate))) {
				...
				throw new DeploymentException(sm.getString(
						"serverContainer.duplicatePaths", path));
			}
		} else {
			// Exact match
	...
	/**
	 * Provides the equivalent of {@link #addEndpoint(ServerEndpointConfig)} for
	...
	public void addEndpoint(Class<?> pojo) throws DeploymentException {
		...
		if (annotation == null) {
			throw new DeploymentException(sm.getString(
					"serverContainer.missingAnnotation", pojo.getName()));
		}
		...
		Class<? extends Configurator> configuratorClazz = annotation
				.configurator();
		...
		if (!configuratorClazz.equals(Configurator.class)) {
			try {
				...
				throw new DeploymentException(sm.getString(
						"serverContainer.configuratorFail", annotation
								.configurator().getName(), pojo.getClass()
								.getName()), e);
		...
		sec.getUserProperties().put(PojoEndpointServer.POJO_METHOD_MAPPING_KEY,
				methodMapping);
	...
	public WsMappingResult findMapping(String path) {
		...
		if (!PojoEndpointServer.class.isAssignableFrom(sec.getEndpointClass())) {
			...
			sec.getUserProperties().put(PojoEndpointServer.POJO_PATH_PARAM_KEY,
					pathParams);
		}
	...
	private void registerAuthenticatedSession(WsSession wsSession,
		...
		if (wsSessions == null) {
			wsSessions = Collections
					.newSetFromMap(new ConcurrentHashMap<WsSession, Boolean>());
	 ...
	 * instance.
	 */
	private static class TemplatePathMatchComparator implements
			Comparator<TemplatePathMatch> {
		...
		public int compare(TemplatePathMatch tpm1, TemplatePathMatch tpm2) {
			return tpm1.getUriTemplate().getNormalizedPath()
