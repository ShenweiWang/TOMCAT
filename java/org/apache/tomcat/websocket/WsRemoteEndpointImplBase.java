...
public abstract class WsRemoteEndpointImplBase implements RemoteEndpoint {
	...
	void sendPartialString(CharBuffer part, boolean last) throws IOException {
		try {
			...
			TextMessageSendHandler tmsh = new TextMessageSendHandler(f2sh,
					part, last, encoder, encoderBuffer, this);
			tmsh.write();
			if (timeout == -1) {
				f2sh.get();
			} else {
		...
		synchronized (messagePartLock) {
			if (Constants.OPCODE_CLOSE == mp.getOpCode()) {
				try {
					...
					log.warn(
							sm.getString("wsRemoteEndpoint.flushOnCloseFailed"),
							e);
				}
			}
	...
	protected void setEncoders(EndpointConfig endpointConfig)
		...
		for (Class<? extends Encoder> encoderClazz : endpointConfig
				.getEncoders()) {
			Encoder instance;
			try {
		...
		public void write() {
			...
			if (mask == null) {
				...
				for (int i = 0; i < toWrite; i++) {
					outputBuffer
							.put((byte) (payload.get() ^ (mask[maskIndex++] & 0xFF)));
					if (maskIndex > 3) {
						maskIndex = 0;
					}
				}
	...
	private static enum State {
		OPEN, STREAM_WRITING, WRITER_WRITING, BINARY_PARTIAL_WRITING, BINARY_PARTIAL_READY, BINARY_FULL_WRITING, TEXT_PARTIAL_WRITING, TEXT_PARTIAL_READY, TEXT_FULL_WRITING
	}
	...
	private static class StateMachine {
		...
		public synchronized void complete(boolean last) {
			if (last) {
				checkState(State.TEXT_PARTIAL_WRITING, State.TEXT_FULL_WRITING,
						State.BINARY_PARTIAL_WRITING,
						State.BINARY_FULL_WRITING, State.STREAM_WRITING,
						State.WRITER_WRITING);
				state = State.OPEN;
			} else {
				checkState(State.TEXT_PARTIAL_WRITING,
						State.BINARY_PARTIAL_WRITING, State.STREAM_WRITING,
						State.WRITER_WRITING);
		...
		private void checkState(State... required) {
			...
			throw new IllegalStateException(sm.getString(
					"wsRemoteEndpoint.wrongState", this.state));
		}
	}
