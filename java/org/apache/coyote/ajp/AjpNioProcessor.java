...
public class AjpNioProcessor extends AbstractAjpProcessor<NioChannel> {
	...
	protected void actionInternal(ActionCode actionCode, Object param) {
		...
		case ASYNC_COMPLETE: {
			if (asyncStateMachine.asyncComplete()) {
				((NioEndpoint) endpoint).processSocket(
						this.socketWrapper.getSocket(), SocketStatus.OPEN_READ,
						false);
			}
			break;
		}
		case ASYNC_SETTIMEOUT: {
			...
			final KeyAttachment ka = (KeyAttachment) socketWrapper.getSocket()
					.getAttachment(false);
	...
	protected void resetTimeouts() {
		...
		// finished.
		final KeyAttachment attach = (KeyAttachment) socketWrapper.getSocket()
				.getAttachment(false);
	...
	protected void output(byte[] src, int offset, int length)
		...
		KeyAttachment att = (KeyAttachment) socketWrapper.getSocket()
				.getAttachment(false);
		if (att == null)
	...
	private int readSocket(byte[] buf, int pos, int n, boolean block)
		...
		if (block) {
			...
			try {
				NioEndpoint.KeyAttachment att = (NioEndpoint.KeyAttachment) socketWrapper
	...
	/**
	 * Receive a chunk of data. Called to implement the 'special' packet in
	 * ajp13 and to receive the data after we send a GET_BODY packet
	 */
	@Override
