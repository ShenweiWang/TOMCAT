...
public class AjpAprProcessor extends AbstractAjpProcessor<Long> {
	...
	protected boolean read(int n) throws IOException {
		...
		if (inputBuffer.capacity() - inputBuffer.limit() <= n
		...
		while (inputBuffer.remaining() < n) {
			nRead = Socket.recvbb(socketWrapper.getSocket().longValue(),
					inputBuffer.limit(),
			...
			if (nRead > 0) {
				...
				if ((-nRead) == Status.ETIMEDOUT || (-nRead) == Status.TIMEUP) {
					...
					throw new IOException(
							sm.getString("ajpprocessor.failedread"));
				}
			}
		}
	 ...
	 *
	 * @param first
	 *            is true if the message is the first in the request, which will
	 *            cause a short duration blocking read
