...
public class Http11NioProcessor extends AbstractHttp11Processor<NioChannel> {
	...
	public SocketState event(SocketStatus status) throws IOException {
		...
		final NioEndpoint.KeyAttachment attach = (NioEndpoint.KeyAttachment) socketWrapper
	...
	protected boolean breakKeepAliveLoop(SocketWrapper<NioChannel> socketWrapper) {
		...
		if (sendfileData != null && !getErrorState().isError()) {
			...
			SelectionKey key = socketWrapper
					.getSocket()
					.getIOChannel()
					.keyFor(socketWrapper.getSocket().getPoller().getSelector());
			// do the first write on this thread, might as well
			if (socketWrapper.getSocket().getPoller()
					.processSendfile(key, (KeyAttachment) socketWrapper, true)) {
	...
	public void actionInternal(ActionCode actionCode, Object param) {
		...
		case REQ_HOST_ADDR_ATTRIBUTE: {
			...
			if ((remoteAddr == null) && (socketWrapper != null)) {
				InetAddress inetAddr = socketWrapper.getSocket().getIOChannel()
		...
		case REQ_LOCAL_ADDR_ATTRIBUTE: {
			if (localAddr == null) {
				localAddr = socketWrapper.getSocket().getIOChannel().socket()
		...
		case REQ_REMOTEPORT_ATTRIBUTE: {
			if ((remotePort == -1) && (socketWrapper != null)) {
				remotePort = socketWrapper.getSocket().getIOChannel().socket()
						.getPort();
			}
		...
		case REQ_LOCALPORT_ATTRIBUTE: {
			if ((localPort == -1) && (socketWrapper != null)) {
				localPort = socketWrapper.getSocket().getIOChannel().socket()
						.getLocalPort();
			}
		...
		case REQ_SSL_CERTIFICATE: {
			if (sslSupport != null) {
				...
				inputBuffer
						.addActiveFilter(inputFilters[Constants.BUFFERED_FILTER]);
				SecureNioChannel sslChannel = (SecureNioChannel) socketWrapper
						.getSocket();
				...
				if (!engine.getNeedClientAuth()) {
					...
					try {
						...
						sslSupport = ((NioEndpoint) endpoint).getHandler()
								...
								.getSSLSupport(engine.getSession());
		...
		case COMET_CLOSE: {
			...
			NioEndpoint.KeyAttachment attach = (NioEndpoint.KeyAttachment) socketWrapper
					.getSocket().getAttachment(false);
			...
			if (rp.getStage() != org.apache.coyote.Constants.STAGE_SERVICE) {
				...
				socketWrapper.getSocket().getPoller()
						.add(socketWrapper.getSocket());
			}
			break;
		}
		...
		case ASYNC_COMPLETE: {
			if (asyncStateMachine.asyncComplete()) {
				((NioEndpoint) endpoint)
						.processSocket(socketWrapper.getSocket(),
	...
	protected boolean prepareSendfile(OutputFilter[] outputFilters) {
		...
		if (fileName != null) {
			...
			sendfileData.pos = ((Long) request
					.getAttribute(org.apache.coyote.Constants.SENDFILE_FILE_START_ATTR))
					.longValue();
			sendfileData.length = ((Long) request
					.getAttribute(org.apache.coyote.Constants.SENDFILE_FILE_END_ATTR))
					.longValue()
					- sendfileData.pos;
			return true;
		}
