...
public class Http11Processor extends AbstractHttp11Processor<Socket> {
	...
	protected void setRequestLineReadTimeout() throws IOException {
		...
		/*
		 * When there is no data in the buffer and this is not the first request
		 * on this connection and timeouts are being used the first read for
		 * this request may need a different timeout to take account of time
		 * spent waiting for a processing thread.
		 * 
		 * This is a little hacky but better than exposing the socket and the
		 * timeout info to the InputBuffer
		 */
	...
	public void actionInternal(ActionCode actionCode, Object param) {
		...
		case REQ_HOST_ADDR_ATTRIBUTE: {
			if ((remoteAddr == null) && (socketWrapper != null)) {
				InetAddress inetAddr = socketWrapper.getSocket()
						.getInetAddress();
				if (inetAddr != null) {
		...
		case REQ_LOCAL_ADDR_ATTRIBUTE: {
			if (localAddr == null)
				localAddr = socketWrapper.getSocket().getLocalAddress()
						.getHostAddress();
