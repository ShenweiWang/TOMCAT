...
public abstract class AbstractHttp11Processor<S> extends AbstractProcessor<S> {
	...
	private boolean isCompressable() {
		...
		MessageBytes contentEncodingMB = response.getMimeHeaders().getValue(
				"Content-Encoding");
	...
	private boolean useCompression() {
		...
		MessageBytes acceptEncodingMB = request.getMimeHeaders().getValue(
				"accept-encoding");
		...
		if (noCompressionUserAgents != null) {
			MessageBytes userAgentValueMB = request.getMimeHeaders().getValue(
					"user-agent");
	...
	public final void action(ActionCode actionCode, Object param) {
		...
		case REQ_SET_BODY_REPLAY: {
			...
			AbstractInputBuffer<S> internalBuffer = (AbstractInputBuffer<S>) request
					.getInputBuffer();
		...
		while (!getErrorState().isError() && keepAlive && !comet && !isAsync()
			...
			try {
				...
				if (endpoint.isPaused()) {
					...
					// JMX
					request.getMimeHeaders().setLimit(
							endpoint.getMaxHeaderCount());
	...
	private void prepareResponse() {
		...
		if (contentLength != -1) {
			...
			if (entityBody && http11 && !connectionClosePresent) {
				...
				contentDelimitation = true;
				headers.addValue(Constants.TRANSFERENCODING).setString(
						Constants.CHUNKED);
			} else {
				getOutputBuffer().addActiveFilter(
						outputFilters[Constants.IDENTITY_FILTER]);
