...
public class Http11AprProtocol extends AbstractHttp11Protocol<Long> {
	...
	protected static class Http11ConnectionHandler extends
			AbstractConnectionHandler<Long, Http11AprProcessor> implements
			Handler {
		...
		// Inbound/Outbound based upgrade
		@Override
		protected void longPoll(SocketWrapper<Long> socket,
			...
			if (processor.isAsync()) {
				...
				if (proto.endpoint.isRunning()) {
					...
					((AprEndpoint) proto.endpoint).processSocket(socket
							.getSocket().longValue(), SocketStatus.STOP);
				}
				...
				((AprEndpoint) proto.endpoint).getPoller().add(
						socket.getSocket().longValue(),
						processor.getUpgradeInbound().getReadTimeout(), true,
						false);
			}
		}
