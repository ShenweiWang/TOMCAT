...
/**
 * Chunked input filter. Parses chunked data according to <a
 * href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec3.html#sec3.6.1"
 * >http://www.w3.org/Protocols/rfc2616/rfc2616-sec3.html#sec3.6.1</a><br>
 * 
...
public class ChunkedInputFilter implements InputFilter {
	...
	private static final StringManager sm = StringManager
			.getManager(ChunkedInputFilter.class.getPackage().getName());
	...
	public int doRead(ByteChunk chunk, Request req) throws IOException {
		...
		if (remaining <= 0) {
			if (!parseChunkHeader()) {
				throwIOException(sm
						.getString("chunkedInputFilter.invalidHeader"));
			}
			if (endChunk) {
				parseEndChunk();
				return -1;
		...
		while (!eol) {
			...
			if (buf[pos] == Constants.CR || buf[pos] == Constants.LF) {
				...
				if (maxExtensionSize > -1 && extensionSize > maxExtensionSize) {
					throwIOException(sm
							.getString("chunkedInputFilter.maxExtension"));
				}
			}
	...
	protected void parseCRLF(boolean tolerant) throws IOException {
		...
		while (!eol) {
			if (pos >= lastValid) {
				if (readBytes() <= 0) {
					throwIOException(sm
							.getString("chunkedInputFilter.invalidCrlfNoData"));
				}
			}
			...
			if (buf[pos] == Constants.CR) {
				if (crfound) {
					throwIOException(sm
							.getString("chunkedInputFilter.invalidCrlfCRCR"));
				}
				crfound = true;
			} else if (buf[pos] == Constants.LF) {
				if (!tolerant && !crfound) {
					throwIOException(sm
							.getString("chunkedInputFilter.invalidCrlfNoCR"));
				}
				eol = true;
			} else {
	...
	private boolean parseHeader() throws IOException {
		...
		while (!colon) {
			...
			if (pos >= lastValid) {
				if (readBytes() < 0) {
					throwEOFException(sm
							.getString("chunkedInputFilter.eosTrailer"));
				}
			}
			...
			while (space) {
				...
				if ((chr == Constants.SP) || (chr == Constants.HT)) {
					...
					if (trailingHeaders.getEnd() > newlimit) {
						throwIOException(sm
								.getString("chunkedInputFilter.maxTrailer"));
					}
