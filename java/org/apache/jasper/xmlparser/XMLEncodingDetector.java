...
public class XMLEncodingDetector {
	...
	public int scanLiteral(int quote, XMLString content) throws IOException {
		...
		while (fCurrentEntity.position < fCurrentEntity.count) {
			c = fCurrentEntity.ch[fCurrentEntity.position++];
			if ((c == quote && (!fCurrentEntity.literal || external))
		...
		if (XMLChar.isSpace(c)) {
			...
			do {
				...
				if (fCurrentEntity.position == fCurrentEntity.count) {
					...
					.isSpace(c = fCurrentEntity.ch[fCurrentEntity.position]));
			return true;
		}
	...
	final boolean load(int offset, boolean changeEntity) throws IOException {
		...
		// read characters
		int length = fCurrentEntity.mayReadChunks ? (fCurrentEntity.ch.length - offset)
	 ...
	 * invoked, but uses the underlying InputStream's read(char[], offset
