 ...
 * object added to the collection with an Entry type, that is returned to the
 * consumer. When removing an object from the list, the consumer provides this
 * Entry object.
 *
 ...
 *
 * Invalidation of Entry objects during removal from the list is done by setting
 * their "valid" field to false. All public methods which take Entry objects as
 * arguments are NOP if the entry is no longer valid.
 *
 ...
 *
 * Whenever the sort position needs to change, the consumer can remove the
 * object and reinsert it in front or at the end in constant time. So keeping
 * the list sorted is very cheap.
 *
 */
public class FastRemovalDequeue<T> {
	...
	/**
	 * Retrieve the size of the list. This method also needs to be externaly
	 * synchronized to ensure correct publication of changes.
	 *
	 ...
	 *
	 * Could also be implemented as remove() and push(), but explicitely coding
	 ...
	 *
	 * Could also be implemented as remove() and unpop(), but explicitely coding
	 * might be a bit faster.
	 *
	 * @param element
	...
	/**
	 * Implementation of a doubly linked list entry. All implementation details
	 * are private. For the consumer of the above collection, this is simply
