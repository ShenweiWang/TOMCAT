 ...
 * upon. If a dependent file changes the JSP page which included it is
 * recompiled.
 *
...
public final class JspRuntimeContext {
		...
		if (log.isDebugEnabled()) {
			if (loader != null) {
				log.debug(Localizer.getMessage(
						"jsp.message.parent_class_loader_is", loader.toString()));
			} else {
				log.debug(Localizer.getMessage(
	...
	public FastRemovalDequeue<JspServletWrapper>.Entry push(
		...
		if (replaced != null) {
			if (log.isDebugEnabled()) {
				log.debug(Localizer.getMessage(
						"jsp.message.jsp_removed_excess", replaced.getJspUri(),
						context.getContextPath()));
			}
	...
	private SecurityHolder initSecurity() {
		...
		if (policy != null) {
			try {
				...
				workDir = workDir + "-";
				permissions
						.add(new FilePermission(workDir, "read,write,delete"));
