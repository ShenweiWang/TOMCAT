 ...
 * application.
 * 
 * Tag Libraries can be defined globally in one of two ways: 1. Via <taglib>
 * elements in web.xml: the uri and location of the tag-library are specified in
 * the <taglib> element. 2. Via packaged jar files that contain .tld files
 * within the META-INF directory, or some subdirectory of it. The taglib is
 * 'global' if it has the <uri> element defined.
 *
...
public class TldLocationsCache {
	 ...
	 * returned as a String array: [0] The location [1] If the location is a jar
	 * file, this is the location of the tld.
	 */
	...
	private synchronized void init() throws JasperException {
		...
		try {
			...
			if (jarScanner != null) {
				jarScanner.scan(ctxt, Thread.currentThread()
		...
		try {
			...
			while (taglibs.hasNext()) {
				...
				if (tagLoc.endsWith(JAR_EXT)) {
					location = new TldLocation("META-INF/taglib.tld", ctxt
							.getResource(tagLoc).toString());
				} else {
	...
	/*
	 * Scans the web application's sub-directory identified by startPath, along
	 * with its sub-directories, for TLDs and adds an implicit map entry to the
	 * taglib map for any TLD that has a <uri> element.
	 * 
	 * Initially, rootPath equals /WEB-INF/. The /WEB-INF/classes and
	 * /WEB-INF/lib sub-directories are excluded from the search, as per the JSP
	 * 2.0 spec.
	 * 
		...
		if (dirList != null) {
			...
			while (it.hasNext()) {
				...
				if (!path.endsWith(TLD_EXT)
						&& (path.startsWith(WEB_INF_LIB) || path
								.startsWith("/WEB-INF/classes/"))) {
					continue;
				}
				if (path.endsWith(TLD_EXT)) {
			...
			for (int i = 0; i < fileList.length; i++) {
				...
				if (fileList[i].isDirectory()) {
					...
					try {
						...
						tldScanStream(fileList[i].toURI().toString(), null,
								stream);
					} finally {
						if (stream != null) {
							try {
	 ...
	 * 
	 * @param entryName If the resource is a JAR file, the name of the entry in
	 * the JAR file
	 * 
