...
public abstract class Compiler {
	...
	private final Log log = LogFactory.getLog(Compiler.class); // must not be
																// static
	...
	protected String[] generateJava() throws Exception {
		...
		if (jspProperty.isDeferedSyntaxAllowedAsLiteral() != null) {
			pageInfo.setDeferredSyntaxAllowedAsLiteral(JspUtil
					.booleanValue(jspProperty.isDeferedSyntaxAllowedAsLiteral()));
		}
		if (jspProperty.isTrimDirectiveWhitespaces() != null) {
			pageInfo.setTrimDirectiveWhitespaces(JspUtil
					.booleanValue(jspProperty.isTrimDirectiveWhitespaces()));
		}
		...
		if (jspProperty.isErrorOnUndeclaredNamespace() != null) {
			pageInfo.setErrorOnUndeclaredNamespace(JspUtil
					.booleanValue(jspProperty.isErrorOnUndeclaredNamespace()));
		}
		if (ctxt.isTagFile()) {
			try {
		...
		try {
			/*
			 * The setting of isELIgnored changes the behaviour of the parser in
			 * subtle ways. To add to the 'fun', isELIgnored can be set in any
			 * file that forms part of the translation unit so setting it in a
			 * file included towards the end of the translation unit can change
			 * how the parser should have behaved when parsing content up to the
			 * point where isELIgnored was set. Arghh! Previous attempts to hack
			 * around this have only provided partial solutions. We now use two
			 * passes to parse the translation unit. The first just parses the
			 * directives and the second parses the whole translation unit once
			 * we know how isELIgnored has been set. TODO There are some
			 * possible optimisations of this process.
			 */
			// Parse the file
			ParserController parserCtl = new ParserController(ctxt, this);

			// Pass 1 - the directives
			Node.Nodes directives = parserCtl
					.parseDirectives(ctxt.getJspFile());
	...
	public void removeGeneratedFiles() {
		...
		try {
			...
			log.warn(
					Localizer
							.getMessage("jsp.warning.compiler.classfile.delete.fail.unknown"),
					e);
		}
	}
