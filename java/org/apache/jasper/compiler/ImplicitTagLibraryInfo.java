		...
		if (dirList != null) {
			...
			while (it.hasNext()) {
				...
				if (path.endsWith(TAG_FILE_SUFFIX)
					 ...
					 */
					String suffix = path.endsWith(TAG_FILE_SUFFIX) ? TAG_FILE_SUFFIX
							: TAGX_FILE_SUFFIX;
						...
						if (in != null) {
							...
							if (blockExternalString == null) {
								...
								blockExternal = Boolean
										.parseBoolean(blockExternalString);
							}
							...
							try {
								double version = Double
										.parseDouble(this.jspversion);
								if (version < 2.0) {
									err.jspError(
											"jsp.error.invalid.implicit.version",
											path);
								}
		...
		if (tagFile == null) {
			...
			try {
				tagInfo = TagFileProcessor.parseTagFileDirectives(pc,
						shortName, path, pc.getJspCompilationContext()
								.getTagFileJarResource(path), this);
