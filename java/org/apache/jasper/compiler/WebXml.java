...
public class WebXml {
	...
	public WebXml(ServletContext ctxt) throws IOException {
		...
		String webXml = (String) ctxt
				.getAttribute(org.apache.tomcat.util.scan.Constants.MERGED_WEB_XML);
		if (webXml != null) {
	...
	public void close() {
		if (stream != null) {
			try {
				...
				log.error(Localizer.getMessage("jsp.error.stream.close.failed"));
			}
		}
	}
}
