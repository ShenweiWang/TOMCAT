...
final class Mark {
	...
	/*
	 * stack of stream and stream state of streams that have included current
	 * stream
	 */
	 ...
	 *
	 * @param reader
	 *            JspReader this mark belongs to
	 * @param inStream
	...
	public void pushStream(char[] inStream, int inFileId, String name,
		...
		includeStack.push(new IncludeState(cursor, line, col, fileId, fileName,
				baseDir, encoding, stream));
	...
	/**
	 * Keep track of parser before parsing an included file. This class keeps
	 * track of the parser before we switch to parsing an included file. In
	 * other words, it's the parser's continuation to be reinstalled after the
	 * included file parsing is done.
	 */
	class IncludeState {
		int cursor, line, col;
		int fileId;
