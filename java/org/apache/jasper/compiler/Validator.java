...
class Validator {
				...
				if ("language".equals(attr)) {
					...
					if (pageInfo.getDeferredSyntaxAllowedAsLiteral() == null) {
						...
						err.jspError(
								...
								pageInfo.getDeferredSyntaxAllowedAsLiteral(),
								value);
					}
	...
	static class ValidateVisitor extends Node.Visitor {
		...
		public void visit(Node.ParamAction n) throws JasperException {
			...
			n.setValue(getJspAttribute(null, "value", null, null,
					n.getAttributeValue("value"), n, null, false));
			visitBody(n);
		}
		...
		public void visit(Node.IncludeAction n) throws JasperException {
			JspUtil.checkAttributes("Include action", n, includeActionAttrs,
					err);
			n.setPage(getJspAttribute(null, "page", null, null,
					n.getAttributeValue("page"), n, null, false));
			visitBody(n);
		}
		...
		public void visit(Node.SetProperty n) throws JasperException {
			...
			n.setValue(getJspAttribute(null, "value", null, null, value, n,
					null, false));
		...
		public void visit(Node.NamedAttribute n) throws JasperException {
			...
			n.setOmit(getJspAttribute(null, "omit", null, null,
					n.getAttributeValue("omit"), n, null, false));
			visitBody(n);
		}
		...
		public void visit(Node.CustomTag n) throws JasperException {
			...
			Hashtable<String, Object> tagDataAttrs = new Hashtable<String, Object>(
					attrsSize);
		...
		private void checkXmlAttributes(Node.CustomTag n,
			...
			for (int i = 0; attrs != null && i < attrs.getLength(); i++) {
				...
				boolean runtimeExpression = ((n.getRoot().isXmlSyntax() && attrs
						.getValue(i).startsWith("%=")) || (!n.getRoot()
						.isXmlSyntax() && attrs.getValue(i).startsWith("<%=")));
				boolean elExpression = false;
				boolean deferred = false;
				double libraryVersion = Double.parseDouble(tagInfo
						.getTagLibrary().getRequiredVersion());
				boolean deferredSyntaxAllowedAsLiteral = pageInfo
				...
				for (int j = 0; tldAttrs != null && j < tldAttrs.length; j++) {
					if (attrs.getLocalName(i).equals(tldAttrs[j].getName())
						...
						if (tldAttr.canBeRequestTime()
							...
							if (!expression) {
								...
								if (tldAttr.isDeferredMethod()) {
									...
									if (m != null) {
										...
										int rti = m.indexOf(' ');
										if (rti > 0) {
											expectedType = m.substring(0, rti)
													.trim();
										}
									} else {
								...
								if (tldAttr.isDeferredValue()) {
									...
									// what is declared as type
									// for the attribute
									expectedType = tldAttr
											.getExpectedTypeName();
								}
								if (expectedType != null) {
									Class<?> expectedClass = String.class;
									try {
										expectedClass = JspUtil.toClass(
												expectedType, loader);
							...
							jspAttrs[i] = new Node.JspAttribute(tldAttr,
									attrs.getQName(i), attrs.getURI(i),
									attrs.getLocalName(i), textAttributeValue,
									false, null, false);
						}
						if (expression) {
			...
			if (value != null) {
				if (n.getRoot().isXmlSyntax() && value.startsWith("%=")) {
					...
					if (el != null) {
						...
						try {
							result.validateEL(
									this.pageInfo.getExpressionFactory(), ctx);
						} catch (ELException e) {
							this.err.jspError(n.getStart(),
									"jsp.error.invalid.expression", value,
									e.toString());
						}
					}
				}
		...
		private boolean isExpression(Node n, String value, boolean checkDeferred) {
			...
			boolean runtimeExpression = ((n.getRoot().isXmlSyntax() && value
					.startsWith("%=")) || (!n.getRoot().isXmlSyntax() && value
					.startsWith("<%=")));
			boolean elExpression = false;
				...
				public void visit(ELNode.Function n) throws JasperException {
					...
					try {
						...
						err.jspError("jsp.error.function.classnotfound", n
								.getFunctionInfo().getFunctionClass(),
								n.getPrefix() + ':' + n.getName(), e
										.getMessage());
					}
					...
					try {
						...
						err.jspError("jsp.error.noFunctionMethod",
								n.getMethodName(), n.getName(), c.getName());
					}
	...
	private static void validateXmlView(PageData xmlView, Compiler compiler)
		...
		for (Iterator<TagLibraryInfo> iter = compiler.getPageInfo()
				.getTaglibs().iterator(); iter.hasNext();) {
