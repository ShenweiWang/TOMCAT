...
class Generator {
			...
			private String createTagHandlerPoolName(String prefix,
				...
				if (attrs != null) {
					...
					for (int i = 0; i < namedAttrs.size(); i++) {
						attrNames[attrs.getLength() + i] = ((NamedAttribute) namedAttrs
								.getNode(i)).getQName();
					}
	...
	class GenerateVisitor extends Node.Visitor {
		...
		public void visit(Node.IncludeAction n) throws JasperException {
			...
			if (page.isNamedAttribute()) {
				...
				pageParam = attributeValue(page, false, String.class, n
						.getRoot().isXmlSyntax());
			}
		...
		public void visit(Node.SetProperty n) throws JasperException {
			...
			if ("*".equals(property)) {
				...
				out.printil("org.apache.jasper.runtime.JspRuntimeLibrary.handleSetProperty("
						...
						+ name
						+ "\"), \"" + property + "\",");
				out.print(attributeValue(value, false, null, n.getRoot()
						.isXmlSyntax()));
				out.println(");");
		...
		private TagHandlerInfo getTagHandlerInfo(Node.CustomTag n)
				throws JasperException {
			Hashtable<String, TagHandlerInfo> handlerInfosByShortName = handlerInfos
					.get(n.getPrefix());
			if (handlerInfosByShortName == null) {
				handlerInfosByShortName = new Hashtable<String, TagHandlerInfo>();
				handlerInfos.put(n.getPrefix(), handlerInfosByShortName);
			}
			TagHandlerInfo handlerInfo = handlerInfosByShortName.get(n
					.getLocalName());
			if (handlerInfo == null) {
				...
				if (attr.isNamedAttribute()) {
					if (n.checkIfAttributeIsJspFragment(attr.getName())) {
						...
						attrValue = generateNamedAttributeJspFragment(
								attr.getNamedAttributeNode(), tagHandlerVar);
					} else {
	...
	private void generateTagHandlerAttributes(TagInfo tagInfo) {
		...
		for (int i = 0; i < attrInfos.length; i++) {
			...
			out.print(JspUtil.makeJavaIdentifierForAttribute(attrInfos[i]
					.getName()));
			out.println(";");
		}
		out.println();
