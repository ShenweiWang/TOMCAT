...
public class SmapStratum {
	...
	public static class LineInfo {
		...
		/**
		 * Sets lineFileID. Should be called only when different from that of
		 * prior LineInfo object (in any given context) or 0 if the current
		 * LineInfo has no (logical) predecessor. <tt>LineInfo</tt> will print
		 * this file number no matter what.
		 */
		...
		/**
		 * Retrieves the current LineInfo as a String, print all values only
		 * when appropriate (but LineInfoID if and only if it's been specified,
		 * as its necessity is sensitive to context).
		 */
	...
	/**
	 * Constructs a new SmapStratum object for the given stratum name (e.g.,
	 * JSP).
	 *
	 * @param stratumName
	...
	/**
	 * Adds record of a new file, by filename and path. The path may be relative
	 * to a source compilation path.
	 *
	 * @param filename
	 *            the filename to add, unqualified by path
	 * @param filePath
	 *            the path for the filename, potentially relative to a source
	 *            compilation path
	 */
	...
	public void optimizeLineSection() {
		...
		/*
		 * Some debugging code for (int i = 0; i < lineData.size(); i++) {
	...
	/**
	 * Adds complete information about a simple line mapping. Specify all the
	 * fields in this method; the back-end machinery takes care of printing only
	 * those that are necessary in the final SMAP. (My view is that fields are
	 * optional primarily for spatial efficiency, not for programmer
	 * convenience. Could always add utility methods later.)
	 *
	 * @param inputStartLine
	 *            starting line in the source file (SMAP <tt>InputStartLine</tt>
	 *            )
	 * @param inputFileName
	 *            the filepath (or name) from which the input comes (yields SMAP
	 *            <tt>LineFileID</tt>) Use unqualified names carefully, and only
	 *            when they uniquely identify a file.
	 * @param inputLineCount
	 *            the number of lines in the input to map (SMAP
	 *            <tt>LineFileCount</tt>)
	 * @param outputStartLine
	 *            starting line in the output file (SMAP
	 *            <tt>OutputStartLine</tt>)
	 * @param outputLineIncrement
	 *            number of output lines to map to each input line (SMAP
	 *            <tt>OutputLineIncrement</tt>). <i>Given the fact that the name
	 *            starts with "output", I continuously have the subconscious
	 *            urge to call this field <tt>OutputLineExcrement</tt>.</i>
	 */
	public void addLineData(int inputStartLine, String inputFileName,
			int inputLineCount, int outputStartLine, int outputLineIncrement) {
	...
	/**
	 * Returns the given stratum as a String: a StratumSection, followed by at
	 * least one FileSection and at least one LineSection.
	 */
	public String getString() {
