...
class TagFileProcessor {
	...
	private static class TagFileDirectiveVisitor extends Node.Visitor {
		...
		private static final JspUtil.ValidAttribute[] tagDirectiveAttrs = {
				...
				new JspUtil.ValidAttribute("deferredSyntaxAllowedAsLiteral"), // JSP
																				// 2.1
		...
		private HashMap<String, NameEntry> nameTable = new HashMap<String, NameEntry>();

		private HashMap<String, NameEntry> nameFromTable = new HashMap<String, NameEntry>();
		...
		public void visit(Node.AttributeDirective n) throws JasperException {
			...
			if (("2.0".equals(tagLibInfo.getRequiredVersion()) || ("1.2"
					.equals(tagLibInfo.getRequiredVersion())))
		...
		private void checkUniqueName(String name, String type, Node n,
			...
			HashMap<String, NameEntry> table = (type == VAR_NAME_FROM) ? nameFromTable
					: nameTable;
	...
	public static TagInfo parseTagFileDirectives(ParserController pc,
			String name, String path, JarResource jarResource,
		...
		TagFileDirectiveVisitor tagFileVisitor = new TagFileDirectiveVisitor(
				pc.getCompiler(), tagLibInfo, name, path);
	...
	private Class<?> loadTagFile(Compiler compiler, String tagFilePath,
		...
		if (tagFilePath.startsWith("/META-INF/")) {
			tagJarResouce = compiler.getCompilationContext()
					.getTldLocation(tagInfo.getTagLibrary().getURI())
					.getJarResource();
		}
		String wrapperUri;
		...
		synchronized (rctxt) {
			...
			if (wrapper == null) {
				wrapper = new JspServletWrapper(ctxt.getServletContext(),
						ctxt.getOptions(), tagFilePath, tagInfo,
						ctxt.getRuntimeContext(), tagJarResouce);
			...
			try {
				if (tripCount > 0) {
					...
					JspServletWrapper tempWrapper = new JspServletWrapper(
							ctxt.getServletContext(), ctxt.getOptions(),
					...
					tempWrapper.getJspEngineContext().setClassLoader(
							ctxt.getClassLoader());
					tempWrapper.getJspEngineContext().setClassPath(
							ctxt.getClassPath());
			...
			try {
				...
				if (tagIns instanceof JspSourceDependent) {
					Iterator<Entry<String, Long>> iter = ((JspSourceDependent) tagIns)
							.getDependants().entrySet().iterator();
					while (iter.hasNext()) {
			...
			if (tagFileInfo != null) {
				...
				if (tagFilePath.startsWith("/META-INF/")) {
					...
					TldLocation location = compiler.getCompilationContext()
							.getTldLocation(
									tagFileInfo.getTagInfo().getTagLibrary()
											.getURI());
					JarResource jarResource = location.getJarResource();
					if (jarResource != null) {
						try {
							// Add TLD
							pageInfo.addDependant(
									jarResource.getEntry(location.getName())
											.toString(), Long
											.valueOf(jarResource
													.getJarFile()
													.getEntry(
							...
							// Add Tag
							pageInfo.addDependant(
									jarResource.getEntry(
											tagFilePath.substring(1))
											.toString(),
									Long.valueOf(jarResource.getJarFile()
											.getEntry(tagFilePath.substring(1))
											.getTime()));
