 ...
 *
 * This class is also responsible for localizing any error codes before they are
 * passed on to the configured error handler.
 * 
 * In the case of a Java compilation error, the compiler error message is parsed
 * into an array of JavacErrorDetail instances, which is passed on to the
...
public class ErrorDispatcher {
	 ...
	 * 
	 * The given error code is localized. If it is not found in the resource
	 * bundle for localized error messages, it is used as the error message.
	 * 
	 ...
	 * @param e Parsing exception
	 */
	public void jspError(Mark where, Exception e, String errCode,
			String... args) throws JasperException {
	 ...
	 *            Error message
	 * @param fname
	 *            Name of Java source file whose compilation failed
	 * @param page
	...
	public static JavacErrorDetail[] parseJavacErrors(String errMsg,
			String fname, Node.Nodes page) throws JasperException, IOException {
	 ...
	 * @param e Parsing exception
	 */
	private void dispatch(Mark where, String errCode, Object[] args, Exception e)
			throws JasperException {
		String file = null;
		String errMsg = null;
		int line = -1;
	...
	/*
	 * Parses the given Java compilation error message, which may contain one or
	 * more compilation errors, into an array of JavacErrorDetail instances.
	 * 
	 ...
	 * compilation errors
	 */
	private static JavacErrorDetail[] parseJavacMessage(String errMsg,
			String fname, Node.Nodes page) throws IOException, JasperException {
		...
		while ((line = reader.readLine()) != null) {
			 ...
			 * following drive letter on Windows (fromIndex = 2). XXX Handle
			 * deprecation warnings that don't have line info
			 */
	...
	public static JavacErrorDetail createJavacError(String fname,
		...
		if ((errNode != null) && (errNode.getStart() != null)) {
			...
			if (errVisitor.getJspSourceNode() instanceof Node.Scriptlet) {
				javacError = new JavacErrorDetail(fname, lineNum, errNode
						.getStart().getFile(), errNode.getStart()
						.getLineNumber()
						+ lineNum
				...
				javacError = new JavacErrorDetail(fname, lineNum, errNode
						.getStart().getFile(), errNode.getStart()
						.getLineNumber(), errMsgBuf, ctxt);
			}
		} else {
			/*
			 * javac error line number cannot be mapped to JSP page line number.
			 * For example, this is the case if a scriptlet is missing a closing
			 * brace, which causes havoc with the try-catch-finally block that
			 * the code generator places around all generated code: As a result
			 * of this, the javac error line numbers will be outside the range
			 * of begin and end java line numbers that were generated for the
			 * scriptlet, and therefore cannot be mapped to the start line
			 * number of the scriptlet in the JSP page. Include just the javac
			 * error info in the error detail.
			 */
