...
class PageDataImpl extends PageData implements TagConstants {
	...
	public PageDataImpl(Node.Nodes page, Compiler compiler)
		...
		SecondPassVisitor secondPass = new SecondPassVisitor(page.getRoot(),
				buf, compiler, firstPass.getJspIdPrefix());
	...
	static class FirstPassVisitor extends Node.Visitor implements TagConstants {
		...
		public FirstPassVisitor(Node.Root root, PageInfo pageInfo) {
			...
			this.rootAttrs
					.addAttribute("", "", "version", "CDATA", JSP_VERSION);
			...
			if (attrs != null) {
				...
				if (rootAttrs.getIndex(qName) == -1) {
					...
					if (location != null) {
						...
						rootAttrs
								.addAttribute("", "", qName, "CDATA", location);
					} else {
				...
				for (int i = 0; i < len; i++) {
					...
					if (rootAttrs.getIndex(qName) == -1) {
						rootAttrs.addAttribute(attrs.getURI(i),
								attrs.getLocalName(i), qName, attrs.getType(i),
								attrs.getValue(i));
					}
				}
			}
	...
	static class SecondPassVisitor extends Node.Visitor implements TagConstants {
		...
		private void appendText(String text, boolean createJspTextElement) {
			if (createJspTextElement) {
				...
				// append jsp:id
				buf.append("  ").append(jspIdPrefix).append(":id")
						.append("=\"");
				buf.append(jspId++).append("\"\n");
				buf.append(">\n");
