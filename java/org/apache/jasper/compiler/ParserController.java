 ...
 * segments included by it (via an include directive), where each segment may be
 * provided in standard or XML syntax. This class selects and invokes the
...
class ParserController implements TagConstants {
	...
	/*
	 * A stack to keep track of the 'current base directory' for include
	 * directives that refer to relative paths.
	 */
	 ...
	 *
	 * @param inFileName
	 *            The path to the JSP page or tag file to be parsed.
	 */
	public Node.Nodes parse(String inFileName) throws FileNotFoundException,
	 ...
	 *
	 * @param inFileName
	 *            The path to the resource to be included.
	 * @param parent
	 *            The parent node of the include directive.
	 * @param jarResource
	 *            The JAR file from which to read the included resource, or null
	 *            of the included resource is to be read from the filesystem
	 */
	public Node.Nodes parse(String inFileName, Node parent,
			JarResource jarResource) throws FileNotFoundException,
			JasperException, IOException {
	 ...
	 *
	 * @param inFileName
	 *            The name of the JSP page or tag file to be parsed.
	 * @param parent
	 *            The parent node (non-null when processing an include
	 *            directive)
	 * @param isTagFile
	 *            true if file to be parsed is tag file, and false if it is a
	 *            regular JSP page
	 * @param directivesOnly
	 *            true if the file to be parsed is a tag file and we are only
	 *            interested in the directives needed for constructing a
	 *            TagFileInfo.
	 * @param jarResource
	 *            The JAR file from which to read the JSP page or tag file, or
	 *            null if the JSP page or tag file is to be read from the
	 *            filesystem
	 */
	private Node.Nodes doParse(String inFileName, Node parent,
			JarResource jarResource) throws FileNotFoundException,
			JasperException, IOException {
		...
		JarFile jarFile = (jarResource == null) ? null : jarResource
				.getJarFile();
		...
		if ((isXml && isEncodingSpecifiedInProlog) || isBomPresent) {
			/*
			 * Make sure the encoding explicitly specified in the XML prolog (if
			 * any) matches that in the JSP config element (if any), treating
			 * "UTF-16", "UTF-16BE", and "UTF-16LE" as identical.
			 */
			if (jspConfigPageEnc != null
					&& !jspConfigPageEnc.equals(sourceEnc)
					&& (!jspConfigPageEnc.startsWith("UTF-16") || !sourceEnc
							.startsWith("UTF-16"))) {
		...
		if (isXml) {
			...
			// JspDocumentParser.
			parsedPage = JspDocumentParser
					.parse(this, absFileName, jarFile, parent, isTagFile,
							directiveOnly, sourceEnc, jspConfigPageEnc,
			...
			try {
				...
				JspReader jspReader = new JspReader(ctxt, absFileName,
						sourceEnc, inStreamReader, err);
				parsedPage = Parser.parse(this, jspReader, parent, isTagFile,
						directiveOnly, jarResource, sourceEnc,
	...
	/*
	 * Checks to see if the given URI is matched by a URL pattern specified in a
	 * jsp-property-group in web.xml, and if so, returns the value of the
	...
	private String getJspConfigPageEncoding(String absFileName)
		...
		JspConfig.JspProperty jspProperty = jspConfig
				.findJspProperty(absFileName);
		return jspProperty.getPageEncoding();
	}

	/**
	 * Determines the syntax (standard or XML) and page encoding properties for
	 * the given file, and stores them in the 'isXml' and 'sourceEnc' instance
	 * variables, respectively.
	 */
	private void determineSyntaxAndEncoding(String absFileName,
			JarFile jarFile, String jspConfigPageEnc) throws JasperException,
			IOException {

		isXml = false;

		/*
		 * 'true' if the syntax (XML or standard) of the file is given from
		 * external information: either via a JSP configuration element, the
		 * ".jspx" suffix, or the enclosing file (for included resources)
		 */
		...
		if (isExternal && !isXml) {
			...
			if (!isXml && sourceEnc.equals("UTF-8")) {
				 ...
				 * Therefore, we need to check to see if the page contains a
				 * <jsp:root> element.
				 * 
				 * We need to be careful, because the page may be encoded in
				 * ISO-8859-1 (or something entirely different), and may contain
				 * byte sequences that will cause a UTF-8 converter to throw
				 * exceptions.
				 * 
				 ...
				 * <jsp:root>) are identical in either encoding (both UTF-8 and
				 * ISO-8859-1 are extensions of ASCII).
				 */
				sourceEnc = "ISO-8859-1";
				revert = true;
			}
		...
		/*
		 * At this point, 'isExternal' or 'isXml' is FALSE. Search for jsp:root
		 * action, in order to determine if we're dealing with XML or standard
		 * syntax (unless we already know what we're dealing with, i.e., when
		 * 'isExternal' is TRUE and 'isXml' is FALSE). No check for XML prolog,
		 * since nothing prevents a page from outputting XML and still using JSP
		 * syntax (in this case, the XML prolog is treated as template text).
		 */
		...
		/*
		 * At this point, we know we're dealing with JSP syntax. If an XML
		 * prolog is provided, it's treated as template text. Determine the page
		 * encoding from the page directive, unless it's specified via JSP
		 * config.
		 */
		if (!isBomPresent) {
			sourceEnc = jspConfigPageEnc;
	...
	/*
	 * Determines page source encoding for page or tag file in JSP syntax, by
	 * reading (in this order) the value of the 'pageEncoding' page directive
	 * attribute, or the charset value of the 'contentType' page directive
	 * attribute.
	 * 
	...
	/*
	 * Scans the given attributes for the attribute with the given name, which
	 * is either 'pageEncoding' or 'contentType', and returns the specified page
	 * encoding.
	 * 
	 * In the case of 'contentType', the page encoding is taken from the content
	 * type's 'charset' component.
	 * 
	 ...
	 * the current base directory for each included file. The 'root' file is
	 * always an 'absolute' path, so no need to put an initial value in the
	 * baseDirStack.
	 */
