...
public class Set implements TagPlugin {
	...
	public void doTag(TagPluginContext ctxt) {
		...
		boolean hasValue = false, hasVar = false, hasScope = false, hasTarget = false;
		...
		// if the attribute var has been specified then assign the result to the
		// var;
		if (hasVar) {
			String jspCtxt = null;
			if (ctxt.isTagFile()) {
			...
			// if the target is a map, then put the result into the map with the
			// key property
			ctxt.generateJavaSource("    if(" + targetName
			...
			ctxt.generateJavaSource("            java.beans.PropertyDescriptor "
					...
					+ targetName + ".getClass()).getPropertyDescriptors();");
			...
			// get the "set" method;
			ctxt.generateJavaSource("                    java.lang.reflect.Method "
					+ methodName
					+ " = "
					+ pdName
					+ "["
			...
			ctxt.generateJavaSource("                    if(null == "
					+ methodName + "){");
			ctxt.generateJavaSource("                        throw new JspException(\"No setter method in &lt;set&gt; for property \"+"
					+ propertyName + ");");
			ctxt.generateJavaSource("                    }");
			...
			ctxt.generateJavaSource("                        "
					...
					+ ".invoke("
					+ targetName
					+ ", new Object[]{org.apache.el.lang.ELSupport.coerceToType("
