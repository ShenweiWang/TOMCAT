...
public class PageContextImpl extends PageContext {
	...
	private void _initialize(Servlet servlet, ServletRequest request,
		...
		this.applicationContext = JspApplicationContextImpl
				.getInstance(context);
		...
		isIncluded = request
				.getAttribute(RequestDispatcher.INCLUDE_SERVLET_PATH) != null;
	}
	...
	public void release() {
		...
		try {
			...
			IllegalStateException ise = new IllegalStateException(
					Localizer.getMessage("jsp.error.flush"), ex);
			throw ise;
		} finally {
			servlet = null;
			config = null;
	...
	public static Object proprietaryEvaluate(final String expression,
			...
			throws ELException {
		Object retValue;
		final ExpressionFactory exprFactory = jspf.getJspApplicationContext(
				pageContext.getServletContext()).getExpressionFactory();
		if (SecurityUtil.isPackageProtectionEnabled()) {
			try {
				retValue = AccessController
						.doPrivileged(new PrivilegedExceptionAction<Object>() {
							...
							public Object run() throws Exception {
								ELContextImpl ctx = (ELContextImpl) pageContext
										.getELContext();
								ctx.setFunctionMapper(new FunctionMapperImpl(
										functionMap));
								ValueExpression ve = exprFactory
										.createValueExpression(ctx, expression,
												expectedType);
