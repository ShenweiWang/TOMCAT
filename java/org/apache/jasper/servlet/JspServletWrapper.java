...
public class JspServletWrapper {
	...
	public JspServletWrapper(ServletContext servletContext, Options options,
		...
		ctxt = new JspCompilationContext(jspUri, tagInfo, options,
				servletContext, this, rctxt, tagJarResource);
	}
		...
		if (reload) {
			synchronized (this) {
				...
				if (reload) {
					...
					try {
						InstanceManager instanceManager = InstanceManagerFactory
								.getInstanceManager(config);
						servlet = (Servlet) instanceManager.newInstance(
								ctxt.getFQCN(), ctxt.getJspLoader());
					} catch (Exception e) {
	 ...
	 * compiling tag files with circular dependencies. A prototype (skeleton)
			...
			if (unloadAllowed) {
				synchronized (this) {
					if (unloadByCount) {
						...
						} else if (lastUsageTime < ctxt.getRuntimeContext()
								.getLastJspQueueUpdate()) {
							ctxt.getRuntimeContext().makeYoungest(unloadHandle);
							lastUsageTime = System.currentTimeMillis();
						}
					} else {
						if (lastUsageTime < ctxt.getRuntimeContext()
								.getLastJspQueueUpdate()) {
							lastUsageTime = System.currentTimeMillis();
						}
					}
	 ...
	 * http://issues.apache.org/bugzilla/show_bug.cgi?id=37062 and
		...
		try {
			...
			if (options.getDisplaySourceFragment()) {
				return new JasperException(Localizer.getMessage(
						...
						+ Constants.NEWLINE
						+ Constants.NEWLINE + "Stacktrace:", ex);
